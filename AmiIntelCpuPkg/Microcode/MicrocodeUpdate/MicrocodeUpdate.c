...
UINT8 *FindMicrocodeFfs(IN EFI_FIRMWARE_VOLUME_HEADER *FvHdr)
{
    ...
    while (FvPtr < EndOfFv && (UINTN)FvPtr < 0xffffffff) {
        if (CompareGuid(&gMcodeFfsguid, &((EFI_FFS_FILE_HEADER*)FvPtr)->Name))
            return FvPtr;
		
		if (MICROCODE_FFS_FILE_SIZE(FvPtr) != 0)
			FvPtr += MICROCODE_FFS_FILE_SIZE(FvPtr);
		else
			return NULL;
...
BOOLEAN InitMicrocodeVariables()
{
	...
	McodeFfsSize = MICROCODE_FFS_FILE_SIZE(FfsPtr);
#if MPDTable_CREATED
...
VOID *FindMicrocodeOfStepping(UINT8 *Mcode, UINT8 *End, UINT32 CpuSignature)
{
	...
	if (Mcode == NULL || End == NULL) return 0;
    for(ptr = Mcode; ptr < End; ptr += MICROCODE_ALIGN_SIZE(ptr)) {
        MICROCODE_HEADER* uC;
		
		if (*(UINT32*)ptr == 0xffffffff || *(UINT32*)ptr == 0) return 0; 
        
		uC = (MICROCODE_HEADER*)ptr;
		
		if (uC->TotalSize == 0 || uC->TotalSize == 0xffffffff || uC->DataSize == 0xffffffff || uC->CpuSignature == 0)
			return 0;
        ...
        if (uC->TotalSize > (uC->DataSize + 48)) {        //Extended signature count.
			...
			ExtSigCount = SigTable->Count;
			if (ExtSigCount == 0) continue;			
