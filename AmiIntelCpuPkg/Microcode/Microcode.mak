...
ifeq ($(USE_NEW_MPACK), 2)

Prepare : $(BUILD_DIR)$(PATH_SLASH)MICROCODE.BIN

ifeq ($(MPDTable_CREATED), 1)
$(BUILD_DIR)$(PATH_SLASH)MICROCODE.BIN :
	$(PYTHON_COMMAND) $(MICROCODE_DIR)$(PATH_SLASH)PackMicrocodes.py -s $(MICROCODE_SLOT_SIZE) -Mpdt -o $@ $(MICROCODE_FILES)
else
$(BUILD_DIR)$(PATH_SLASH)MICROCODE.BIN :
	$(PYTHON_COMMAND) $(MICROCODE_DIR)$(PATH_SLASH)PackMicrocodes.py -s $(MICROCODE_SLOT_SIZE) -o $@ $(MICROCODE_FILES)
endif
...
ifeq ($(SECONDARY_MICROCODE_FFS), 1)
Prepare : $(BUILD_DIR)/MICROCODE.BIN $(BUILD_DIR)/MICROCODE2.BIN
else
Prepare : $(BUILD_DIR)/MICROCODE.BIN
endif
else
ifeq ($(SECONDARY_MICROCODE_FFS), 1)
Prepare : $(BUILD_DIR)/MICROCODE.BIN $(BUILD_DIR)/MICROCODE_BLANK.BIN $(BUILD_DIR)/MICROCODE2.BIN
else
...
ifeq ($(SECONDARY_MICROCODE_FFS), 1)
Prepare : $(BUILD_DIR)/MICROCODE2.BIN
endif
...
$(BUILD_DIR)/MICROCODE2.BIN : $(BUILD_DIR)/Token.mak $(MICROCODE_DIR)/Microcode.mak
	$(MPACK) $(MPACK_ALIGN_FLAGS) -r $(MICROCODE2_PAD_SIZE) -o $@ $(MICROCODE2_FILES)

	
endif  #ifeq ($(USE_NEW_MPACK), 1)

#====================== OLD IMPLEMENTATION ========================
ifeq ($(USE_NEW_MPACK), 0)
