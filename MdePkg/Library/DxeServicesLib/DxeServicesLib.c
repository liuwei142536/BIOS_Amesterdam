...
//*** AMI PORTING BEGIN ***//
  //Fix assert when performance measurements are enabled
  //This function is used by SmmCorePerformanceLib to locate SmmCore FV; however,
  //SmmCore does not have a proper image handle, which triggered assert in the original code.
