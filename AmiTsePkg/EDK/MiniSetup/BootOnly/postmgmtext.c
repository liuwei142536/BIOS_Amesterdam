...
extern EFI_STATUS ExportSetupNVLockPassword(CHAR16* Password);
/**
...
UINT32 CheckSystemPassword(UINT32 EmptyPasswordType, UINTN *NoOfRetries, UINTN *TimeOut)
{
    ...
    ExportSetupNVLockPassword(PasswordEntered);
...
VOID _DrawPasswordWindow(UINT16 PromptToken, UINTN PasswordLength, UINTN *CurrXPos, UINTN *CurrYPos)
{
   ...
   if (!EFI_ERROR (Status))
    ...
    //Set desired Mode.
		...
		{
			SPrint( PasswordPrompt,((StrLen(Title)+3)*sizeof(CHAR16)), L" %s ",Title );
			...
			if ( IsTSEMultilineControlSupported() ) //If multiline supported
				{
					...
					tempText = StringWrapText(PasswordPrompt,(UINT16)DlogWidth,&Height);
		...
		*CurrXPos = (gMaxCols-2 - DlogWidth) / 2;
		...
		DrawWindow( *CurrXPos, *CurrYPos, DlogWidth, 4+(UINTN)Height, GetPasswordPopupWindowColor(), TRUE, FALSE );	
		...
		if ( PasswordPrompt != NULL )
		{
			if ( IsTSEMultilineControlSupported() ) //If multiline supported
			{
					if ((TestPrintLength(PasswordPrompt) / NG_SIZE +6) <= (gMaxCols-2))
						...
						DrawMultiLineStringWithAttribute ((*CurrXPos)+2, *CurrYPos, DlogWidth-3, (UINTN)Height, PasswordPrompt, 0);
			}
			else
				DrawStringJustified( 0, gMaxCols-2, *CurrYPos, JUSTIFY_CENTER, PasswordPrompt );
		}
   	...
   	*CurrXPos = ((gMaxCols - DlogWidth) / 2)+1;
		*CurrYPos += (UINTN)Height+1;

		FlushLines( *CurrYPos - (UINTN)Height-1, *CurrYPos + 2);
		DoRealFlushLines();
	    MouseRefresh();
	}
}
