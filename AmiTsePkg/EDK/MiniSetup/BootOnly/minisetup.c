...
CHAR16 *gMainTitle =  L"Aptio Setup - AMI";
CHAR16 *gMainCopyright = L"Version %x.%02x.%04x Copyright (C) %04x AMI";
...
EFI_STATUS MiniSetupExit( EFI_STATUS Status )
{
	...
	if(gST->ConOut!=NULL)
	{
		...
		gST->ConOut->EnableCursor( gST->ConOut, PcdGetBool(PcdDefaultCursorState) );
	}
...
{
	SETUP_DEBUG_TSE ("[TSE] InTseGopNotificiationFunciton(), gPostStatus = %s\n", gPostStatusArray[gPostStatus]);

    if((TSE_POST_STATUS_BEFORE_POST_SCREEN == gPostStatus) || (TSE_POST_STATUS_PROCEED_TO_BOOT == gPostStatus) || IsSetupUseGopSplitter()) // Don't do anything before post screen or entered into boot
		return;
...
void PrintGopAndConInDetails()
{
	...
	{
		SETUP_DEBUG_TSE ("[TSE] gST->ConOut = %x, MaxMode = %d, Mode = %d\n", gST->ConOut, gST->ConOut->Mode->MaxMode,gST->ConOut->Mode->Mode);
		for ( i = 0; i < (UINTN)gST->ConOut->Mode->MaxMode; i++ )
		{
			Status = gST->ConOut->QueryMode( gST->ConOut, i, &ModeCols, &ModeRows );
			if(Status == EFI_SUCCESS)
				SETUP_DEBUG_TSE ("[TSE] Mode = %d:[Cols = %d, Rows = %d]\n", i, ModeCols,ModeRows);
		}
	}
#endif
}
