...
#define EFIVARSTORE_LOOKUP_TABLE_SUPPORT 1
...
VOID InitAmiLib( IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable);  // hii.c
...
extern BOOLEAN          			gApplicationDataRelocated;
...
#define PRINT_UEFI_VAR          16
    ...
    #define SETUP_DEBUG_VAR(format,...) if((gDbgPrint & PRINT_UEFI_VAR)== PRINT_UEFI_VAR) SetupDebugPrint(format, ##__VA_ARGS__)
...
VOID RedrawGif(VOID);
BOOLEAN LoadOptionhidden(UINT16, BOOLEAN);
...
UINT16 GetMsgboxWidth(VOID); // MAX_MSGBOX_WIDTH macro changed as token and handled from binary
...
typedef struct _TSE_VARIABLE_DATA
{
...
Still not grouped separately to suppress in ESA build since same variables are used in ESA side which are defined in EsaSrcHelper.c*/ 
...
struct _TSE_SETUP_GLOBAL_DATA
{
	...
	TSE_VARIABLE_DATA VariableData;
