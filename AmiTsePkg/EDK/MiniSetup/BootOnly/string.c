...
BOOLEAN IsLineBreak(CHAR16 Character );
/**
...
CHAR16 *StrDup( CHAR16 *string )
{
	...
	if ( text != NULL )
		StrCpyS( text, 1 + EfiStrLen( string ), string );
    ...
    for(i=0; i<TextLen; (*height)++)
    {
        ...
        if( width >= (TestPrintLength(ThisLine)/(NG_SIZE)) ) //Can accomodate this line
        {
            if ((IsANSIEscapeCodeSupported ()) && (*ESCCode))
            {
        		...
        		{
        		    StrCpyS (newText + newTextLen, StrLenforBoundaryCheck - newTextLen, ThisLine);
        		    
        		}
        		else
        		{
        		    StrCpyS (newText + newTextLen, StrLenforBoundaryCheck - newTextLen, ESCCode);
     
	                 StrCatS (newText + newTextLen, StrLenforBoundaryCheck - newTextLen, ThisLine);   
                ...
                if(ThisLine[j] == L' ')
                {
							...
							StrCpyS (newText + newTextLen, StrLenforBoundaryCheck - newTextLen, ThisLine);
								...
								else
								{
	                             ...
	                             StrCatS (newText + newTextLen, StrLenforBoundaryCheck - newTextLen, ESCCode);
        ...
        if(!((IsANSIEscapeCodeSupported ()) && (*ESCCode) ) && *(text+i) == L'\0')
        {
            newText[newTextLen] = L'\n';
            newTextLen++;
            i++;
...
CHAR8 *StrDup8( CHAR8 *string )
{
	...
	if ( text != NULL )
	    AsciiStrCpyS( text, 1 + StrLen8( string ), string );
	return text;
}
