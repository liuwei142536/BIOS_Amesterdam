...
VOID DrawBootOnlyBbsPopupMenu( VOID )
{
    ...
    if (gNumOfDev) 
    {
        ...
        {
            DrawStringWithAttribute( START_COL_IDX, START_ROW_IDX,text,EFI_LIGHTGRAY);
        }
    }
	...
	while(TRUE)
	{
        ...
        case _DOWN_ARROW:
            ...
            else
            {
                 if(gStrIdx == 0)
                 {
                     DrawStringWithAttribute( START_COL_IDX, (START_ROW_IDX+gHiStr), gDevNames[gStrIdx].String, EFI_LIGHTGRAY);
                     gStrIdx = (gNumOfDev-1);
                     DownDisplayList(gStrIdx-(DispLines-1), gStrIdx, DispLines);
                 }
                 gHiStr = DispLines-1;
			...
			case _ENTER_KEY:
				if (gNumOfDev) {
					if( ( gStrIdx >= 0) && ( gStrIdx <= gNumOfDev) ) {
						...
						PERF_START (0, AMITSE_TEXT("Boot"), NULL, 0);
						gBBSPopupSel = (UINT16)gDevNames[gStrIdx].Index;
						EnoughOfThis = 1 ;
					}
				}
				break;
...
void CheckStrings(void)
{
	...
	for( idx=0; idx < gNumOfDev; idx++ ) {
		...
		if(Length > MAX_STR_LEN)
		{
			...
			BbsStrnCpy (TmpStr + EfiStrLen(TmpStr), L"...", (MAX_STR_LEN - EfiStrLen(TmpStr)));
...
VOID RestorePasswordScreen()
{
	...
	SETUP_DEBUG_TSE ("[TSE] RestorePasswordScreen\n");
	if(gST && gST->ConOut)
	{
