...
VOID InitPostScreen( VOID )
{
   ...
   UINTN       LogoType = 0;
	...
	if ( pConOut == NULL )
	{
		SETUP_DEBUG_TSE("[TSE] InitPostScreen Exiting since pConOut = NULL\n");
		return;
	}
	SETUP_DEBUG_TSE ("[TSE]gPostStatus = %s\n",gPostStatusArray[gPostStatus]);
	PrintGopAndConInDetails();
	SETUP_DEBUG_TSE ("[TSE]Initial gMaxCols = %d gMaxRows = %d \n",gMaxCols,gMaxRows);
	...
	if(gConsoleControl != NULL)
	{
		...
		SETUP_DEBUG_TSE("[TSE] Setting Text screen\n");
	}
#endif
#endif
	...
	if ( gPostStatus > TSE_POST_STATUS_IN_TSE ) 
	{
		 ...
		 SETUP_DEBUG_TSE("[TSE] Not drawing post screen/logo since currently not in or before Post screen\n");
		 return;
	}
	SETUP_DEBUG_TSE ("[TSE] Before Drawing Post Logo :\n");
	PrintGopAndConInDetails();
    SETUP_DEBUG_TSE ("[TSE]Updated gMaxCols = %d gMaxRows = %d \n",gMaxCols,gMaxRows);
	...
	if(gGOP )
	{
		...
		SETUP_DEBUG_TSE ("[TSE] Draw Post Logo at [%dx%d]\n",CoordinateX,CoordinateY);
	...
	MouseRefresh();
	SETUP_DEBUG_TSE("[TSE] InitPostScreen: Done\n\n");
}
...
VOID InvalidateStatusInBgrt (VOID)
{
	...
	if(BgrtTable->Status != 0)
	{
	    ...
	    AcpiHdr->Checksum = 0;
	    //Since changing the status bit, recalculating the checksum
	    AcpiHdr->Checksum = CalculateChecksum (gBgrtSafeBuffer, AcpiHdrSize + (UINT8)sizeof (BGRT_TABLE));
    }
	...
	SETUP_DEBUG_UEFI("\n[TSE] InvalidateStatusInBgrt(),BgrtTable Status updated to %d\n",BgrtTable->Status);  
}
/**
...
VOID ReInstallBgrt(VOID)
{
    ...
    SETUP_DEBUG_TSE ("In ReInstall Bgrt\n");
...
VOID UninstallBGRT (VOID)
{
	...
	EFI_GUID    AcpiTableProtocolGuid = EFI_ACPI_TABLE_PROTOCOL_GUID;
	EFI_ACPI_TABLE_PROTOCOL *AcpiTableInstance = NULL;
	SETUP_DEBUG_TSE ("In UninstallBGRT\n");
