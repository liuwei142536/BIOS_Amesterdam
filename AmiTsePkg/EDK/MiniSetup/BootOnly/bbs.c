...
{
    CHAR16  varName [9];             //Bootxxxx + 1 NULL char
    BOOT_OPTION *pOption;
    UINTN OptionSize;
    ...
    SPrint( varName,sizeof (varName),gBootFormarSpecifier, pBootData->Option );
...
//	UINT16        DevType=0; unused Variable
	...
	for(j=0;j<BootOrderCount;j++)
	{
		...
		if ( BBSValidDevicePath( pBootData->DevicePath ) )
		{
		    if((size < pBootData->LegacyEntryOffset) || ((size - pBootData->LegacyEntryOffset)< sizeof(BBS_ORDER_TABLE)))
		    {
...
VOID _BBSSetDevicePriorities( BBS_TABLE *BBSInfo, UINT16 *Priority, BOOT_DATA *BootData )
{
    ...
    if((size < BootData->LegacyEntryOffset)||((size - BootData->LegacyEntryOffset)< sizeof(BBS_ORDER_TABLE)))
    {
        MemFreePointer((VOID **) &pDevOrder);
        return;
    }
...
EFI_STATUS BBSSetBootNowPriority( BOOT_DATA *BootData,UINTN uiPrefferedDevice,BOOLEAN ShowAllBbsDev)
{
	...
	if(!ShowAllBbsDev)
	{
		...
		if ( (pDev->Length >= size) ||(0 == pDev->Length) ) 
		{
			...
			if(!IsBootToDisabledBootOption())
			{
				for ( i = 0; i < count; i++ )
				{
					if(((DISABLED_BOOT_OPTION != pDev->Data[i] ) || (gBootNextFlag == TRUE ))&& BBSInfo)
					{
            ...
            MemFreePointer((VOID **) &pDevOrder); //end of if(!ShowAllBbsDev)
	...
	else
	{
	   ...
	   if(BBSInfo && BBSList && (uiPrefferedDevice < count))
	   {
	       if(BBSList[uiPrefferedDevice].Index < BBSCount)
	       {
    ...
    else
    {
        ...
        if ( Status != EFI_SUCCESS ) {
            ...
            case BBS_TYPE_FLOPPY:
                StrCpyS(String, MAX_DRIVE_NAME, L"Floppy");
                break;

            case BBS_TYPE_HARDDRIVE:
                StrCpyS(String, MAX_DRIVE_NAME, L"Hard Drive");
            ...
            case BBS_TYPE_CDROM:	           
                StrCpyS(String, MAX_DRIVE_NAME, L"CDROM");
            ...
            case BBS_TYPE_EMBEDDED_NETWORK:
                StrCpyS(String, MAX_DRIVE_NAME, L"Network");
                break;

            case BBS_TYPE_DEV:
                StrCpyS(String, MAX_DRIVE_NAME, L"BEV Device");
                break;

            case BBS_TYPE_UNKNOWN:
            default:
                StrCpyS(String, MAX_DRIVE_NAME, L"Unknown");
                break;
            }
        }
    }
...
VOID UpdatePlatformRecoveryLegacyData ()
{
	...
	BBS_ORDER_TABLE *LegacyOrder = NULL, *NewLegacyOrder = NULL, *DevEntry, *NewDevEntry = NULL;
