...
{
    return GenericHookType9( HOOKID40, (TSE_HOOK *)SetupReentryHook );
}


EFI_STATUS GenericHookType10( UINTN HookNum, TSE_HOOK * defaultfn, EFI_GUID* OldPageGuid, UINT16* OldPageFormId, EFI_GUID* NewPageGuid, UINT16* NewPageFormId)
{
    EFI_STATUS ret = EFI_SUCCESS;
    UINTN ii;
    UINTN firsthookfn = 0;
    ...
    {
        for ( ii = firsthookfn; ii <= lasthookfn; ii++ )
            ret = ((TSE_HOOK_T10 *)AMITSE_HookList[ii])(OldPageGuid, OldPageFormId, NewPageGuid, NewPageFormId);
    }
    else
        ret = ((TSE_HOOK_T10 *)defaultfn)(OldPageGuid, OldPageFormId, NewPageGuid, NewPageFormId);
...
{
    return GenericHookType10( HOOKID41, (TSE_HOOK *)SetupPageChangeHook, OldPageGuid, OldPageFormId, NewPageGuid, NewPageFormId );
}
