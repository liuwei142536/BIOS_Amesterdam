...
typedef struct {
    ...
    CHAR16 *String;
    UINT16 Token;
} SPECIAL_CONTROL_INFO;

static UINTN SpecialCtrlInfoCount;
static SPECIAL_CONTROL_INFO *SpecialCtrlInfo = NULL;
...
VOID _SpecialFixupLanguages( CONTROL_INFO *control )
{
	...
	SETUP_DEBUG_TSE("\n[TSE] Language: StringID = 0x%X: %s", GetSpecialCtrlToken( gHiiHandle, tempPtr ), tempPtr);
...
VOID _SpecialFixupBootOrder (CONTROL_INFO *control, UINT16 value)
{
  ...
  length = UefiGetIfrLength(templatePtr); 
	...
	for ( i = 0; i < gBootOptionCount; i++ ) {
		...
		if(i == value)
		{
		    SETUP_DEBUG_TSE("\n[TSE] Boot Option: StringID = 0x%X: %s", option, BootGetOptionName( &(gBootData[i]) ));
		}
	}
...
VOID _SpecialFixupDriverOrder (CONTROL_INFO *control, UINT16 value)
{
	...
	for ( i = 0; i < gDriverOptionCount; i++ )
	{
		...
		if(i == value)
		{
		    SETUP_DEBUG_TSE("\n[TSE] Driver Option: StringID = 0x%X: %s", option, DriverGetOptionName( &(gDriverData[i]) ));
		}
	}
	//Create one of option for Disabled
	flag = 0;
...
VOID _SpecialFixupBBSOrder( CONTROL_INFO *control, UINT16 value )
{
  ...
  length = UefiGetIfrLength(templatePtr);
    ...
    UefiSetOneOfOption(oneOf, bbsOptionVal, BOOT_OPTION_SIZE, flag, option);
    	...
    	if(i == value)
    	 {
    	    SETUP_DEBUG_TSE("\n[TSE] BBS Option: StringID = 0x%X: %s", option, BBSGetOptionName( &bbsOptionVal,(UINT16)i ));
    	 }
	}
...
VOID SpecialFixupDelBootOption (CONTROL_INFO *control)
{
	...
	for ( i = 0; i < gBootOptionCount; i++ )
	{
		...
		else
		{
			...
			SETUP_DEBUG_TSE("\n[TSE] Del BootOption: StringID = 0x%X: %s", option, BootGetOptionName (&(gBootData[i])));
			// Add one of option
			UefiSetOneOfOption (oneOf, gBootData[i].Option, BOOT_OPTION_SIZE, flag, option);
			oneOf = (UINT8*)oneOf +length;
		}
...
VOID _SpecialFixupAddBootOption( CONTROL_INFO *control)
{
    ...
    // Add one of option
		...
		SETUP_DEBUG_TSE("\n[TSE] Add Boot Option: StringID = 0x%X: %s", option, gFsList[i].FsId);
...
VOID SpecialFixupDelDriverOption (CONTROL_INFO *control)
{
	...
	for (i = 0; i < gDriverOptionCount; i++)
	{
		...
		else
		{
			...
			SETUP_DEBUG_TSE("\n[TSE] Del Driver Option: StringID = 0x%X: %s", option, DriverGetOptionName (&(gDriverData [i])));
			// Add one of option
			UefiSetOneOfOption (oneOf, gDriverData [i].Option, BOOT_OPTION_SIZE, flag, option);
			oneOf = (UINT8*)oneOf +length;
		}
		...
		switch(ctrlData->ControlData.ControlType) 
		{
            case CONTROL_TYPE_SUBMENU:// submenu control
                ...
                if (guidInfo->GuidKey == GUID_KEY_BOOT_NOW)
                {
                    ...
                    SETUP_DEBUG_TSE("\n[TSE] Boot Now: StringID = 0x%X: %s", token, BootGetBootNowName(value,gShowAllBbsDev,BootNowInBootOrderSupport()));
		        ...
		        else if ( guidInfo->GuidKey == GUID_KEY_ERROR_MANAGER )
		        {
                    ...
                    if ( ( errManager != NULL ) && ( value < errManager->ErrorCount ) )
                    {
                        ...
                        text = HiiGetString( ctrlData->ControlData.ControlHandle, ctrlData->ControlData.QuestionId);
                        SETUP_DEBUG_TSE("\n[TSE] Error Manager: StringID = 0x%X: %s",ctrlData->ControlData.QuestionId , text);                       
			        }
                    if( errManager != NULL )
						...
						if((pageInfo->PageFlags.PageDynamic == TRUE) && (pageInfo->PageHandle != NULL))
					 	{
							if(Index == value)
							{
		                        ...
		                        SETUP_DEBUG_TSE("\n[TSE] Dynamic Page: StringID = 0x%X: %s", token, text);
                        		UefiSetPromptField(ctrlData->ControlData.ControlPtr, token);
                        		 MemFreePointer( (VOID **)&text ); 
						...
						if((pageInfo->PageFlags.PageDynamic == TRUE) && (pageInfo->PageHandle != NULL))
					 	{
						    ...
						    {    
						        if(Index == value)
                                {
                                    ...
                                    text = HiiGetString( pageInfo->PageHandle, pageInfo->PageSubTitle);
                                    token = GetSpecialCtrlToken( ctrlData->ControlData.ControlHandle, text);
                                    SETUP_DEBUG_TSE("\n[TSE] Dynamic Device Page: StringID = 0x%X: %s", token, text);
                                    UefiSetPromptField(ctrlData->ControlData.ControlPtr, token);
                                     MemFreePointer( (VOID **)&text ); 
                                    // Display Formset Help for Dynamic page
                                    if(Help != 0)
						...
						if((pageInfo->PageFlags.PageDynamic == TRUE) && (pageInfo->PageHandle != NULL))
						{
							...
							{    
								if(Index == value)
								{
									...
									text = HiiGetString( pageInfo->PageHandle, pageInfo->PageSubTitle);
									token = GetSpecialCtrlToken( ctrlData->ControlData.ControlHandle, text);
									SETUP_DEBUG_TSE("\n[TSE] Dynamic Page class: StringID = 0x%X: %s", token, text);
									UefiSetPromptField(ctrlData->ControlData.ControlPtr, token);
									MemFreePointer( (VOID **)&text ); 
									// Display Formset Help for Dynamic page
									if(Help != 0)
				...
				{             
					...
					if(IDEStrToken)
					{
						...
						token = GetSpecialCtrlToken( ctrlData->ControlData.ControlHandle, text);
						SETUP_DEBUG_TSE("\n[TSE] IDE Security: StringID = 0x%X: %s", token, text);
						//Update the submenu prompt field.
						UefiSetPromptField(ctrlData->ControlData.ControlPtr, token);
						MemFreePointer( (VOID **)&text );
					}
...
/**/
                    ...
                    for(i=0,j=0; i<gBootOptionCount; i++)
                    {
                        ...
                        if ( BBSValidDevicePath(bootData->DevicePath) )
                        {
                            if(value == j)
                            {
                                ...
                                token = GetSpecialCtrlToken( ctrlData->ControlData.ControlHandle, text);
                                SETUP_DEBUG_TSE("\n[TSE] BBS Order: StringID = 0x%X: %s", token, text);
                                //Update the submenu prompt field.
                                UefiSetPromptField(ctrlData->ControlData.ControlPtr, token);
				...
				else if (GUID_KEY_DRIVER_HEALTH == guidInfo->GuidKey)
				{
					...
					{
						token = GetSpecialCtrlToken (ctrlData->ControlData.ControlHandle, text);
						SETUP_DEBUG_TSE("\n[TSE] Driver Health: StringID = 0x%X: %s", token, text);
						UefiSetPromptField (ctrlData->ControlData.ControlPtr, token);
					}
					MemFreePointer( (VOID **)&text );
				}
				else if (GUID_KEY_DRV_HEALTH_CTRL_COUNT == guidInfo->GuidKey)
				{
					...
					{
						token = GetSpecialCtrlToken (ctrlData->ControlData.ControlHandle, text);
						SETUP_DEBUG_TSE("\n[TSE] Driver Health Controller: StringID = 0x%X: %s", token, text);
						UefiSetPromptField (ctrlData->ControlData.ControlPtr, token);
					}
					MemFreePointer( (VOID **)&text );
				}
				...
				else if (GUID_KEY_DYNAMIC_PAGE_GROUP == guidInfo->GuidKey)
                {
                    ...
                    {
                        token = GetSpecialCtrlToken (ctrlData->ControlData.ControlHandle, text);
                        SETUP_DEBUG_TSE("\n[TSE] Dynamic Page Group: StringID = 0x%X: %s", token, text);
                        UefiSetPromptField (ctrlData->ControlData.ControlPtr, token);
                    }
                    MemFreePointer( (VOID **)&text );
...
VOID FreeControlConditionPtr(UINT32 CurrentPage)
{
	...
	if (thisPage->PageHandle == NULL || thisPage->PageHandle == (VOID *)(UINTN)0xFFFF)
	    return;
...
VOID FreeSpecialCtrlInfo()
{
    if (SpecialCtrlInfo)
    {
        UINTN i;

        for (i = 0; i < SpecialCtrlInfoCount; i++) 
        {
            if (SpecialCtrlInfo[i].String) 
                MemFreePointer((VOID **)&SpecialCtrlInfo[i].String);
        }
        MemFreePointer((VOID**)&SpecialCtrlInfo);
        SpecialCtrlInfoCount = 0;
    }
}
...
UINT16 GetSpecialCtrlToken(VOID* handle, CHAR16 *String) 
{
    ...
    if(!SpecialCtrlInfo)
    {   
        SpecialCtrlInfoCount = MAX_SPECIAL_CTRL_COUNT;
        SpecialCtrlInfo = EfiLibAllocateZeroPool(sizeof(*SpecialCtrlInfo) * MAX_SPECIAL_CTRL_COUNT);
    }
    
    for (i = 0; i < SpecialCtrlInfoCount; i++)
    {
        if (0 == SpecialCtrlInfo[i].Token)
            break;

        if (SpecialCtrlInfo[i].Handle != handle)
            continue;

        if (0 != EfiStrCmp(SpecialCtrlInfo[i].String, String)) 
            continue;

        Text = HiiGetString( handle, SpecialCtrlInfo[i].Token);
        if (0 != EfiStrCmp(SpecialCtrlInfo[i].String, Text)) 
        {   
            SpecialCtrlInfo[i].Handle = NULL;
            MemFreePointer((VOID **)&Text);
            continue;
        }

        MemFreePointer((VOID **)&Text);
        return SpecialCtrlInfo[i].Token;
    }

    if (i == SpecialCtrlInfoCount)
    {
        SpecialCtrlInfo = MemReallocateZeroPool(
                            SpecialCtrlInfo,
                            sizeof(*SpecialCtrlInfo) * SpecialCtrlInfoCount,
                            sizeof(*SpecialCtrlInfo) * (SpecialCtrlInfoCount + MAX_SPECIAL_CTRL_COUNT));
        SpecialCtrlInfoCount += MAX_SPECIAL_CTRL_COUNT;
    }
    SpecialCtrlInfo[i].Token = HiiAddString(handle, String);
    SpecialCtrlInfo[i].Handle = handle;
    SpecialCtrlInfo[i].String = AllocateCopyPool (StrSize (String), String);
