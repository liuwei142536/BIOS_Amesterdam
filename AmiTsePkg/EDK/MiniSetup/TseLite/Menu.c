...
VOID UpdateMenuEntry(MENU_DATA *menu);
...
EFI_STATUS MenuDraw( MENU_DATA *menu )
{
    ...
    if(IsLinkHistorySupport() && (menu->IsSubMenu) && !IsSubMenuDisplayTitle())
	...
	for(i=0;(i < ItemsInPage) && ( i+j < menu->NumEntries) ;i++)
	{
        ...
        if( (Pos-2+length) >=(UINTN) (menu->MenuWidth-2) )
        {
      	  ...
      	  if ((AvailableWidth >= SLength)&& (TLength >= SLength))          
      	  {
      		  ...
      		  {
      			  DestMax = (EfiStrLen(text) + 1) - (TLength - SLength);
      		      StrCpyS( &text[TLength - SLength], DestMax, L"...");
      		    
      		  }
      		  else
      		  {
      		    DestMax = (EfiStrLen(text) + 1)  - BoundaryLength;
      		    StrCpyS( &text[BoundaryLength], DestMax, L"...");
...
/**
    Updates the CurrEntry for the sub menu's by finding the real parent using GetParentFormID 

    @param object MENU_DATA *menu
...
VOID UpdateMenuEntry(MENU_DATA *menu)
{
    ...
    Page = gApp->PageList[gApp->CurrentPage]; 
    INFO = (PAGE_INFO*)&( Page->PageData); // Current Page Info
    PageId = INFO->PageID; //Page ID of current page

    if( (INFO->PageHandle == NULL ) || (INFO->PageHandle == (VOID*)(UINTN)0xffff ) || (INFO->PageParentID  == 0) ) //Check for Invalid page and is this rootpage ?
        return;

    if( menu->ControlData.ControlPageID == PageId )
    {
        while(!Found)
        {
            (VOID)GetParentFormID((UINT16)PageId, &ParentPgID, &stackIndex); //Gets the parent page of the current page
            ...
            if((ParentPgID ==0 ))
            {
                for(j=0;j<menu->NumEntries;j++)
                {
                    if( ( PageId == menu->Entries[j].PageID) )
                    {
                        menu->CurrEntry =  j; //Found rootmenu
                        Found = TRUE;
                    }
                }  
                if(!Found) // If not able to found making Main menu as CurrEntry
                {
                    menu->CurrEntry =  0;
                    Found = TRUE;
            ...
            PageId = ParentPgID; // make parent page as current page for iteration
            ParentPgID = 0 ;
        }
    }
}
