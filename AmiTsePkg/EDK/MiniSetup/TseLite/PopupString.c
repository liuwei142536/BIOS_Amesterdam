...
EFI_STATUS PopupStringDraw(POPUP_STRING_DATA *popupstring )
{
	...
	if ( text != NULL )
	{
		...
		Status = VarGetValue( popupstring->ControlData.ControlVariable, UefiGetQuestionOffset(ifrData), UefiGetWidth(ifrData), (VOID *)temptxt );
		//If temptxt is NULL then getting the status as EFI_INVALID_PARAMETER from VarGetValue
	...
	if(ifrData)
	{
		...
		if(Len)
		{
			...
			StrCpyS(text, (Len+1), popupstring->Text);
            ...
            {			
    			if ( Len  > (UINTN)(popupstring->Width - popupstring->TextMargin) )
    			{
    			    ...
    			    DestMax = (EfiStrLen(text)+1) - (Len - 3);
    				StrCpyS( &(text[Len-3 /*3 for "..."*/]), DestMax, L"..." );
    			}
...
EFI_STATUS PopupStringHandleAction( POPUP_STRING_DATA *popupstring, ACTION_DATA *Data)
{
	...
	if ( Data->Input.Type == ACTION_TYPE_KEY )
    {
		...
		StrCpyS(TmpPopupStringText, (popupstring->TextWidth + 1), popupstring->Text);
		...
		if(_DoPopupStringEdit( popupstring,UefiGetPromptField(popupstring->ControlData.ControlPtr), &(popupstring->Text)) == EFI_SUCCESS)
		{
			...
			if(((TmpNewPopupStringTextWidth < MinVal) || (TmpNewPopupStringTextWidth > MaxVal)))
			{
				...
				if(TmpPopupStringText) {
					...
					StrCpyS(popupstring->Text, (MaxVal + 1), TmpPopupStringText);
						MemFreePointer((VOID **)&TmpPopupStringText);
				}
				//Report Message box  
            ...
            if ( UefiIsInteractive(&popupstring->ControlData) )
            {
				...
				if(EFI_ERROR(Status))
				{
					...
					if(TmpPopupStringText) {
					    ...
					    StrCpyS(popupstring->Text, (popupstring->TextWidth + 1), TmpPopupStringText);
