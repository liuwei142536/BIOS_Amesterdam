...
extern UINTN gMaxBufX,gPSMaxRows;
extern UINTN gMaxBufY,gPSMaxCols;
...
EFI_STATUS MiniSetupEntry(VOID)
{
    ...
    DebugShowPrintPageDetails((UINT16)0,(UINT16)gPages->PageCount);
#endif 
    gDisableFlag = SkipDisableOp; 
    gSetupStatus  = TSE_SETUP_STATUS_INITIAL_FORM_OPEN;
    SETUP_DEBUG_TSE("[TSE] Calling the Wrapper function with action AMI_CALLBACK_FORM_OPEN\n");
    UEFICallSetupFormCallBack(AMI_CALLBACK_FORM_OPEN);//Calling the Wraper function with action AMI_CALLBACK_FORM_OPEN
    gDisableFlag = 0; 
    gSetupStatus  = TSE_SETUP_STATUS_INITIAL_BUILD_DEFAULTS;
    SETUP_DEBUG_TSE("[TSE] Calling the function to build the default variables\n");
	Status = VarBuildDefaults();
    if(EFI_ERROR(Status)){
        goto DONE;// Reset to read the variable from NVRAM
    }
    gSetupStatus  = TSE_SETUP_STATUS_LOAD_VARIABLES;
    SETUP_DEBUG_TSE("[TSE] Calling the function to load the variables\n");
    ...
    gDisableFlag = SkipDisableOp; 
    gSetupStatus  = TSE_SETUP_STATUS_INITIAL_RETRIEVE;
    //Call Retrive callback after loading Current values to variable Cache.
    SETUP_DEBUG_TSE("[TSE] Calling the Wrapper function with action AMI_CALLBACK_RETRIEVE\n");
    ...
    SETUP_DEBUG_TSE("[TSE] checking all the controls whether is it having disableif over it and evaluate the expresison if yes\n");
    ...
    SETUP_DEBUG_TSE("[TSE] Calling the Wrapper function with action AMI_CALLBACK_FORM_OPEN for the evaluated Controls\n");
    UEFICallSetupFormCallBack(AMI_CALLBACK_FORM_OPEN);
    SETUP_DEBUG_TSE("[TSE] Calling the Wrapper function with action AMI_CALLBACK_RETRIEVE for the evaluated Controls\n");
...
DONE:
    gSetupStatus  = TSE_SETUP_STATUS_UNDEFINED;
...
EFI_STATUS MainSetupLoop( VOID )
{
	...
	UINT16 ZeroFormID = 0;
	EFI_GUID NullGuid = { 0x0, 0x0, 0x0, {0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 }};
...
//	SetDesiredTextMode();		//StyleInit will set the desired text mode
    ...
    if(thisPage != NULL)
    {   
        SETUP_DEBUG_TSE("[TSE] Start Page: PageFormID = %d, PageID = %d, PageVisible = %d, PageHandle - %x\n", thisPage->PageData.PageFormID, thisPage->PageData.PageID, thisPage->PageData.PageFlags.PageVisible, thisPage->PageData.PageHandle);
        // Check current page is workable.
        if (thisPage->PageData.PageFlags.PageVisible != TRUE)
        {
            CurrentPageUpdated =TRUE;
            SETUP_DEBUG_TSE("[TSE] Root Page found: PageId - %d\n", thisPage->PageData.PageID);
        }
        ...
        for ( ii = 0; ii < gApp->PageCount; ii++ )
        {
            if ( gApp->PageList[ii] != NULL )
            {
                if ((gApp->PageList[ii]->PageData.PageID != 0) && (gApp->PageList[ii]->PageData.PageParentID == 0) && (gApp->PageList[ii]->PageData.PageFlags.PageVisible != TRUE))
                {
                    gApp->CurrentPage = ii;
                    CurrentPageUpdated = TRUE;
                    SETUP_DEBUG_TSE("[TSE] Alternative Root Page Found: PageID = %d\n", gApp->CurrentPage);
                    break;
                }
            }
        }
        ...
        for ( ii = 0; ii < gApp->PageCount; ii++ )
        {
            if ( gApp->PageList[ii] != NULL )
            {
                if (gApp->PageList[ii]->PageData.PageFlags.PageVisible != TRUE)
                {
                    gApp->CurrentPage = ii;
                    CurrentPageUpdated = TRUE;
                    SETUP_DEBUG_TSE("[TSE] Alternative Page Found: PageID = %d\n", gApp->CurrentPage);
                    break;
                }
            }
        }
    ...
    if (!CurrentPageUpdated)
    {
    	...
    	SETUP_DEBUG_TSE("[TSE] Start Page: PageFormID = %d, PageID = %d, PageVisible = %d, PageHandle - %x is not Available\n", thisPage->PageData.PageFormID, thisPage->PageData.PageID, thisPage->PageData.PageFlags.PageVisible, thisPage->PageData.PageHandle);
        goto Exit;
    }
    ...
    // Call SetupPageChangeHookHook for the first page
    PageInfo = (PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[gApp->CurrentPage]);
    PageIdInfo = (PAGE_ID_INFO *)(((UINT8 *) gPageIdList) + gPageIdList->PageIdList[PageInfo->PageIdIndex]);
    SETUP_DEBUG_TSE ("\n[TSE] SetupPageChange: From: PageID %d, FormID %d, %g, To: PageID %d, FormID %d, %g\n", 
            0xFFFF, ZeroFormID, &(NullGuid), gApp->CurrentPage, PageInfo->PageFormID, &(PageIdInfo->PageGuid));
    SetupPageChangeHookHook( &(NullGuid), &(ZeroFormID),&(PageIdInfo->PageGuid), &(PageInfo->PageFormID));
...
EFI_STATUS DrawBbsPopupMenu( VOID )
{
	...
	UINTN  newMaxRows = 0, newMaxCols = 0;
	
	if ( EFI_SUCCESS == SetResAndScreenBufferHook (&newMaxRows, &newMaxCols) )   //To show BBS popup menu with setup resolution 
	{
	   if((newMaxRows != 0) && (newMaxCols != 0) && IsValidRes(newMaxRows,newMaxCols))
	   {
	      gMaxBufX = gMaxRows = newMaxRows;
...
DONE:
    if(!gEnterSetup)            //If we are not entering into setup then updating screen with post screen resolution, after displaying BBS menu with setup resolution. And displaying post messages from the top when we boot to a bootoption from the BBS menu
    {
        gMaxRows = gPSMaxRows;
        gMaxCols = gPSMaxCols;
        SetDesiredTextMode();
	...
	for ( control= 0; control < pageInfo->PageControls.ControlCount; control++ )
	{
		...
		if ( controlInfo->ControlDataWidth > 0 )
		{
			...
			{                
			    ...
			    {
			        int size = UefiGetWidth(controlInfo->ControlPtr);
			        SETUP_DEBUG_VAR("[TSE] _LoadPageDefaults SET var = %d\n", controlInfo->ControlVariable);
			        SETUP_DEBUG_VAR("[TSE] BitField VAR %d, off = %d, width = %d, size = %d\n", 
			                controlInfo->ControlVariable, ControlVarOffset, controlInfo->ControlDataWidth, size); 
			        _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, gFailsafeDefaults, controlInfo->ControlVariable, ControlVarOffset, size, failValue, controlInfo->ControlPtr );
			        _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, gOptimalDefaults, controlInfo->ControlVariable, ControlVarOffset, size, optValue ,controlInfo->ControlPtr);                
			        // Dont' Update the cache of exsiting variable.
			        if(controlInfo->ControlVariable > OldVariableCount)
			            _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, gVariableList, controlInfo->ControlVariable, ControlVarOffset, size, optValue ,controlInfo->ControlPtr);                
			    }
			    else
			    {
...
EFI_STATUS SetParentFormID(UINT16 ParentID, UINT16 TargetID)
{
    ...
    if(NavPageStack == NULL){
		if (gApp->PageList[TargetID]->PageData.PageParentID != 0)  //target page is not a valid start of navigation stack
		{        
			SETUP_DEBUG_TSE("\n[TSE] NavPageStack Skip PageParentID  %d, Parent %d -> TargetID: %d\n", 
			gApp->PageList[TargetID]->PageData.PageParentID, ParentID,  TargetID );
			return Status;
		}
                        ...
                        )
{
    ...
    while ((String = VA_ARG (Args, CHAR16 *)) != NULL) {
   	...
   	DestMax = ((DefaultLength + Size) / sizeof(CHAR16)) + 4;
   	StrCatS(InfoString, DestMax, String);
   	StrCatS(InfoString, DestMax, L"\n");
