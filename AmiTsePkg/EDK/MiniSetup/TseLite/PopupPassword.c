...
BOOLEAN PopupPasswordAuthenticateLocal( POPUP_PASSWORD_DATA *popuppassword, CHAR16 *Password )
{
	...
	StrCpyS(TempEncoded, (size+1), Password);
...
EFI_STATUS SetTitleUsingControlPtr(UINTN Title, CONTROL_INFO *ControlData, UINT16 *newtoken, UINT16* orgtext )
{
	...
	if ( IsShowPromptStringAsTitle() )
	{
		...
		*orgtext = ((AMI_IFR_MSGBOX*)(Title))->Text;
		...
		StrCpyS (tempTitle, (strlen+2), temptext);
		...
		DestMax = EfiStrLen (tempTitle) + 2 +  EfiStrLen (temptext) + 2;
		StrCatS (tempTitle, DestMax , temptext);
        StrCatS (tempTitle, DestMax, L"?");
...
EFI_STATUS HandleESCOnInteractivePassword(POPUP_PASSWORD_DATA *popuppassword,CHAR16 *Text )
{
    ...
    if((Text != NULL) ){
        ...
        if(OldPwLen){
            ...
            StrCpyS(TempEncoded, ((UINT8)UefiGetMaxValue(data)+1), Text);
...
SavePassword:
		if ( !Status )
		{
			...
			if ( ( NULL!=ConfirmNewPswd ) && (NewPswd != NULL)&& (EfiStrCmp(NewPswd, ConfirmNewPswd)==0) ) // Avoid crashing when give empty password
			{
					...
					StrCpyS(popuppassword->Text, (NewPwLen + 1), NewPswd);
					...
					if(UefiIsInteractive(&popuppassword->ControlData))
					{
							...
							if(NewPwLen){
								...
								if ( TempEncoded == NULL )
									return EFI_NOT_READY;
								StrCpyS(TempEncoded, ((UINT8)UefiGetMaxValue(data)+1), NewPswd);
								SavePswdString(&(popuppassword->ControlData),TempEncoded);
								StringZeroFreeMemory( (VOID **)&TempEncoded );
							}
	...
	if(retValue ==0)
	{
	    ...
	    *Text = EfiLibAllocateZeroPool( (gPopupEditCb.Length+1) * sizeof(CHAR16) );
		...
		return EFI_OUT_OF_RESOURCES;
	     ...
	     StrCpyS( *Text, (gPopupEditCb.Length+1), gPopupEditCb.Data );
