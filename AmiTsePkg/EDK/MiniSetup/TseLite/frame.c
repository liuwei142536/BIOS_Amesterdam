...
EFI_STATUS FrameHandleAction( FRAME_DATA *frame, ACTION_DATA *action )
{
	...
	switch ( action->Input.Type )
	{
        ...
        case ACTION_TYPE_KEY:
			...
			if (EFI_ERROR(Status)&& (! GetTseInconsistentPolicy()))
			{
				...
				{//If the control action is known and if its hot key only we are popuping up the ERROR popup
					...
					if((frame->FrameData.FrameType == MAIN_FRAME) && CheckInconsistence((PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[gApp->CurrentPage])) )
					{
...
VOID _FrameSubmenuCallback( FRAME_DATA *frame, SUBMENU_DATA *submenu, VOID *cookie )
{
	...
	if ( ! EFI_ERROR( Status ) )
	{
		...
		else if(gApp->CurrentPage != OldPage ) //focus is updated to first focusable control.
		{
...
VOID _FrameVariableCallback( FRAME_DATA *frame, CONTROL_DATA *control, VOID *cookie )
{
	...
	if ( callbackData->Variable >= gVariables->VariableCount )
		return;
	if (control->ControlData.ControlFlags.BitFieldVarstore)
	{
	    if (( list == NULL ) )
	    {
	        list = gOptimalDefaults;
	    }
	    _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, list,callbackData->Variable, callbackData->Offset, callbackData->Length, callbackData->Data,control->ControlData.ControlPtr );
	}
	else
	{
...
EFI_STATUS _PreviousFrame (FRAME_DATA *frame)
{
	UINT32 thisControl,PrevFirstVisibleCtrl= frame->FirstVisibleCtrl;
	BOOLEAN bFocused;
	EFI_STATUS Status = EFI_UNSUPPORTED;
	...
	if(frame->FirstVisibleCtrl > 0)
	{
		...
		if( frame->FirstVisibleCtrl == PrevFirstVisibleCtrl )
		{
		    frame->FirstVisibleCtrl = frame->FirstVisibleCtrl-1;
		}
...
EFI_STATUS _NextFrame (FRAME_DATA *frame)
{
	UINT32 thisControl,PrevLastVisibleCtrl = frame->LastVisibleCtrl;
	BOOLEAN bFocused;
	...
	StyleFrameSetControlPositions(frame, &(frame->LastVisibleCtrl));
		...
		if(frame->LastVisibleCtrl == PrevLastVisibleCtrl)
		{
		    frame->LastVisibleCtrl = frame->LastVisibleCtrl+1;
		}
	}
		...
		)
   {
	   ...
	   for(i=frame->FirstVisibleCtrl; i<=frame->LastVisibleCtrl; i++)
	   {
			if( (MouseInfo.Top >= frame->ControlList[i]->Top) && (MouseInfo.Top < (UINT32)(frame->ControlList[i]->Top + frame->ControlList[i]->Height))
			       && (MouseInfo.Left >= (UINT32)frame->FrameData.Left) )
	      {
