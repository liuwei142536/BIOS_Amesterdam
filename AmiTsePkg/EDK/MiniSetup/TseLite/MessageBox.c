...
EFI_STATUS MsgBoxDraw( MSGBOX_DATA *msgbox )
{
	...
	if ((Str != NULL) && ( msgbox->MsgBoxType == MSGBOX_TYPE_NULL ) )
	{
		...
		if( (UINT16)(TestPrintLength( Str ) / (NG_SIZE)) > (UINT16)(msgbox->Width-2))
		   {
		    DestMax = (EfiStrLen(Str)+1) - HiiFindStrPrintBoundary(Str,(msgbox->Width-5));
		    StrCpyS( &Str[HiiFindStrPrintBoundary(Str,(msgbox->Width-5))], DestMax, L"...");
		    }
		if ( PadTitle != NULL )
			SPrint( PadTitle, ( msgbox->Width +3)* sizeof(CHAR16), L" %s ",Str );
	}
