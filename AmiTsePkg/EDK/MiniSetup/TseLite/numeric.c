...
BOOLEAN CheckForRangefromsetup(NUMERIC_DATA *numeric, NUMERIC_VALUE_BASE *Num)
{
    if(numeric->Base == 32)
    {
        ...
        {
            //Range -128 to 127
            if(!(((INT64)(*Num) <(INT64) 0x80)&&((INT64)(*Num)>=(INT64)0xFFFFFFFFFFFFFF80)))
                return FALSE;
        ...
        else if(numeric->ControlData.ControlDataWidth == 2)
        {
            ...
            if(!(((INT64)(*Num) <(INT64) 0x8000)&&((INT64)(*Num)>=(INT64)0xffffffffffff8000 )))
                return FALSE;
        ...
        else if(numeric->ControlData.ControlDataWidth == 4)
        {
            ...
            if(!(((INT64)(*Num) <=(INT64)0x7fffffff)&&((INT64)(*Num)>=(INT64)0xffffffff80000000)))
                return FALSE;
...
EFI_STATUS NumericInitialize( NUMERIC_DATA *numeric, VOID *data )
{
	...
	if ( Num != NULL )
	{
        if (numeric->ControlData.ControlFlags.BitFieldVarstore)
        {
            if (( list == NULL ) )
            {
                list = gOptimalDefaults;
            }
            numeric->MinValue= ((EFI_IFR_NUMERIC *) ControlPtr)->data.u32.MinValue;
            numeric->MaxValue= ((EFI_IFR_NUMERIC *) ControlPtr)->data.u32.MaxValue;
            numeric->Step    = ((EFI_IFR_NUMERIC *) ControlPtr)->data.u32.Step;

            _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, list, numeric->ControlData.ControlVariable, 
                UefiGetQuestionOffset(ControlPtr), Size, Num, ControlPtr );
        }
        else
        {
		...
		if(CheckForRange( numeric, Num))
		{
			...
			{              
			    if (numeric->ControlData.ControlFlags.BitFieldVarstore)
			    {
			        SETUP_DEBUG_VAR("[TSE] NumericInitialize: VarGetDefaults BEFORE GET var = %d\n", numeric->ControlData.ControlVariable);

			        _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, gOptimalDefaults, numeric->ControlData.ControlVariable, 
			                UefiGetQuestionOffset(ControlPtr), Size, Num, ControlPtr );
			    }
			    else
			    {
...
EFI_STATUS NumericDraw(NUMERIC_DATA *numeric )
{
	...
	if ( Num != NULL )
	{
        if (numeric->ControlData.ControlFlags.BitFieldVarstore)
        {
            ...
            _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, list, numeric->ControlData.ControlVariable, 
                UefiGetQuestionOffset(ControlPtr),  numeric->ControlData.ControlDataWidth, 
                Num, ControlPtr );
        }
        else
        {
...
//	  		CallbackShowMessageBox( (UINTN)gInvalidRangeFailMsgBox, MSGBOX_TYPE_OK );
			...
			{              
			    ...
			    {
			        _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, gOptimalDefaults, numeric->ControlData.ControlVariable, 
			                UefiGetQuestionOffset(ControlPtr), numeric->ControlData.ControlDataWidth, Num, ControlPtr );
			    }
			    else
			    {
			        VarGetDefaults( numeric->ControlData.ControlVariable, 
...
EFI_STATUS ConvertStringtoUINT64(NUMERIC_DATA *numeric, NUMERIC_VALUE_BASE *Num);

EFI_STATUS NumericHandleActionKeyMouse(NUMERIC_DATA *numeric, ACTION_DATA *Data)
{
	EFI_STATUS Status = EFI_UNSUPPORTED;
	EFI_STATUS ActionStatus = EFI_SUCCESS;
	CHAR16 Chr;
    ...
    BOOLEAN ControlUpdatedFlag = FALSE;
    ...
    Handle=numeric->ControlData.ControlHandle;
    Key = UefiGetControlKey(&(numeric->ControlData));
    IsInteractive = UefiIsInteractive(&numeric->ControlData);
    ...
    switch(Action)
    {
        case ControlActionSelect:      		
			...
			if((numeric->Text[0] ==0x0)||((numeric->Base == AMI_BASE_INT_DEC)&&(L'-' == numeric->Text[0])&&( EfiStrStr(numeric->Text+1, L"-"))))
				...
				Status = ConvertStringtoUINT64( numeric ,&Num);

			// to support signed integers for NUMERIC opcode
           if (( Status == EFI_SUCCESS)&& CheckForRangefromsetup( numeric, &Num) )
			{
...

                {
					if(UefiIsInteractive(&numeric->ControlData))
                    {
            			...
            			if(Control != (CONTROL_DATA*)numeric)
            			{
            				...
            				ActionStatus = EFI_ABORTED;
							ControlUpdatedFlag = TRUE;
            			}
                    }
        ...
        case ControlActionDecreament:
			if ( ( numeric->Step != 0 ) && ( ! numeric->ControlActive ))
			{
				...
				if((bValueChanged)&& CheckForRangefromsetup( numeric, &Num))
				{
					...
					ActionStatus = gNumeric.HandleAction(numeric,&TempData);
				    if((IsInteractive) && (ActionStatus == EFI_ABORTED))
                    {
                        CONTROL_DATA *Control;
                        Control = GetUpdatedControlData((CONTROL_DATA*)numeric,CONTROL_TYPE_NUMERIC,Handle,Key);
                        if(Control == NULL) // Control deleted ?
			...
			else if( (numeric->ControlActive )&&(numeric->Base == AMI_BASE_INT_DEC)&&(ControlActionDecreament == Action) && (numeric->Text[0]== 0))
			{
				DataOk = TRUE;
			}
    ...
    if((IsInteractive)&& (ActionStatus == EFI_ABORTED)&& (ControlUpdatedFlag != TRUE))
    {
        CONTROL_DATA *Control;
        Control = GetUpdatedControlData((CONTROL_DATA*)numeric,CONTROL_TYPE_NUMERIC,Handle,Key);
        if(Control == NULL) // Control deleted ?
...
VOID NumSPrint(NUMERIC_DATA *numeric,NUMERIC_MIN_MAX_BASE Num)
{
    ...
    {
    case AMI_BASE_HEX:
        SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%lx", Num );
        break;
    case AMI_BASE_INT_DEC:
        if(numeric->ControlData.ControlDataWidth == 1)
        {
            if((INT8)Num < 0)
            {
                SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%d", (INT8)Num );
				 break;
            }
        }
        else if(numeric->ControlData.ControlDataWidth == 2)
        {
            if((INT16)Num < 0)
            {
                SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%d", (INT16)Num );  
				break;
            }
        }
        else if(numeric->ControlData.ControlDataWidth == 4)
        {
            if((INT32)Num < 0)
            {
                SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%d", (INT32)Num ); 
				break;
            }
        }
        else if(numeric->ControlData.ControlDataWidth == 8)
        {
            if((INT64)Num < 0)
            {
                SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%ld", (INT64)Num ); 
				break;
            }
        }
        if(numeric->ControlData.ControlDataWidth == 8)
            SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%ld",Num );
        else
            SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%d", Num );

        break;
    case AMI_BASE_DEC:
    default:
        SPrint( numeric->Text, (numeric->TextWidth+1) * sizeof(CHAR16), L"%lu", Num );
        break;
    }
    UpdateNumericDisplayString (&(numeric->Text), Num, numeric->Base, numeric->ControlData.ControlDataWidth, (numeric->TextWidth+1) * sizeof(CHAR16)); //Provision to change the Numeric string format
}
...
{  
    ...
    //EXcept INT_DEC flag + ControlDataWidth = 8  will use atoi_base for covertion
    if (numeric->Base != 32 || numeric->ControlData.ControlDataWidth != 8 )
    {
        *Num = (NUMERIC_MIN_MAX_BASE)atoi_base( numeric->Text, numeric->Base );
        return EFI_SUCCESS;
    ...
    {
        //For negative inputs striping the '-' and setting the flag
        if(numeric->Text[0]== L'-')
        {
            NegativeInput = TRUE;

            Status = StrDecimalToUint64S(&numeric->Text[1],NULL,Num);
        }
        else
            Status = StrDecimalToUint64S(numeric->Text,NULL,Num);

        // If the number represented by String exceeds the range defined by UINT64 will retun error status.

        if(Status != EFI_SUCCESS)
            return Status;

        //For oversized values returning EFI_UNSUPPORTED
         if(!( ((NegativeInput == FALSE) &&((*Num) <= 0x7FFFFFFFFFFFFFFF)) || ((NegativeInput == TRUE) && (*Num) <= 0x8000000000000000)))
            return EFI_UNSUPPORTED;

        //For values within the range returning success status and fixup for negative input
