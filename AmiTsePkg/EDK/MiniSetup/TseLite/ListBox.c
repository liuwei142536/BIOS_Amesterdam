...
extern EFI_STATUS UefiGetValidOptionsCondition(CONTROL_INFO *CtrlInfo, POSTMENU_TEMPLATE* ItemInfo, UINT16 MaxItems);
/**
...
/**
    Function to get last visible option of List Box.
...
**/
UINT16 GetLastVisibleSel(LISTBOX_DATA *listbox)
{
    INTN Height = 0;
    UINT16 LastVisibleSel = 0,OptionHeight=0,Index =0 ;
    CHAR16 *text1 = NULL, *text = NULL; 
    if(TseEnablePopupMultilineSupport())
    {
        Height = listbox->Height - 2;
        while(((INT16)Height>0) && ((listbox->FirstVisibleSel+LastVisibleSel) < (listbox->ItemCount-listbox->HiddenItemCount)))
        {
            if(listbox->Style == POPUP_STYLE_BBS)
                text1 = HiiGetString( listbox->ListHandle, listbox->PtrItems[listbox->FirstVisibleSel + Index++].ItemToken );
            else
                text1 = HiiGetString( listbox->ListHandle, listbox->PtrTokens[listbox->FirstVisibleSel + Index++]);
            
            text = StringWrapText( (CHAR16*)text1, listbox->Width - 4, &OptionHeight );
            MemFreePointer( (VOID **)&text);
            Height -= OptionHeight;
            if((INT16)Height >= 0 && (Index != 1))
            LastVisibleSel++;  
       }
        LastVisibleSel += listbox->FirstVisibleSel;
    }
    else
        LastVisibleSel = listbox->FirstVisibleSel + listbox->Height - 3;
    
    if(LastVisibleSel>=(listbox->ItemCount-listbox->HiddenItemCount))
        LastVisibleSel = listbox->ItemCount-listbox->HiddenItemCount-1;
    
    return LastVisibleSel;
}
...
VOID SetListBoxCommonFields(LISTBOX_DATA *listbox)
{
    ...
    if(TseEnablePopupMultilineSupport())
    {
        ...
        if(listbox->Width > (UINT16)(gMaxCols-5))
            listbox->Width = (UINT16)(gMaxCols-5);

        ListBoxGetControlHeight((LISTBOX_DATA*)listbox, NULL, &TempHeight); 
        listbox->Height = TempHeight ;       
    }
	...
	if(listbox->Height > MAX_VISIBLE_SELECTIONS )
	{
		...
		listbox->LastVisibleSel = GetLastVisibleSel(listbox); // 2 for borders and 1 more because is zero based.
...
EFI_STATUS ListBoxDraw( LISTBOX_DATA *listbox )
{
	...
	UINT16 tmpHeight=0,offset = 0,TempHeight = 0;
	...
	listbox->UseScrollbar = FALSE;
	 ...
	 if(TseEnablePopupMultilineSupport()) //Customized
	 {
	    ListBoxGetControlHeight((LISTBOX_DATA*)listbox, NULL, &TempHeight);
	   if( TempHeight >  (UINT16)MAX_VISIBLE_SELECTIONS)
	       TempHeight = (UINT16)MAX_VISIBLE_SELECTIONS ;
	 }
		else
		TempHeight = listbox->ItemCount-listbox->HiddenItemCount;

	
	// check if we need to use do scrollbar
	if(TempHeight > (UINT16)(MAX_VISIBLE_SELECTIONS - 2))
	{
	    ...
	    if (!ScrollBarMoveAction)
	    {
	        if( listbox->Sel < listbox->FirstVisibleSel)
	        {
	            if((listbox->Sel != 0) && (listbox->Sel < listbox->LastVisibleSel -1))
	            {
	                ...
	                listbox->LastVisibleSel = GetLastVisibleSel(listbox);
	            }
	            else
	            {
	                if( listbox->Sel !=0)
	        ...
	        {
	            if( (( listbox->Sel + (listbox->LastVisibleSel - listbox->FirstVisibleSel)) <= (listbox->ItemCount-listbox->HiddenItemCount-1) ) && (listbox->Sel > listbox->LastVisibleSel +1))
	            {
	                listbox->FirstVisibleSel = listbox->Sel;
	                listbox->LastVisibleSel = GetLastVisibleSel(listbox);
	            }
	            else
	            {
	                ...
	                {
	                    listbox->FirstVisibleSel = listbox->Sel - (listbox->LastVisibleSel - listbox->FirstVisibleSel) /*--*/;
	                    listbox->LastVisibleSel = GetLastVisibleSel(listbox);
	                }
	                else
	                {
					    if(TseEnablePopupMultilineSupport())
					    {   
						    UINT16 i = 0, OptionHeight=0;
                            Index = listbox->Sel;
                            tmpHeight = listbox->Height - 2;//MaxLin
                            while((INT16)tmpHeight>0)
                            {
                                i = Index;
                                if(listbox->Style == POPUP_STYLE_BBS)
                                    text1 = HiiGetString( listbox->ListHandle, listbox->PtrItems[Index--].ItemToken );
                                else
                                    text1 = HiiGetString( listbox->ListHandle, listbox->PtrTokens[Index--]);
                                
                                text = StringWrapText( (CHAR16*)text1, listbox->Width - 4, &OptionHeight );
                                tmpHeight -= OptionHeight;
                                
                                if(((INT16)tmpHeight)>0 && ((INT16)Index >= 0))//MaxLin
                                {
                                    listbox->FirstVisibleSel = i;       
                                }
                                else 
                                    break;
                            }
	...
	for ( Index = listbox->FirstVisibleSel/*0*/; Index <= listbox->LastVisibleSel/*listbox->ItemCount*/; Index++ )
	{
		...
		if ( Index == listbox->Sel )
		{
			...
			if(listbox->PtrItems && (listbox->Style == POPUP_STYLE_BBS))
			{
			...
			if(TseEnablePopupMultilineSupport())
			{
			    text1 = StringWrapText( (CHAR16*)text, listbox->Width - 4, &tmpHeight );  
			    DrawMultiLineStringWithAttribute( listbox->Left + 2,  listbox->Top + offset/* - listbox->FirstVisibleSel*/  + 1, 
			                            (UINTN)(listbox->Width - 4),(UINTN) listbox->Height,
			                            text1, BGColor | FGColor );
			    offset += tmpHeight;
			...
			else
			{
			    ...
			    DrawStringWithAttribute( listbox->Left + 2, listbox->Top + Index - listbox->FirstVisibleSel  + 1, text, BGColor | FGColor);
			}
		}	
		else
		{
			...
			if(listbox->Style == POPUP_STYLE_BBS)
			{
				if (listbox->PtrItems && listbox->PtrItems[Index].ItemToken)
				{
...
EFI_STATUS ListBoxGetControlHeight( LISTBOX_DATA *object,VOID *frame, UINT16 *height )
{
    if(TseEnablePopupMultilineSupport())
    {
        ...
        EFI_STATUS Status = EFI_UNSUPPORTED;
        UINT16 TempItemCount = 0; //save item count at first.
        
        if(TseLiteIsSpecialOptionList((CONTROL_DATA *)object) != TRUE)
        {
            TempItemCount = object->ItemCount; //save item count at first.
            if(object->Style != POPUP_STYLE_BBS)
            {
                Status = UefiGetOneOfOptions(&object->ControlData,&object->ListHandle,
                        &OptionList, NULL, &object->ItemCount,NULL,NULL);
                if (EFI_ERROR(Status))
                    return Status;
                if (TempItemCount != object->ItemCount)
                {
                    if( object->PtrTokens != NULL )
                      MemFreePointer( (VOID **)&(object->PtrTokens));
                    object->PtrTokens = EfiLibAllocatePool( object->ItemCount * sizeof(UINT16) );
                    if ( object->PtrTokens == NULL )
                    {
                        Status = EFI_OUT_OF_RESOURCES;
                        return Status;
                    }
                    for( Index = 0; Index < object->ItemCount; Index++ )
                    {
                        object->PtrTokens[Index] = OptionList[Index];
                    }
                }
                MemFreePointer( (VOID **)&(OptionList));
            }
        }
        Width = (UINT16)(object->Width-4) ;
        *height = 0 ;
        
        for(Index = 0; Index < object->ItemCount; Index++)
        { 
            if(object->PtrItems && (object->PtrItems[Index].Attribute == AMI_POSTMENU_ATTRIB_HIDDEN))
                continue;
            
            if(object->Style == POPUP_STYLE_BBS)
                text1 = HiiGetString( object->ListHandle, object->PtrItems[Index].ItemToken );
            else
                text1 = HiiGetString( object->ListHandle, object->PtrTokens[Index]);
            ...
            if (text1 != NULL)
            {
                ...
                MemCpy(text, text1, Len);                
                MemFreePointer( (VOID **)&text1 );

                text1 = StringWrapText( (CHAR16*)text, Width, &tmpHeight );
                MemFreePointer( (VOID **)&text1 );  
                *height += tmpHeight;
                MemFreePointer( (VOID **)&text );
            }
    ...
    {
        *height = object->ItemCount - object->HiddenItemCount + 2;
    }
    return EFI_SUCCESS;
}
...
{	
	...
	switch ( bScrollUp )
	{
		...
		case FALSE:
		    ...
		    if( Size >= (listbox->ItemCount - listbox->HiddenItemCount - listbox->LastVisibleSel) )
		         Size = listbox->ItemCount - listbox->HiddenItemCount - listbox->LastVisibleSel - 1;
