...
EFI_STATUS MemoDraw( MEMO_DATA *memo )
{
	...
	if(!IsTSEMultilineControlSupported() && memo->ParentFrameType == MAIN_FRAME){
	    ...
	    {
			StrCpyS(newText, (EfiStrLen(text)+1), text);
	       DestMax = (EfiStrLen(text)+1)  - HiiFindStrPrintBoundary(newText,(UINTN)(memo->Width-SLength));
	       StrCpyS( &newText[HiiFindStrPrintBoundary(newText,(UINTN)(memo->Width-SLength))],  DestMax, L"...");
