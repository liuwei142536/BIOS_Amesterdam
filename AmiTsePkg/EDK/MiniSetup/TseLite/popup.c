...
VOID PopupDrawString(POPUP_DATA *popup, CHAR16 *Str,UINT16 Top)
{
	...
	if ( ( (OpCode == EFI_IFR_PASSWORD_OP ) || (EFI_IFR_STRING_OP == OpCode) ) && IsTSEMultilineControlSupported() )
	{
		if (Str)
		{
			...
			if ( PadTitle != NULL )
				SPrint( PadTitle, ((Strlen + 3)* sizeof (CHAR16)), L"%s",Str );
	...
	else
	{
		if ( Str != NULL )
		{
			...
			if ( PadTitle != NULL )
				SPrint( PadTitle, (EfiStrLen(Str) + 3) * sizeof(CHAR16), L" %s ",Str );
		}
...
#define HELP_LINES	3
		...
		for(i=0;i<NumOfHelpLines;i++)
		{
			...
			*String = L'\0';
			...
			{
				 DestMax = (EfiStrLen(text) + 1) - HiiFindStrPrintBoundary(text,(UINTN)(popup->Width-5));
			    StrCpyS( &text[HiiFindStrPrintBoundary(text,(UINTN)(popup->Width-5))], DestMax, L"...");
			}
