     ...
     )
{
    ...
    {
    case HW_PCI_DP:
        return SPrint(Str,Length,HWDP[0],((PCI_DEVICE_PATH*)Path)->Device,((PCI_DEVICE_PATH*)Path)->Function);
    case HW_PCCARD_DP:
        return SPrint(Str,Length,HWDP[1],((AMITSE_PCCARD_DEVICE_PATH*)Path)->FunctionNumber);
    case HW_VENDOR_DP:
       return SPrint(Str,Length,HWDP[2],&((VENDOR_DEVICE_PATH*)Path)->Guid);
    }
    return SPrint(Str,Length,DefaultDevicePath,Path->Type,Path->SubType);
}
    ...
    )
{
    ...
    {
    case MSG_ATAPI_DP:
       return SPrint(Str,Length,MSGDP[0],
               ((ATAPI_DEVICE_PATH*)Path)->PrimarySecondary ? L"Sec" : L"Pri",
               ((ATAPI_DEVICE_PATH*)Path)->SlaveMaster ? L"Sl" :L"Ma");
    case MSG_SCSI_DP:
        return SPrint(Str,Length,MSGDP[1],((SCSI_DEVICE_PATH*)Path)->Pun,((SCSI_DEVICE_PATH*)Path)->Lun);
    case MSG_VENDOR_DP:
       return SPrint(Str,Length,MSGDP[2],((VENDOR_DEVICE_PATH*)Path)->Guid);
    case MSG_USB_DP:
       return SPrint(Str,Length,L"USB(%x,%x)\\",
            ((USB_DEVICE_PATH*)Path)->ParentPortNumber,((USB_DEVICE_PATH*)Path)->InterfaceNumber);
    case MSG_I2O_DP:
       return SPrint(Str,Length,L"I2O(%x)\\",((I2O_DEVICE_PATH*)Path)->Tid);
    case MSG_USB_CLASS_DP:
        return SPrint(Str,Length,L"USB_CLASS(%X,%X,%x,%X,%X)\\",((USB_CLASS_DEVICE_PATH*)Path)->VendorId,((USB_CLASS_DEVICE_PATH*)Path)->ProductId,((USB_CLASS_DEVICE_PATH*)Path)->DeviceClass,((USB_CLASS_DEVICE_PATH*)Path)->DeviceSubClass,((USB_CLASS_DEVICE_PATH*)Path)->DeviceProtocol);
    case MSG_USB_WWID_DP:
       return SPrint(Str,Length,L"USB_WWID(%X,%X,%x)\\",((USB_WWID_DEVICE_PATH*)Path)->VendorId,((USB_WWID_DEVICE_PATH*)Path)->ProductId,((USB_WWID_DEVICE_PATH*)Path)->InterfaceNumber);
    case MSG_DEVICE_LOGICAL_UNIT_DP:
        return SPrint(Str,Length,L"LOGICAL_UNIT(%x)\\",((DEVICE_LOGICAL_UNIT_DEVICE_PATH*)Path)->Lun);
    case MSG_SATA_DP:
        return SPrint(Str,Length,L"SATA(%x,%X,%x)\\",((SATA_DEVICE_PATH*)Path)->HBAPortNumber,((SATA_DEVICE_PATH*)Path)->PortMultiplierPortNumber,((SATA_DEVICE_PATH*)Path)->Lun);
    case MSG_MAC_ADDR_DP:
		SPrint(Str,Length,L"MAC(");
        Offset = EfiStrLen(L"MAC(");
        for(i=0;i<MAC_ADDR_SIZE;i++)
        {
            SPrint(Str + Offset,Length,L"%02X",((MAC_ADDR_DEVICE_PATH*)Path)->MacAddress.Addr[i]);
            Offset += 2;
        }
        return SPrint(Str + Offset,Length,L",%x)\\",((MAC_ADDR_DEVICE_PATH*)Path)->IfType);
    case MSG_IPv4_DP:
        return SPrint(Str,Length,L"IPv4(%d.%d.%d.%d)\\",((IPv4_DEVICE_PATH*)Path)->LocalIpAddress.Addr[0],((IPv4_DEVICE_PATH*)Path)->LocalIpAddress.Addr[1],((IPv4_DEVICE_PATH*)Path)->LocalIpAddress.Addr[2],((IPv4_DEVICE_PATH*)Path)->LocalIpAddress.Addr[3]);
    case MSG_IPv6_DP:
            return SPrint(Str,Length,L"Ipv6(%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x)\\",
    ...
    case MSG_ISCSI_DP:
        return SPrint(Str,Length,L"iSCSI(%x,%lx,%x,%s)\\",((ISCSI_DEVICE_PATH*)Path)->TargetPortalGroupTag,((ISCSI_DEVICE_PATH*)Path)->Lun,((ISCSI_DEVICE_PATH*)Path)->LoginOption,(((ISCSI_DEVICE_PATH*)Path)->NetworkProtocol == 0 ? L"TCP" : L"reserved"));
    case MSG_SASEX_DP:
        return SPrint(Str,Length,L"SASEX(%x,%x,%x,%x)\\",((SASEX_DEVICE_PATH*)Path)->SasAddress,((SASEX_DEVICE_PATH*)Path)->Lun,((SASEX_DEVICE_PATH*)Path)->RelativeTargetPort,((SASEX_DEVICE_PATH*)Path)->DeviceTopology);
    case MSG_NVME_NAMESPACE_DP:
        Uuid = (UINT8 *)&((NVME_NAMESPACE_DEVICE_PATH*)Path)->NamespaceUuid ;
        return SPrint(Str,Length,L"NVMe(0x%x,%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x)\\",((NVME_NAMESPACE_DEVICE_PATH*)Path)->NamespaceId,Uuid[7], Uuid[6], Uuid[5], Uuid[4],Uuid[3], Uuid[2], Uuid[1], Uuid[0]);
    case MSG_UFS_DP:
        return SPrint(Str,Length,L"UFS(%x,%x)\\",((UFS_DEVICE_PATH*)Path)->Pun,((UFS_DEVICE_PATH*)Path)->Lun);
    case MSG_SD_DP:
        return SPrint(Str,Length,L"SD(0x%x)\\",((SD_DEVICE_PATH*)Path)->SlotNumber);
    case MSG_DNS_DP:
        ...
        if(((DNS_DEVICE_PATH*)Path)->IsIPv6)
        {
            ...
            for(i=0;i<n;i++)
            {
                SPrint(Str + Offset,Length,L"%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x:%02x%02x,",
            ...
            return SPrint(Str + Offset - 1,Length,L")\\");
        }
        else
        {
            for(i=0;i<n;i++)
            {
                SPrint(Str + Offset,Length,L"%d.%d.%d.%d,",((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v4.Addr[0],((DNS_DEVICE_PATH*)Path)->DnsServerIp[i].v4.Addr[1],
    ...
    )
{
	...
	case MEDIA_HARDDRIVE_DP:
        {
           ...
           return SPrint(Str,Length,L"HD(Part%d,Sig%08X)", Hd->PartitionNumber, *((UINT32 *)(&(Hd->Signature[0]))));
            case SIGNATURE_TYPE_GUID:
            return SPrint(Str,Length, L"HD(Part%d,Sig%g)",Hd->PartitionNumber,(EFI_GUID *) &(Hd->Signature[0]));
            default:
            return SPrint(Str,Length,L"HD(Part%d,MBRType=%02x,SigType=%02x)", 
                Hd->PartitionNumber,
                Hd->MBRType,
                Hd->SignatureType
                );
	...
	case MEDIA_CDROM_DP:
        return SPrint(Str,Length,MEDIADP[0], ((CDROM_DEVICE_PATH*)Path)->BootEntry);
	case MEDIA_VENDOR_DP:
        return SPrint(Str,Length,MEDIADP[1], ((VENDOR_DEVICE_PATH*)Path)->Guid);
	case MEDIA_FILEPATH_DP:
        return SPrint(Str,Length,MEDIADP[2], &((AMITSE_MEDIA_FW_VOL_FILEPATH_DEVICE_PATH*)Path)->FvFileName);
	case MEDIA_RAM_DISK_DP:
		if (EfiCompareGuid (&((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->TypeGuid, &EfiVirtualDiskGuid))
            return SPrint(Str,Length,L"RAM_DISK(VirtualDisk,%d)\\",((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->Instance);
		else if (EfiCompareGuid (&((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->TypeGuid, &EfiVirtualCdGuid))
            return SPrint(Str,Length,L"RAM_DISK(VirtualCD,%d)\\",((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->Instance);
		else if (EfiCompareGuid (&((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->TypeGuid, &EfiPersistentVirtualDiskGuid))	
            return SPrint(Str,Length,L"RAM_DISK(PersistentVirtualDisk,%d)\\",((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->Instance);
		else if (EfiCompareGuid (&((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->TypeGuid, &EfiPersistentVirtualCdGuid))
            return SPrint(Str,Length,L"RAM_DISK(PersistentVirtualCD,%d)\\",((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->Instance);
		else
            return SPrint(Str,Length,L"RAM_DISK(%g,%d)\\",((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->TypeGuid,((MEDIA_RAM_DISK_DEVICE_PATH*)Path)->Instance);            
...
CHAR16* GetFileNameFromDevicePath(EFI_DEVICE_PATH_PROTOCOL *Path)
{
    ...
    UINTN BufferSize = 512 * sizeof(CHAR16);
    ...
    for(devPath = Path;!IsDevicePathEnd(devPath);devPath = NextDevicePathNode(devPath))
    {
        ...
        {
            _MEDIAToStr(devPath, p , BufferSize);
            break;
        }
    }
...
UINT16 *_DevicePathToStr(EFI_DEVICE_PATH_PROTOCOL *Path)
{
	...
	for(;;)
	{
		...
		case HARDWARE_DEVICE_PATH:
		    Buffersize = (Buffersize-(Count* sizeof(CHAR16)));
		    if(Buffersize < 110)
		        break;
		    Count = _HWToStr(Path, p,Buffersize);
		    break;
...
to display
		...
		case MESSAGING_DEVICE_PATH:
		    ...
		    Count = _MSGToStr(Path, p,Buffersize);
		    break;
		case MEDIA_DEVICE_PATH:
		    ...
		    Count = _MEDIAToStr(Path, p,Buffersize);
		    break;
		...
		default:
		    ...
		    Count = _UnknownToStr(Path, p,Buffersize);
		}
...
VOID DiscardAddDelBootOptions(VOID)
{
    ...
    for(i=0;i<gDelOptionCount;i++)
    {
        if(!gDelBootData[i].bNewOption)
        {
			...
			if(DelOptionList)// deleted option that need to added.
...
// If newly add/Del option discorded.
	if(AddOptionListCount || DelOptionListCount)
	{
		...
		VarSetValue(VARIABLE_ID_BOOT_MANAGER, 0, sizeof(UINT16), (VOID *)(&gBootOptionCount));
		gVariableList[VARIABLE_ID_BOOT_ORDER].Buffer = (UINT8 *)NewBootOrder;
        gVariableList[VARIABLE_ID_BOOT_ORDER].Size = gBootOptionCount * sizeof(UINT16);
	}
...
VOID DiscardAddDelDriverOptions(VOID)
{
    ...
    for(i=0;i<gDriverOptionCount;i++)
    {
		...
		{ 	if(AddOptionList)// Newly added option that need to removed.
...
// If newly add/Del option discorded.
	if(AddOptionListCount || DelOptionListCount)
	{
		...
		VarSetValue( VARIABLE_ID_DRIVER_MANAGER, 0, sizeof(UINT16), (VOID *)(&gDriverOptionCount) );
		gVariableList[VARIABLE_ID_DRIVER_ORDER].Buffer = (UINT8 *)NewDriverOrder;
        gVariableList[VARIABLE_ID_DRIVER_ORDER].Size = gDriverOptionCount * sizeof(UINT16);
	}
    MemFreePointer((VOID **) &DelOptionList);// Memory Leak fix - to avoid leak if deleted only newly added optoin, not old options.
...
VOID SaveAddDelBootOptions(VOID)
{
	...
	for(i=0;i<gBootOptionCount;i++)
	{
		if(gBootData[i].bNewOption)
		{
			...
			StrCpyS( pOption->Name, EfiStrLen(gBootData[i].Name) + 1 , gBootData[i].Name );
...
VOID SaveAddDelDriverOptions(VOID)
{
	...
	for(i=0;i<gDriverOptionCount;i++)
	{
		if(gDriverData[i].bNewOption)
		{
			...
			pOption->PathLength = dpLength;
			StrCpyS( pOption->Name, EfiStrLen(gDriverData[i].Name) + 1 , gDriverData[i].Name );
...
static VOID AddFileTypeEntry(FILE_TYPE **List, UINTN *FileCount, EFI_FILE_INFO *FileInfo)
{
    ...
    Length = (EfiStrLen(FileInfo->FileName)+5)*sizeof(CHAR16);
       ...
       // NewList[Index].Name = EfiLibAllocateCopyPool(Length, NewList[Index].Name);
        ...
        if((NewList[Index].Name) != NULL )
        {
            ...
            if(NewList[Index].Type == EFI_FILE_DIRECTORY)
                SPrint(NewList[Index].Name, Length, L"<%s>", FileInfo->FileName);
            else
                SPrint(NewList[Index].Name, Length, L"%s", FileInfo->FileName);
	...
	if(EfiStrLen(FilePath))
	{
		if( idx==0 ) {
			if(EfiStrCmp(CurFile,L".."))  {
			    DestMax =  EfiStrLen(FilePath)+ EfiStrLen(CurFile)+ 3;
			    StrCatS(FilePath, DestMax, L"\\");
		
			    StrCatS(FilePath, DestMax, CurFile);
	    ...
	    DestMax =  EfiStrLen(CurFile)+ 3;
	    StrCpyS(FilePath, DestMax, L"\\");
