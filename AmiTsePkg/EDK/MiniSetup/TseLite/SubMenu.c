...
extern EFI_STATUS UpdateIFRRefXControls(PAGE_INFO *PageInfo, CONTROL_INFO * CtrlInfo);
...
EFI_STATUS SubMenuDraw( SUBMENU_DATA *submenu )
{
	...
	if ( submenu->SubMenuType != 1 )
	{
		...
		else
		{
			...
			{
			DestMax = (length / sizeof(CHAR16)) - HiiFindStrPrintBoundary(text,(UINTN)(submenu->Width-2));
			StrCpyS( &text[HiiFindStrPrintBoundary(text,(UINTN)(submenu->Width-2))], DestMax, L"...");
			}
			DrawStringWithAttribute( submenu->Left - 2, submenu->Top, (CHAR16*)text, 
					(UINT8)(  (submenu->ControlFocus) ? 
	...
	else
	{
		...
		{
		   DestMax = (length / sizeof(CHAR16)) - HiiFindStrPrintBoundary(text,(UINTN)(submenu->Width-5));
		   StrCpyS( &text[HiiFindStrPrintBoundary(text,(UINTN)(submenu->Width-5))], DestMax, L"...");		
		}
...
EFI_STATUS SubMenuHandleAction( SUBMENU_DATA *submenu, ACTION_DATA *Data)
{
	...
	if(Selected)
	{
        ...
        if (( submenu->Callback )&&( submenu->Cookie ))
        {
			PAGE_INFO *PageInfo = (PAGE_INFO *)((UINTN)gApplicationData + gPages->PageList[submenu->ControlData.ControlPageID]);
			UpdateIFRRefXControls(PageInfo, &(submenu->ControlData));
		}
		...
		if(UefiIsInteractive(&submenu->ControlData))
		{
			...
			Status = CallFormCallBack(&(submenu->ControlData),Key,0,AMI_CALLBACK_CONTROL_UPDATE);
			...
			// Don't change page when Callback fails, change is allowed ifStatus = EFI_UNSUPPORTED
		...
		if (FormBrowserHandleValid() && submenu->ControlData.ControlDestPageID != 0xFFFF)//Checking for valid form
		{
...
EFI_STATUS SubMenuGetControlHeight(SUBMENU_DATA *submenu, VOID *frame, UINT16 *height)
{
	...
	if(IsTSEMultilineControlSupported())
	{
		...
		text = StringWrapText( newText, Width+2, height );
