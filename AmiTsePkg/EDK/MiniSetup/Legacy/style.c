...
**/
extern BOOLEAN isSubPageRootPage(PAGE_INFO *info);
	...
	switch ( FrameType )
	{
		case MAIN_FRAME:
			...
			if( gApp->CurrentPage == 0)
			{
			    ...
			    {
			        controlInfo = (CONTROL_INFO *) ((UINT8 *)gControlInfo + data->PageControls.ControlList[i]);

			        Pageinfo = (PAGE_INFO *)((UINTN)gApplicationData + gPages->PageList[controlInfo ->ControlDestPageID] ) ;

			        if((controlInfo) && isSubPageRootPage(Pageinfo)&& (controlInfo->ControlFlags.ControlVisible))
			        {
			            data->PageControls.ControlList[j] = data->PageControls.ControlList[i];
			            j++;
			        }
			    }
			    
                for ( i = j; i < data->PageControls.ControlCount; i++ )
                    data->PageControls.ControlList[j] = 0;

                data->PageControls.ControlCount = j;
