...
// Variables
//---------------------------------------------------------------------------
BOOLEAN gPackUpdateHappened = FALSE;
BOOLEAN gIFRChangeNotify = FALSE;
BOOLEAN gBlockduplicatePack = FALSE;
	...
	)
{
	...
	SETUP_DEBUG_UEFI_NOTIFICATION ("\n[TSE] IFRChangeNotifyFn: Handle %x NotifyType %x\n",  Handle,  NotifyType);
	
	if (FALSE == gEnableDrvNotification)		
	{
		SETUP_DEBUG_UEFI_NOTIFICATION ("[TSE] Illegal Notification: NotifyType: 0x%x\n", NotifyType);
	}
	...
	if ( (gSetupStatus == TSE_SETUP_STATUS_BLOCK_PACKUPDATEPROCESS) 
	   ||(gSetupStatus == TSE_SETUP_STATUS_LOAD_VARIABLES ))
	{
        goto DONE;
	}    
	if(gEnableDrvNotification && !gEnableProcessPack)  // if gEnableProcessPack is TRUE, then some notification is already being processed
	{
		SETUP_DEBUG_UEFI_NOTIFICATION ("[TSE] IFRChangeNotifyFn: ProcessPackNotificationHook, gSetupStatus = %d \n", gSetupStatus);
...
DONE:
  SETUP_DEBUG_UEFI_NOTIFICATION ("[TSE] IFRChangeNotifyFn: Exit status: %r\n\n", status);
  return status;
}
...
//	EFI_HII_DATABASE_NOTIFY_TYPE NotifyType = 0;
	...
	SETUP_DEBUG_UEFI_NOTIFICATION ("[TSE] Before RemoveDuplicateIFRNotification(), NumOfNotification left: %d\n", gNotificationQueue->NumOfNotification);
...
//	NotifyType = tempNotifInfo->NotifyType; Not used
	...
	SETUP_DEBUG_UEFI_NOTIFICATION ("[TSE] After RemoveDuplicateIFRNotification(), NumOfNotification left: %d\n", gNotificationQueue->NumOfNotification);
DONE:
	return status;
}
...
EFI_STATUS ProcessPackNotification (VOID)
{
	...
	do
	{
		...
		{
			SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] Pack notification received inside SendForm \n");
			...
			if(SfCurrNotifInfo)
			{
				...
				SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE]Found notification for current sendform handle 0x%x with notification type 0x%x\n",Handle,NotifyType);
			}
			else
			{
				SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] Skipping ProcessPackNotification since no notification pending related to Current Context...\n");	
		...
		SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] gNotificationQueue->NumOfNotification left: %d \n", gNotificationQueue->NumOfNotification);	
	
		switch(NotifyType)
		{
			case EFI_HII_DATABASE_NOTIFY_REMOVE_PACK:
				SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] NotifyType: EFI_HII_DATABASE_NOTIFY_REMOVE_PACK \n[TSE] Removing IFR Pack \n");
				
				if(gBlockduplicatePack &&( gRemovedHandle == Handle ))
				...
				gBlockduplicatePack = TRUE;

                break;

			case EFI_HII_DATABASE_NOTIFY_NEW_PACK:
				SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] NotifyType: EFI_HII_DATABASE_NOTIFY_NEW_PACK \n[TSE] New IFR Pack \n");
				if(gIFRChangeNotify)
				{
					if(gRemovedHandle!=NULL)
					{
			...
			case EFI_HII_DATABASE_NOTIFY_ADD_PACK:
				SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] NotifyType: EFI_HII_DATABASE_NOTIFY_ADD_PACK \n[TSE] Add IFR Pack \n");
				status = HandleAddIFRPack(Handle, PackData);
				gIFRChangeNotify = FALSE;
				FixSetupData();
				gBlockduplicatePack = FALSE;
				break;
			default:
				break;
		}
...
//  cannot use File operations with TPL NOTIFY		
...
//#endif
		...
		SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] NumOfNotification left: %d, headNotifInfo: 0x%x \n", gNotificationQueue->NumOfNotification, gNotificationQueue->headNotifInfo);
	}while(1);

DONE:
	gPackUpdatePending = FALSE;
    gPackUpdateHappened = TRUE;
	return status;
}
/**
