...
UINT32 _AddFormSetVariable(EFI_GUID * VarGuid)
{
    ...
    StrCpyS(Variable.VariableName, VARIABLE_NAME_LENGTH, L"Setup");

	i = VariableListPtr->VariableCount;
	SETUP_DEBUG_UEFI_PARSE("[TSE] Adding Formset variable Name: %s variable Guid: %g\n", Variable.VariableName, VarGuid);
...
//----------------------------------------------------------------------------
// Procedure:    _CleanVarKeyEfiTable
//
// Description: Function to free the memory created to VarEfiKeyTable 
//
//----------------------------------------------------------------------------

VOID _CleanVarKeyEfiTable()
{
    PVAR_KEY_TABLE pNext = VarEfiKeyTable.Next;
    while(pNext)
    {
        SETUP_DEBUG_UEFI ("\n[TSE]VarKeyEfi pNext = 0x%x,VarID: %d; Index: %d Handle: 0x%x, Next: 0x%x\n",
                pNext, pNext->VarId,  pNext->Index, pNext->Handle, pNext->Next);
        VarEfiKeyTable.Next = pNext->Next;
        MemFreePointer((VOID**)&pNext);
        pNext = VarEfiKeyTable.Next;
    }
}

//----------------------------------------------------------------------------
// Procedure:    __CreateVarKeyEfiTable
//
// Description: Function to create a list of EfiVariable
//
//----------------------------------------------------------------------------

EFI_STATUS _CreateVarKeyEfiTable(UINT16 varId,UINT16 i, EFI_HII_HANDLE Handle,CHAR16   *Varname)
{
    PVAR_KEY_TABLE pVarloopTable,pefiVarTable = NULL;
    EFI_STATUS status = EFI_SUCCESS;
    BOOLEAN Matched = FALSE;
    pefiVarTable = &VarEfiKeyTable;
    for(pVarloopTable = &VarEfiKeyTable ; pVarloopTable->Next; pVarloopTable= pVarloopTable->Next)
    {
         if(((pVarloopTable->VarId)==varId) &&(pVarloopTable->Handle == Handle))
            {
                Matched = TRUE;
                SETUP_DEBUG_UEFI ("\n[TSE] EFI Variable: %s  Handle: 0x%x is already added to EfiKeyTable at index: %d and varid :%d and address pVarloopTable:0x%x ", Varname,Handle,pVarloopTable->Index,varId,pVarloopTable);        
                break;
            }
    }
    if(Matched == FALSE)
    {
        if (pefiVarTable->Handle != 0) // not the first node, advance
        {    
            while(pefiVarTable->Next)
            {
                pefiVarTable = pefiVarTable->Next;
           }    
            pefiVarTable->Next = EfiLibAllocateZeroPool(sizeof(VAR_KEY_TABLE));
            if(pefiVarTable->Next == NULL)
            {
        ...
        pefiVarTable->VarId = varId;
        pefiVarTable->Index = i;
        pefiVarTable->Handle = Handle;
        SETUP_DEBUG_UEFI ("\n[TSE] EFI Variable: %s Handle:0x%x is added to EfiKeyTable at index %d and it's varid :%d and address:0x%x  ", Varname,Handle, i,varId,pefiVarTable);        
    }
    
    pefiVarTable = &VarEfiKeyTable;
    SETUP_DEBUG_UEFI ("\n[TSE]VarKeyEfi TABLE: pefiVarTable 0x%x;  VarID: %d; Index: %d Handle: 0x%x, Next: 0x%x\n",
                pefiVarTable, pefiVarTable->VarId,  pefiVarTable->Index, pefiVarTable->Handle, pefiVarTable->Next);
    while(pefiVarTable->Next)
    {
        pefiVarTable = pefiVarTable->Next;
        SETUP_DEBUG_UEFI ("\n[TSE]VarKeyEfi TABLE: pefiVarTable 0x%x;  VarID: %d; Index: %d Handle: 0x%x, Next: 0x%x\n",
...
EFI_STATUS _AddVariable(UINT8 *IFRData, EFI_HII_HANDLE Handle)
{
	...
	case EFI_IFR_VARSTORE_NAME_VALUE_OP:
		...
		StrCpyS (varName, VARIABLE_NAME_LENGTH, L"");
		attributes |= 0;
	...
	for(i=0; i < VariableListPtr->VariableCount; i++)
	{
		...
		if(guid && EfiCompareGuid(guid, &(variable->VariableGuid)) )//Compare Guid
		{
			switch(opHeader->OpCode)
			{
			case EFI_IFR_VARSTORE_OP:
			...
			case EFI_IFR_VARSTORE_EFI_OP:
				...
				break;
			case EFI_IFR_VARSTORE_NAME_VALUE_OP:
			    if((variable->ExtendedAttibutes & VARIABLE_ATTRIBUTE_NAMEVALUE) == VARIABLE_ATTRIBUTE_NAMEVALUE &&
...
#if EFIVARSTORE_LOOKUP_TABLE_SUPPORT
	if ((EFI_IFR_VARSTORE_EFI_OP == opHeader->OpCode) && (bGreaterUEFIVersion)) //Name is available in the opcode only if UEFI spec followed is
	{                                                                                //greater than 2.3
	    status = _CreateVarKeyEfiTable(varId,i,Handle,varName);
	    if(status == EFI_OUT_OF_RESOURCES)
	    {
	        SETUP_DEBUG_UEFI( "\n[TSE] _AddVariable() returned with status from _CreateVarKeyEfiTable = %r \n" , status );
	        goto DONE;
	    }
	}   
#endif	   
	...
	if(!varfound)
	{
	    ...
	    {
	        StrCpyS(newVariable.VariableName, VARIABLE_NAME_LENGTH , varName);
		...
		SETUP_DEBUG_UEFI( "\n[TSE] Variable: %s %g %x is added to Variable List at index:%d\n", newVariable.VariableName, guid, Handle, VariableListPtr->VariableCount - 1);
	}else
	{
		...
		{
		    StrCpyS(variable->VariableName, VARIABLE_NAME_LENGTH , varName);
	    ...
	    SETUP_DEBUG_UEFI( "\n[TSE] Variable: %s %g %x already present in the list. So updated its details at old index:%d\n", variable->VariableName, guid, Handle, i); 
	}
	...
	{
		status = EFI_OUT_OF_RESOURCES;
	    SETUP_DEBUG_UEFI( "\n[TSE] _AddVariable() returned with status = %r \n" , status );
		goto DONE;
	}
...
//    UINT16 RecommandedDefaultSize=0; // unused - GCC warning
    ...
    do
	{
		...
		case EFI_IFR_DATE_OP:
     		...
     		if((DateOp->Flags & EFI_QF_DATE_STORAGE) == QF_DATE_STORAGE_NORMAL)
      		{
		...
		case EFI_IFR_TIME_OP:
      		...
      		if((TimeOp->Flags & QF_TIME_STORAGE) == QF_TIME_STORAGE_NORMAL)
      		{
       ...
       case EFI_IFR_OR_OP:
        ...
        case EFI_IFR_SHIFT_RIGHT_OP:
          ...
          switch (ifrData->OpCode) {
            ...
            case EFI_IFR_MODULO_OP:
              Value->Value.u64 = 0;
              AmiTseDivU64x32(Data1.Value.u64, (UINT32)Data2.Value.u64, &TempValue);
...
EFI_STATUS VarGetandSetQuestionValue(UINT32 variable, UINTN Offset, UINTN Size ,UINT64 qValue, VARIABLE_INFO *varInfo, BOOLEAN PushThisToNvram, CONTROL_INFO *ControlInfo);
...
VOID EvaluateReadWrite(UINT8 *buf, CONTROL_INFO *ControlInfo, EFI_HII_VALUE *Value1)
{
  ...
  do
  {
    ...
    case EFI_IFR_THIS_OP:
        ...
        CtrlOffset = UefiGetQuestionOffset(ControlInfo->ControlPtr);
        ThisValue = questionValue->Value.u64;
        Value = questionValue;
        break;
    case EFI_IFR_TO_BOOLEAN_OP:				//boolval opcode need to be handled for Read\Write
		...
		else if(Value->Type == EFI_IFR_TYPE_UNDEFINED)
          goto DONE;
        break;
    ...
    case EFI_IFR_GET_OP:
    {
        ...
        UINT16 VarStoreId = 0;
        UINT64 *qValue=0;
        EFI_IFR_TYPE_VALUE    questionValue1 = {0};
        UINT64 def;
        ...
        if (varInfo != NULL) {      //Handling Varstore\EfiVarstore in same case.
            if(varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_VARSTORE || varInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_EFI_VARSTORE)
            {
                ...
                qValue =(UINT64 *) VarGetNvramQuestionValue(VarStoreId, IfrGet->VarStoreInfo.VarOffset, (( Size > sizeof(UINT64))? sizeof(UINT64) : (UINT32)Size ));
                if(qValue == NULL)
                {
                    goto DONE;
                }
                if(Size == sizeof(UINT8))
                    Value->Value.u64 = (UINT8)*qValue;
                else if(Size == sizeof(UINT16))
                    Value->Value.u64 = (UINT16)*qValue;
                else if(Size == sizeof(UINT32))
                    Value->Value.u64 = (UINT32)*qValue;
                else
                    Value->Value.u64 = (UINT64)*qValue;
            }
            else if (varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_NAMEVALUE)        //Added code to handle NameValue varstore. Not able to test as of now, as vfrcompiler gives error.
            {
                if (IfrGet->VarStoreType != EFI_IFR_TYPE_STRING) {
                    ...
                    qValue =(UINT64 *) VarGetNvramQuestionValue(VarStoreId, 0, (( Size > sizeof(UINT64))? sizeof(UINT64) : (UINT32)Size ));
                    if(qValue == NULL)
                    {
                        goto DONE;
                    }
    ...
    case EFI_IFR_SET_OP:
    {
      ...
      if(IfrSet->VarStoreInfo.VarOffset == CtrlOffset && (Data1.Value.u64 == ThisValue ))//If Offset is same then for same control we are going to apply write set value 
             PushThisToNvram = TRUE;
        ...
        if (varInfo != NULL) {      //Handling Varstore\EfiVarstore in same case.
            if(varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_VARSTORE || varInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_EFI_VARSTORE)
            {
                ...
                if(Data1.Type != EFI_IFR_TYPE_UNDEFINED){
                    Status = VarGetandSetQuestionValue(VarStoreId, IfrSet->VarStoreInfo.VarOffset, (( Size > sizeof(UINT64))? sizeof(UINT64) : (UINT32)Size ), qValue, varInfo,PushThisToNvram, ControlInfo);
                }
                else
                {
                    Value->Type = EFI_IFR_TYPE_UNDEFINED;
            ...
            else if (varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_NAMEVALUE)    //Added code to handle NameValue varstore. Not able to test as of now, as vfrcompiler gives error.
            {
                if (IfrSet->VarStoreType != EFI_IFR_TYPE_STRING) {
                    qValue = Data1.Value.u64;
                    if(Data1.Type != EFI_IFR_TYPE_UNDEFINED){
                        Status = VarGetandSetQuestionValue(VarStoreId, 0, ((Size > sizeof(UINT64)) ? sizeof(UINT64) : (UINT32)Size), qValue, varInfo,PushThisToNvram, ControlInfo);
                     } 
                    else
                    {
                        Value->Type = EFI_IFR_TYPE_UNDEFINED;
    ...
    case EFI_IFR_SHIFT_RIGHT_OP:
        GetValueFromOpCode(ifrData,Value,Result);
        if(Value->Type == EFI_IFR_TYPE_UNDEFINED)
          goto DONE;
      break;
  ...
  if (Value && EFI_ERROR (Status)) 
  {
      Value->Type = EFI_IFR_TYPE_UNDEFINED;
  }    
  if(Value)
      MemCopy(Value1,Value,sizeof(EFI_HII_VALUE));
}
...
UINTN _AddHpkControls(EFI_HII_HANDLE Handle, UINT8 *buff,UINTN Size, PAGE_INFO **NewPgInfo, UINT32 *AllocatedPageSize, UINT32 *PgOffset)
{
   ...
   UINTN  i =0, end=0,j=0,index=0, itr = 0;
    ...
    case EFI_IFR_GUID_OP:
        if (EfiCompareGuid ((EFI_GUID *)((UINT8*)opHeader + sizeof (EFI_IFR_OP_HEADER)), &gEdkiiIfrBitVarstoreGuid)) 
        {
            SETUP_DEBUG_VAR("[TSE] _AddHpkControls gEdkiiIfrBitVarstoreGuid opcode Found \n" ); 
            BitWiseVariable = TRUE;
        }
        addControl = FALSE;
        break;
	 default:
      ...
      break;
		...
		if(addControl)
		{
	      defaults_size =0;
            ...
            if (BitWiseVariable == TRUE)
            {
                control_Flags.BitFieldVarstore = 1;
                BitWiseVariable = FALSE;
            }
            ...
            if (BitWiseVariable == TRUE)
            {
                control_Info->ControlFlags.BitFieldVarstore = 1;
                BitWiseVariable = FALSE;
            }
...
//	EFI_STATUS status = EFI_SUCCESS;
  ...
  if(gIFRChangeNotify)
  {
	...
	{
		//Allocate 8k at a time
		gControlInfo = MemReallocateZeroPool( gControlInfo, ControlListSize, ControlListSize + 8192 );
/*
...
DONE:
	SETUP_DEBUG_UEFI( "\n[TSE] In AddPageIdToList(), Adding PageIdInfo [PageGuid: %g Pageclass: %d PageSubClass: %d] returns with Status %r\n",&NewPageIdInfo->PageGuid,NewPageIdInfo->PageClass,NewPageIdInfo->PageSubClass, status) ; 
...
EFI_STATUS AddPageToList(PAGE_INFO *NewPgInfo, UINT32 PageSize)
{
  ...
  if(gIFRChangeNotify)
  {
    ...
    {
    	SETUP_DEBUG_UEFI_PARSE( "\n[TSE] Page added into its old index(%d) \n",NewPgInfo->PageID);
      	goto DONE;
    }
	...
	if(PageInfoOffset + PageSize >= PageInfoSize)
	{
		...
		{
			status = EFI_OUT_OF_RESOURCES;
		    SETUP_DEBUG_UEFI_PARSE( "\n[TSE] AddPageIdToList() returned, status = %r\n" , status );
			goto DONE;
		}
	...
	SETUP_DEBUG_UEFI_PARSE( "\n[TSE] Page added into the index(%d) \n",NewPgInfo->PageID);
...
EFI_STATUS AddVariableToList(VARIABLE_INFO *NewVariableInfo)
{
	...
	if(!VariableListPtr)
	{
		...
		{
			status = EFI_OUT_OF_RESOURCES;
		    SETUP_DEBUG_UEFI( "\n[TSE] In AddVariableToList() returned,  status = %r\n" , status );
			goto DONE;
		}
	...
	else
	{
		...
		if(*Offset + BuffSize >= *AllocatedSize)
		{
			...
			*PageInfo = MemReallocateZeroPool( *PageInfo, *AllocatedSize, *AllocatedSize + 128 );
			...
			{
				status = EFI_OUT_OF_RESOURCES;
			    SETUP_DEBUG_UEFI( "\n[TSE] CreatePage() returned,  status = %r \n" , status );
				goto DONE;
			}
	...
	//                {
				    if (FilterDHCredentialClassGuid())
				    {
	                    ...
	                    if ((NULL == gSfHandles) && ((EfiCompareGuid (&formSet->Guid, &DriverHealthHiiGuid))||
	                            (IsSFGuidMatchClassId((TSE_EFI_IFR_FORM_SET*)formSet, &DriverHealthHiiGuid))||
	                            (EfiCompareGuid (&formSet->Guid, &CredentialHiiGuid))||
	                            (IsSFGuidMatchClassId((TSE_EFI_IFR_FORM_SET*)formSet, &CredentialHiiGuid))) )         //if driverhealth page we have to not consider it as dynamic page so returning aborted
...
DONE:
    SETUP_DEBUG_UEFI ("\n[TSE] ParseForm() returned,   status = %r \n" , status ); 
	return status;
}
    ...
    if(pageInfo->PageHandle == (VOID *)(UINTN)0xFFFF)
    {
      if(pageInfo->PageFormID == NewPgInfo->PageFormID)
      {
        ...
        }else
        {
          ...
          SETUP_DEBUG_UEFI( "\n[TSE] _AdvAddPageToList(), Page with PageID: %d is added at Index: %d\n",NewPgInfo->PageFormID,NewPgInfo->PageIdIndex );
          status = EFI_UNSUPPORTED;
        }
        break;
      }
...
DONE:
    SETUP_DEBUG_UEFI ("[TSE] ValidateParseForm() returned, status = %r \n" , status ); 
	return status;
}
...
UINT32 CreateFakeVariable (EFI_HII_HANDLE Handle, void *IfrData)
{
    ...
    i = VariableListPtr->VariableCount;
	SETUP_DEBUG_UEFI_PARSE("[TSE] Adding Fake Vairable(%s)\n", FakeVariableName);
    AddVariableToList(&Variable);
...
EFI_STATUS VarGetandSetQuestionValue(UINT32 variable, UINTN Offset, UINTN Size ,UINT64 qValue, VARIABLE_INFO *varInfo, BOOLEAN PushThisToNvram, CONTROL_INFO *ControlInfo)
{
    ...
    {
        Status = _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, gVariableList, variable, 
            (UINT32) Offset, Size, (VOID*)&buffer, ControlInfo->ControlPtr);
    }
    else 
    {
        Status = VarGetValue(variable, (UINT32) Offset, Size, (VOID*)&buffer );
    }
    ...
    if ( ! EFI_ERROR( Status ) )  
    {
        ...
        if(value != qValue || (PushThisToNvram == TRUE))
		{
            Status  = UefiVarSetNvram(varInfo, &qValue, Offset, Size );
            if(! EFI_ERROR( Status ))
            {
                if (ControlInfo->ControlFlags.BitFieldVarstore)
                {
                    Status = _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, gVariableList, variable, 
                            (UINT32) Offset, Size, &qValue, ControlInfo->ControlPtr);
                }
                else 
                {
                    Status = VarSetValue(variable,(UINT32) Offset, Size, &qValue);
                }
            }
		}       
    }
