...
#if((ESA_BINARY_SUPPORT ==1) && defined(ESA_TSE_FULL_SUPPORT )&& (ESA_TSE_FULL_SUPPORT ==0) && (SETUP_GIF_LOGO_SUPPORT == 1))
...
#endif
	SETUP_DEBUG_TSE("[TSE] Inside SendForm(): Handles %x, HandleCount = %d  FormSetGuid = %g, FormId - %x\n", Handles, HandleCount, FormSetGuid, FormId);
    UpdateGoPUgaDraw();
	...
	if(HandleCount)
	{
		...
		for(i=0;i<HandleCount;i++)
		{
			...
			if( FormSet != NULL )
			{
                ...
                SETUP_DEBUG_TSE("[TSE] gSfHandles[%x] = 0x%x\n",gSfHandleCount, gSfHandles[gSfHandleCount]);
				gSfHandleCount++;
			}
		}
...
/**
    This function is used to show the warning infobox when a driver tries to update its value at action EFI_BROWSER_ACTION_FORM_OPEN

           
    @param VarHandle current variable handle
   
**/

void ShowBrowserActionOpenWarning(EFI_HANDLE VarHandle)
{
    ...
    CHAR16      *DriverName = NULL, *newString = NULL;
    UINTN length = 0;
    EFI_EVENT Event = NULL;
    CHAR16 *Title = NULL,*Message = NULL;
    static EFI_HANDLE   BackupHandle = NULL;
    if(BackupHandle != VarHandle)
    {
       SETUP_DEBUG_UEFI_CALLBACK("\n[TSE]ShowBrowserActionOpenWarning() Entering: \n");
        Status = gHiiDatabase->GetPackageListHandle (gHiiDatabase, VarHandle, (VOID**)&DriverHandle);
       SETUP_DEBUG_UEFI_CALLBACK("[TSE]GetPackageListHandle Status :%r\n",Status);

    if(!EFI_ERROR(Status))
    {  
        SETUP_DEBUG_UEFI_CALLBACK("[TSE]Got DriverHandle %g from VarHandle %g\n" ,DriverHandle,VarHandle);
        ...
        SETUP_DEBUG_UEFI_CALLBACK("[TSE]Status of HandleProtocol[ EfiComponentName2ProtocolGuid]: %r\n",Status);
        ...
        if (!EFI_ERROR (Status))
        {
            ...
            SETUP_DEBUG_UEFI_CALLBACK("[TSE]Status of GetDriverName: %r\n",Status);
            if (!EFI_ERROR (Status))

            {
                SETUP_DEBUG_UEFI_CALLBACK("[TSE]DriverName from GetDriverName: %s\n",DriverName);
                if( gSetupStatus == TSE_SETUP_STATUS_INITIAL_FORM_OPEN )
                {
                    Title = HiiGetString(gHiiHandle, STRING_TOKEN(STR_WARNING));
                    newString = HiiGetString(gHiiHandle,STRING_TOKEN(STR_BROWSER_ACTION_OPEN_WARNING));
                    length = EfiStrLen(newString) + EfiStrLen(DriverName);
                    Message = EfiLibAllocateZeroPool( length * sizeof(CHAR16) );
                    SPrint( Message, length * sizeof(CHAR16), newString, DriverName );
                   ...
                   SETUP_DEBUG_UEFI_CALLBACK("[TSE]ShowBrowserActionOpenWarning was successfully done with Message: %s\n",Message);
                ...
                SETUP_DEBUG_UEFI_CALLBACK("[TSE] Can't retrive DriverName from GetDriverName\n");
        }

        }
        BackupHandle = VarHandle;
        SETUP_DEBUG_UEFI_CALLBACK("\n[TSE]ShowBrowserActionOpenWarning  exiting\n");
    }
}
/**
...
//  CONTROL_INFO  *ctrlInfo = NULL; Unused
  ...
  UINT32 Offset = 0;
	...
	if((gBrowserCallbackEnabled == FALSE) && (gEnableDrvNotification == FALSE) ) { 	//Allow Browser callback for AMI ExecuteCallback(Except EFI_BROWSER_ACTION_FORM_OPEN)/AccessConfig/RouteConfig calls
		status = EFI_UNSUPPORTED;
		goto DONE;
	}
	
    if((gBrowserCallbackAction == EFI_BROWSER_ACTION_FORM_OPEN )&&(gSetupStatus == TSE_SETUP_STATUS_INITIAL_FORM_OPEN))
    {
     SETUP_DEBUG_UEFI_CALLBACK ( "[TSE]Calling ShowBrowserActionOpenWarning() since gBrowserCallbackAction and gSetupStatus is FORM_OPEN");
     ShowBrowserActionOpenWarning(gCurrVarHandle);
    }
    ...
    for(index = 0; index < gVariables->VariableCount; index++)
    {
      ...
      if(EfiCompareGuid(&varInfo->VariableGuid, (EFI_GUID *)VariableGuid))
      {
        if((varInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_NAMEVALUE) != VARIABLE_ATTRIBUTE_NAMEVALUE)
        {
          ...
          {
            status = EFI_NOT_FOUND;
		    SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Variable Guid(%g) passed without Variable Name(%g). So exiting with status %r\n", &varInfo->VariableGuid, varInfo->VariableName, varInfo->VariableHandle, status);
            goto DONE;
          }
          ...
          if (EfiStrCmp (varInfo->VariableName, (CHAR16 *)VariableName) == 0)
          {
				...
				{
					SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Found Variable(%s  %g) but not from same driver [Variable Handle = 0x%X, callback notified handle = 0x%X]\n", varInfo->VariableName, &varInfo->VariableGuid, varInfo->VariableHandle, gCurrVarHandle  );
				}
          }
        }
      }
  ...
  } else
  {
	...
	SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Variable Guid is NULL (Handle = 0x%X). So trying with gCurrVarIndex(%d)\n", gCurrVarIndex);
	...
	if (NULL == varInfo){
		SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Variable Not found in TSE list\n");
		status = EFI_NOT_FOUND;
		goto DONE;
	}

  SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Found Variable(%s  %g) from same driver(0x%X) to which ConfigAccess Callback notified\n", varInfo->VariableName, &varInfo->VariableGuid, varInfo->VariableHandle  );
  ...
  if(buffer == NULL)
  {
  	SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Variable buffer is not initialized\n");
  	status = EFI_NOT_FOUND;
    goto DONE;
  }
  ...
  status = GetConfigHeader(varInfo, &configHdr, &length);
	...
	{
		SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] GetConfigHeader returned,  status = %r\n" , status );  
		goto DONE;
	}
  ...
  if(RetrieveData)
  {
	  ...
	  do
	  {
		  ...
		  if (EFI_ERROR(status))
		  {
			  SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] _StorageToConfigResp returned,  status = %r\n" , status ); 
			  goto DONE;
		  }
...
/*
#if TSE_DEBUG_MESSAGES
	{
		...
		SPrint(BrowserCallbackFilename,50 * sizeof (CHAR16),L"BROWSERCALLBACK_%s_%03d.txt", varInfo->VariableName, HpkFileCount);
		SPrint(BrowserCallbackContent, 200 * sizeof (CHAR16), L"VariableName: %s\nVariableGuid: %g\nResultsDataSize: 0x%x\n", VariableName, &VariableGuid, *ResultsDataSize);
		...
		{
		    StrCpyS (BrowserCallbackBuffer, Len / sizeof(CHAR16), BrowserCallbackContent);
		    StrCatS (BrowserCallbackBuffer, Len / sizeof(CHAR16), ResultsData);
...
*/
    ...
    StrCpyS (configResp, bufferSize / sizeof(CHAR16), configHdr);
    ...
    SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] Displaying Browser Uncommitted Data Buffer: Result Data : %s \n",ResultsData );
    ...
    do{
    	...
    	if (EFI_ERROR (status))
    	{
    		SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] _ConfigRespToStorage returned status = %r \n" , status );
    		goto DONE;
    	}
		
		SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Uncommitted data in buffer \n");
#if TSE_DEBUG_MESSAGES
    if((gDbgPrint & PRINT_UEFI_VAR) == PRINT_UEFI_VAR)
	{
    	...
    	SETUP_DEBUG_UEFI_CALLBACK (  "[TSE] Updating Browser Uncommitted Data : Offset = 0x%x  Size  = 0x%x\n",Offset, size );
...
//MEMFREE
  ...
  SETUP_DEBUG_UEFI_CALLBACK ( "[TSE] Exiting BrowserCallback(),  status = %r \n\n" , status );  
...
////////Variables available for both TSE and ESA Ends///////////
	...
	SETUP_DEBUG_TSE ("[TSE] SaveTseSetupGlobalData(): returned with status EFI_SUCCESS\n");
	return EFI_SUCCESS;

ERROR:
	MemFreePointer((VOID**)&temp); //Free the node
	SETUP_DEBUG_TSE ("[TSE] SaveTseSetupGlobalData(): returned with status EFI_UNSUPPORTED\n");
	return EFI_UNSUPPORTED;
}
...
EFI_STATUS RestoreTseSetupGlobalData()
{
	...
	if(temp == NULL)
	{
		SETUP_DEBUG_TSE ("[TSE] In RestoreTseSetupGlobalData() returning - No Buffer found in Stack\n");
...
////////Variables available only for TSE Ends///////////		
	...
	MemFreePointer((VOID**)&temp); //Free the node
	SETUP_DEBUG_TSE ("[TSE] RestoreTseSetupGlobalData(): Restored succesfully\n");
	return EFI_SUCCESS;
}
            ...
            for(i = 0; i < gSfHandleCount; i++)
            {
                if(PageInfo->PageHandle == gSfHandles[i])
                {
                    *StartPageId = PageInfo->PageID;
                    Status = EFI_SUCCESS;
                    SETUP_DEBUG_TSE("[TSE] Handle Found = 0x%x\n", PageInfo->PageHandle);
                    goto END;
                }
            }
        }
...
/**
	This functions clears the gBootData and gDriverData. Usually we won't clear boto related buffer. 
	But in case of sendform, we will push the old boot data (if available) into global variable stack
	and we will recreate boot data for sendform. So we need to free it.
...
// Free gBootData   
   for(Index = 0; Index < gBootOptionCount && gBootData; Index++)
   {
       if(gBootData[Index].DevicePath)
           MemFreePointer((void**)&gBootData[Index].DevicePath);
       
       if(gBootData[Index].Name)
           MemFreePointer((void**)&gBootData[Index].Name);
       
       if(gBootData[Index].LoadOptions)
           MemFreePointer((void**)&gBootData[Index].LoadOptions);
       
       if(gBootData[Index].OrderList)
           MemFreePointer((void**)&gBootData[Index].OrderList);
   }
...
//Free gDriverData
   for(Index = 0; Index < gDriverOptionCount && gBootData; Index++)
   {
       if(gDriverData[Index].DevicePath)
           MemFreePointer((void**)&gDriverData[Index].DevicePath);
       
       if(gDriverData[Index].Name)
           MemFreePointer((void**)&gDriverData[Index].Name);
       
       if(gDriverData[Index].LoadOptions)
           MemFreePointer((void**)&gDriverData[Index].LoadOptions);
   }
