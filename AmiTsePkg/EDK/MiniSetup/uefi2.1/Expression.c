  ...
  )
{
  ...
  StrCpyS (StringPtr, ( EfiStrSize (String[1]) + EfiStrSize (String[0])) / sizeof(CHAR16), String[1]);
  StrCatS (StringPtr, (EfiStrSize (String[1]) + EfiStrSize (String[0])) / sizeof(CHAR16), String[0]);
...
*/
	...
	if(found)
	{
		if (NULL != controlInfo->ControlPtr)      //For inconsistent and nosubmitif controlptr will be NULL
		{
			...
			else if(controlInfo->ControlFlags.BitFieldVarstore)
			{
			    NVRAM_VARIABLE *list = gVariableList;
			    if (( list == NULL ) )
			        list = gOptimalDefaults;

			    Status = _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, list, varId, questionHdr->VarStoreInfo.VarOffset, 
			            controlInfo->ControlDataWidth, &questionValue,controlInfo->ControlPtr );
			    if(EFI_ERROR(Status))
			    {
			        goto DONE;
			    }
...
#define ZERO 0
    ...
    case EFI_IFR_GET_OP:
    {
        ...
        if (varInfo != NULL) {      //Handling Varstore\EfiVarstore in same case.
            ...
            else if (varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_NAMEVALUE)        
            {
        ...
        if (varInfo != NULL) {      //Handling Varstore\EfiVarstore in same case.
            if(varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_VARSTORE || varInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_EFI_VARSTORE)
            {
                ...
                if(Data1.Type != EFI_IFR_TYPE_UNDEFINED){
                    Status = VarSetValue(VarStoreId, IfrSet->VarStoreInfo.VarOffset, (( Size > sizeof(UINT64))? sizeof(UINT64) : (UINT32)Size ), &qValue);
                }
                if(EFI_ERROR(Status))
                {
                    goto DONE;
            ...
            else if (varInfo->ExtendedAttibutes == VARIABLE_ATTRIBUTE_NAMEVALUE) 
            {
                if (IfrSet->VarStoreType != EFI_IFR_TYPE_STRING) {
                    qValue = Data1.Value.u64;
                    if(Data1.Type != EFI_IFR_TYPE_UNDEFINED){
                        Status = VarSetValue(VarStoreId, 0, (( Size > sizeof(UINT64))? sizeof(UINT64) : (UINT32)Size ), &qValue);
                    }
                    if(EFI_ERROR(Status))
                    {
                        goto DONE;
...
BOOLEAN EvaluateDisableifOverForm(EFI_IFR_OP_HEADER *OpHeader)
{
	...
	if(DisableScope && tempopHdr) // To avoid dummy disable if.
	{
		...
		MemFreePointer((void**)&DummyControl);
