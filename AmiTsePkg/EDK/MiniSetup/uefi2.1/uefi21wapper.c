...
//	CHAR16 tmpString = (CHAR16)NULL; cast from pointer to integer of different size
  ...
  status = GetConfigHeader(VariableInfo, &configuration, &length);
	...
	if(VariableInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_NAMEVALUE)
	{
		...
		StrCatS(configuration, len , VariableInfo->VariableName);
	...
	{
	    SETUP_DEBUG_UEFI("\n[TSE]UefiVarGetNvram()->ExtractConfig(): %s \n",configuration);
		status = gHiiConfigRouting->ExtractConfig (gHiiConfigRouting, configuration, &progress, &result);
		SETUP_DEBUG_UEFI("\n[TSE] Return status = %r, result = %s\n",status, (result != NULL) ? result : L"<null>");
	}
	else
	{
		...
		if(!EFI_ERROR(status))
		{
			...
			if(!EFI_ERROR(status))
			{
			    SETUP_DEBUG_UEFI("\n[TSE] UefiVarGetNvram()->ExtractConfig() for NameValue: %s \n",configuration);
				status = ConfigAccess->ExtractConfig (ConfigAccess, configuration, &progress, &result);
				SETUP_DEBUG_UEFI("\n[TSE] Return status = %r, result = %s\n",status, result ? result : L"NULL" );
...
EFI_STATUS UefiVarSetNvram(VARIABLE_INFO *VariableInfo, VOID *Buffer, UINTN Offset, UINTN Size)
{
  ...
  //
    ...
    if (!(VariableInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_NAMEVALUE))
    {
        StrCatS(configuration, finalLen / sizeof(CHAR16), L"&VALUE=");
    }
    StrCatS(configuration, finalLen / sizeof(CHAR16), tempBuffer);
    if (VariableInfo->ExtendedAttibutes & VARIABLE_ATTRIBUTE_NAMEVALUE)
    {
        StrCatS (configuration, finalLen / sizeof(CHAR16), L"&");

    }
    MemFreePointer((VOID**)&tempBuffer);
    PreservegEnableDrvNotification =  gEnableDrvNotification; 
	 gEnableDrvNotification = TRUE; // Set to enable notification processing
	 SETUP_DEBUG_UEFI("\n[TSE] In UefiVarSetNvram(), calling RouteConfig() for varstore configuration = %s \n",configuration);
    status = gHiiConfigRouting->RouteConfig (gHiiConfigRouting, configuration, &progress);
    SETUP_DEBUG_UEFI("\n[TSE] In UefiVarSetNvram(), RouteConfig() returned with status = %r progress = %s \n",status,progress ? progress : L"NULL" );
...
VOID HiiRemoveString(EFI_HII_HANDLE Handle, UINT16 Token)
{
  ...
  {
  	HiiChangeStringLanguage( Handle, Token, gLanguages[i].Unicode, L"" );
  }
}
  ...
  if(UefiAction->Header.Length == sizeof(EFI_IFR_ACTION))
  {
    if(UefiAction->QuestionConfig)
    {
	  ...
	  if((configuration != NULL) && EfiStrLen(configuration))
	  {
		  PreservegEnableDrvNotification = gEnableDrvNotification;
        gEnableDrvNotification = TRUE; //Set to enable notification processing
        SETUP_DEBUG_UEFI("In ProcessActionQuestionConfiguration(), Before calling RouteConfig() configuration = %s \n",configuration);
		  Status = gHiiConfigRouting->RouteConfig(gHiiConfigRouting, configuration, &progress);
		  SETUP_DEBUG_UEFI("In ProcessActionQuestionConfiguration(), after calling RouteConfig(),status = %r progress = %s \n",Status,progress ? progress : L"NULL" );
		  if (!PreservegEnableDrvNotification) // If gEnableDrvNotification is already True Don't touch it
        	gEnableDrvNotification = FALSE; //Reset to disable notification processing
	  }
	  if(configuration)
...
EFI_STATUS DoDisconnectAndConnectController (UINT32 OldPageId, UINT32 NewPageId);
extern BOOLEAN BrowserActionDiscard;
EFI_HANDLE ReconnectPageHandle = NULL;
EFI_STATUS ProcessBrowserActionRequest(EFI_BROWSER_ACTION_REQUEST ActionReq)
{
  ...
  case EFI_BROWSER_ACTION_REQUEST_RECONNECT:
    if(gApp->CurrentPage)
       ReconnectPageHandle = ((PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[gApp->CurrentPage]))->PageHandle;
	break;
  default:
    break;
  }
...
{	
    ...
    UINT32 Size = 0;
	EFI_STATUS Status=EFI_UNSUPPORTED;
    EFI_STATUS CallbackStatus=EFI_UNSUPPORTED;
	UINT8 Flags = 0;
	UINT32 offset = UefiGetQuestionOffset(ControlData->ControlPtr);
	...
	else
	{
		...
		Size = (UefiGetMaxEntries (ControlData->ControlPtr)) * Size;
		VarSize = Size;
		
	}
    if (ControlData->ControlFlags.BitFieldVarstore)
    {
        NvBuf = _VarGetValueForBitFieldNvram(ControlData->ControlVariable, offset, Size, ControlData->ControlPtr); 
    }
    else
    {   
    ...
    if(NvBuf == NULL){ // No Variable Assosiated then Just refrsh the page
        ...
        {    
            CallbackStatus = CallFormCallBack(ControlData,UefiGetControlKey(ControlData),0,EFI_BROWSER_ACTION_RETRIEVE);
	...
	case CONTROL_TYPE_PASSWORD:
        if (ControlData->ControlFlags.BitFieldVarstore)
        {
            _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, gVariableList, ControlData->ControlVariable, offset, Size, CurBuf, ControlData->ControlPtr);
         }
        else 
        {
              VarGetValue( ControlData->ControlVariable, UefiGetQuestionOffset(ControlData->ControlPtr), Size, CurBuf );
        }
		break;
	case CONTROL_TYPE_DATE:
	case CONTROL_TYPE_TIME:
	...
	if(MemCmp((UINT8 *)NvBuf, (UINT8 *)CurBuf, Size))
	{
	    if (ControlData->ControlFlags.BitFieldVarstore)
	    {
	        _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, gVariableList, ControlData->ControlVariable, offset, Size, NvBuf, ControlData->ControlPtr);
	...
	{    
	    ...
	    if(!EFI_ERROR(CallbackStatus))
	        Status = CallbackStatus; //Refresh if retrieve was successful, otherwise use the Status from buffer comparison; if the same - do nothing
	}
...
//	EFI_GUID 	DriverHealthProtocolGuid = EFI_DRIVER_HEALTH_PROTOCOL_GUID;
    ...
    SETUP_DEBUG_UEFI ("\n[TSE] Entering FindDriverHealthDriverName: DriverEntry = %d\n", DriverEntry );
...
CHAR16 *GetCtrlNameAndHealth (UINT16 ControllerEntry)
{
	...
	SETUP_DEBUG_UEFI("[TSE]GetCtrlNameAndHealth: ControllerEntry :%X ContrNameAndHlth :%s\n",ControllerEntry,ContrNameAndHlth ? ContrNameAndHlth : L"NULL");
...
VOID AddHandleIntoList (EFI_HANDLE ControllerHandle, EFI_HANDLE ChildHandle, EFI_DRIVER_HEALTH_STATUS HealthStatus)
{
	...
	SETUP_DEBUG_UEFI("\n[TSE] AddHandleIntoList(): Driver health handle [ControllerHandle = 0x%X ChildHandle = 0x%X HealthStatus = %d] added into the list\n", ControllerHandle, ChildHandle, HealthStatus);
}
...
VOID FreeControllersList (VOID)
{
	...
	SETUP_DEBUG_UEFI("\n[TSE] FreeControllersList() : Freed all controllers\n");
}
...
VOID SetDrvHealthHandleAndCtrlCount (UINT16 ItemEntry)
{
	...
	SETUP_DEBUG_UEFI("\n[TSE]SetDrvHealthHandleAndCtrlCount() for Entry = 0x%X  TotControllers:%X \n",ItemEntry, TotControllers);
}
...
VOID DisplayMsgListMessageBox (TSE_EFI_DRIVER_HEALTH_HII_MESSAGE *MessageList)
{
    ...
    while (MessageList [Count].HiiHandle)
    {
        ...
        { //If messagecode is non zero only, append it to String
            ...
            if(MessageList[Count].MessageCode)
            {
                ...
                SPrint(MessageCode,50 * sizeof (CHAR16),L" Message Code : %ld \n",MessageList[Count].MessageCode);
  ...
  StrCatS (String,(EfiStrLen (MessageCode) + 2) + (EfiStrLen (String) + 1), MessageCode);
  
            }              
            text = HiiGetString (gHiiHandle, STRING_TOKEN (STR_DRV_HLTH_TITLE));
                SETUP_DEBUG_UEFI("\n[TSE] DisplayMsgListMessageBox(): Message string shown in message box :%s  HealthStatus = %s\n",text ? text : L"NULL", String);
	...
	if(!EFI_ERROR(Status))
    {
		...
		if (!EFI_ERROR(Status))
		{
			...
			if (!EFI_ERROR(Status))
			{
				...
				if (!EFI_ERROR(Status))
	 			{
					SETUP_DEBUG_VAR ( "[TSE] In CreateFile(), File created successfully with name %s\n", filename ? filename : L"NULL");
					return FileHandle;
				}
			}
		}
...
DONE:
	if(!Status && filename)
	    SETUP_DEBUG_VAR( "[TSE] WriteDataToFile(): Data written in Filename: %s successfully\n",filename);  
	else
	    SETUP_DEBUG_VAR( "[TSE] WriteDataToFile():  failed to write data in Filename: %s Status: %x\n", Status);
...
VOID ProcessPackToFile(EFI_HII_DATABASE_NOTIFY_TYPE NotifyType, EFI_HANDLE Handle)
{
	...
	SPrint(filename, 50 * sizeof (CHAR16) , filenameFormat, Handle, HpkFileCount);

	Status = WriteDataToFile(filename, formSetData, length, 0) ;
	
	if (!EFI_ERROR (Status))
	{
		SETUP_DEBUG_UEFI_NOTIFICATION( "\n\n[TSE] ProcessPackToFile(): HpkFile written into file successfully\n");
		HpkFileCount++ ;
	}
...
#if TSE_DEBUG_MESSAGES
	if(((gDbgPrint & PRINT_UEFI_NOTIFICATION) == PRINT_UEFI_NOTIFICATION) && EFI_ERROR(Status))
		SETUP_DEBUG_UEFI_NOTIFICATION( "\n\n[TSE] ProcessPackToFile(): Failed to write HpkFile data into file\n");
#endif
	if(formSetData)
...
EFI_STATUS WritePackToFile(CHAR16 *filenameFormat, EFI_HANDLE Handle, UINT8 *PackData, UINTN length)
{
	...
	SPrint(filename, 50 * sizeof (CHAR16), filenameFormat, Handle, HpkFileCount);
	SETUP_DEBUG_UEFI_NOTIFICATION( "[TSE] In WritePackToFile \n");
...
VOID DebugShowBufferContent(UINTN bufSize, VOID *buffer)
{
 ...
 //#if TSE_DEBUG_MESSAGES == 2
    ...
    for(index = 0; index < size; ){
        SETUP_DEBUG_TSE ( "[0x%08x (%04d)]  ", index, index);
        for(j = 0; (j < 16) && (temp != NULL); index++, j++){
            if(index >= size){
                break;
            }
            if(j==8) SETUP_DEBUG_TSE ( "- ");
            SETUP_DEBUG_TSE ( "%02x ", (UINT8)temp[index]);
        }
        SETUP_DEBUG_TSE ("\n");
	}
...
EFI_STATUS DebugShowControlInfo(UINT32 formID, VOID *passedCtrlInfo)
{
    ...
    if (String)
    {
	    SETUP_DEBUG_TSE("Control Prompt: (0x%x) %s\n", ctrlInfo->QuestionId, String);
    }
    ...
    SETUP_DEBUG_TSE("ControlDataLength: 0x%x (%d)\n", ctrlInfo->ControlDataLength, ctrlInfo->ControlDataLength);
    SETUP_DEBUG_TSE("ControlDataWidth: 0x%x (%d)\n", ctrlInfo->ControlDataWidth, ctrlInfo->ControlDataWidth);
    SETUP_DEBUG_TSE("ControlKey: 0x%x (%d)\n", ctrlInfo->ControlKey, ctrlInfo->ControlKey);
    ...
    if(ctrlInfo->ControlHandle != NULL){
            SETUP_DEBUG_TSE("ControlVarID: %d \n", ctrlInfo->ControlVariable );
            SETUP_DEBUG_TSE("ControlOffset: 0x%x (%d)\n", UefiGetQuestionOffset(ctrlInfo->ControlPtr), UefiGetQuestionOffset(ctrlInfo->ControlPtr));
            SETUP_DEBUG_TSE("ControlWidth: 0x%x (%d) \n", UefiGetWidth(ctrlInfo->ControlPtr), UefiGetWidth(ctrlInfo->ControlPtr));
    } 
DONE:
	MemFreePointer( (VOID **)&String );
		...
		switch(opHeader->OpCode)
		{
			case EFI_IFR_FORM_OP: //0x01 - Form
                ...
                if(((EFI_IFR_FORM*)(ifrData + i))->FormId == pageInfo->PageFormID)
                {
                   ...
                   if(Text)
                   {
                       SETUP_DEBUG_TSE ("[TSE] FormTitle: %s Handle: 0x%x\n", Text,pageInfo->PageHandle);
                       MemFreePointer( (VOID **)&Text );
                   }
...
VOID DebugShowPrintPageDetails(UINTN Start, UINTN End)
{
    UINTN i;
    for(i = Start; i < End; i++)
    {
            ...
            if((pageInfo->PageHandle != NULL) && (pageInfo->PageHandle != (void *)0xFFFF))
            {
                ...
                if(String)
                {
                    SETUP_DEBUG_TSE("[TSE] PageName: %s PageID: %d PPID: %d H: 0x%x PageFormID: %d\n",String, pageInfo->PageID, pageInfo->PageParentID, pageInfo->PageHandle,pageInfo->PageFormID); 
                    MemFreePointer((VOID**)&String);
                }
                else
                {
                    SETUP_DEBUG_TSE("[TSE] PageName: ??? PageID: %d PPID: %d H: 0x%x PageFormID: %d\n", pageInfo->PageID, pageInfo->PageParentID, pageInfo->PageHandle,pageInfo->PageFormID); 
                }
            }
     }
}

VOID DebugShowPrintVariableDetails(UINT16 Start, UINT16 End)
{
    UINT16 i; 
    VARIABLE_INFO *varInfo = (VARIABLE_INFO *)NULL;
    for ( i = Start; i < End; i++)
    {
        ...
        if(varInfo)
        {
            ...
            SETUP_DEBUG_TSE("Var[%d] %s Guid: %g Handle: 0x%X Size: %d Buf: 0x%X\n", 
                i, varInfo->VariableName, &(varInfo->VariableGuid), varInfo->VariableHandle, varInfo->VariableSize, gVariableList[i].Buffer);      
		}
    }
}

EFI_STATUS DebugShowPrintOptimalDefaults(UINT32 varIndex)
{
	EFI_STATUS status = EFI_SUCCESS;
	NVRAM_VARIABLE *nvPtr = (NVRAM_VARIABLE *)NULL;
    VARIABLE_INFO *varInfo = (VARIABLE_INFO *)NULL ;
	...
	if(size){
        ...
        SETUP_DEBUG_VAR ("----------- End of DebugShowOptimalDefaults, status: 0x%x -----------\n\n", status); 
	} else{
        SETUP_DEBUG_VAR ("!!!!!! VariableName: %s, size: %d !!!!!\n", varInfo->VariableName, nvPtr[varIndex].Size); 
    }
...
EFI_STATUS DebugShowPrintFailSafeDefaults(UINT32 varIndex)
{
    EFI_STATUS status = EFI_SUCCESS;
    NVRAM_VARIABLE *nvPtr = (NVRAM_VARIABLE *)NULL;
    VARIABLE_INFO *varInfo = (VARIABLE_INFO *)NULL ;
    UINTN index = 0, j = 0, size = 0 ;
    UINT8 *temp = (UINT8 *)NULL ;
    if(!gFailsafeDefaults)
      return EFI_UNSUPPORTED;  
    nvPtr = gFailsafeDefaults;
    size = nvPtr[varIndex].Size < 0xFFFF ? nvPtr[varIndex].Size : 0;
    temp = nvPtr[varIndex].Buffer;
    varInfo = VarGetVariableInfoIndex( varIndex );

    if(size){
        SETUP_DEBUG_VAR ("\n----------- DebugShowFailsafeDefaults -----------\n");
        SETUP_DEBUG_VAR ("VariableName: %s, VariableHandle: 0x%x, Buffer size: %d, Buffer: 0x%x \n", varInfo->VariableName, varInfo->VariableHandle, size, nvPtr[varIndex].Buffer); 
        ...
        SETUP_DEBUG_VAR ("----------- End of DebugShowFailsafeDefaults, status: 0x%x -----------\n\n", status); 
    } else{
        SETUP_DEBUG_VAR ("!!!!!! VariableName: %s, size: %d !!!!!\n", varInfo->VariableName, nvPtr[varIndex].Size); 
    }
...
//	gProcessedForms [gProFormOffset++] = FormID;
	...
	for(index = 0; index < count; index++)
	{
		...
		if (NULL == ctrlInfo->ControlPtr) //Setup crashes when clicking Discard and Exit goto control in simulator
			continue;
		if (ctrlInfo->ControlFlags.BitFieldVarstore)
		{
		...
		else
		{
		    ...
		    {   
		        offset = 0; //offset is 0 for namevalue variables
		    }
		    else
		    {
		        offset = UefiGetQuestionOffset (ctrlInfo->ControlPtr);
		...
		if (!(ctrlInfo->ControlFlags.BitFieldVarstore) && ((offset + size) > CurVarSize))
		{
        ...
        if (ctrlInfo->ControlFlags.BitFieldVarstore)
        {
            status = _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, gVariableList, ctrlInfo->ControlVariable, offset, size, CurrentValue, ctrlInfo->ControlPtr);
            if (!EFI_ERROR (status))
            {
                _VarSetValueForBitFieldNvram(ctrlInfo->ControlVariable, offset, size, CurrentValue, ctrlInfo->ControlPtr);
            }
        }
        else
        {
...
EFI_STATUS UefiLoadPreviousValuesforCurrentform(UINT32 FormID)
{
    ...
    for(index = 0; index < count; index++)
    {
        ...
        if (ctrlInfo->ControlFlags.BitFieldVarstore)
        {
            NewValue = _VarGetValueForBitFieldNvram(ctrlInfo->ControlVariable, offset, size, ctrlInfo->ControlPtr); 
            status = _VarGetSetValueForBitField( VAR_COMMAND_GET_VALUE, gVariableList, ctrlInfo->ControlVariable, offset, size, CurrentValue, ctrlInfo->ControlPtr);
            if (!EFI_ERROR (status))
            {
                _VarGetSetValueForBitField( VAR_COMMAND_SET_VALUE, gVariableList, ctrlInfo->ControlVariable, offset, size, NewValue, ctrlInfo->ControlPtr);
            }
        }
        else
        {  
...
EFI_STATUS DoDisconnectAndConnectController (UINT32 OldPageId, UINT32 NewPageId)
{
    ...
    {  
        EFI_HANDLE OldPageHandle = ((PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[OldPageId]))->PageHandle;
        EFI_HANDLE NewPageHandle = ((PAGE_INFO*)((UINTN)gApplicationData + gPages->PageList[NewPageId]))->PageHandle;
        if(OldPageHandle != NewPageHandle)
        {
            EFI_HANDLE DriverHandle;
            Status = gHiiDatabase->GetPackageListHandle(gHiiDatabase, ReconnectPageHandle, &DriverHandle);		
            if (!EFI_ERROR (Status))
            {
                pBS->DisconnectController (DriverHandle, NULL, NULL);
                pBS->ConnectController (DriverHandle, NULL, NULL, TRUE);
                ReconnectPageHandle= NULL;
            }
