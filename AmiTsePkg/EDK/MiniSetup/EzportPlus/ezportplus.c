...
{
    gFrame.Destroy = OverRideEzportPlusFrameDestroy;
}

/**
    Destroys an existing frame and frees memory of EzportPlus Style
...
EFI_STATUS OverRideEzportPlusFrameDestroy( FRAME_DATA *frame, BOOLEAN freeMem )
{
    ...
    for ( i = 0; i < frame->ControlCount; i++, control++ )
    {
        if (*control)
        {
            if(((*control)->ParentFrameType == HELPTITLE_FRAME) || ((*control)->ParentFrameType == NAV_FRAME))
            {
