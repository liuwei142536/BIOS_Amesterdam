...
//AptioV server override - Start: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
/*  
  //
...
*/
  //AptioV server override - End: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
...
//AptioV server override - End: Acquire/Release Lock will raise TPL level, which might result in Assert because of TPL conflicts.
  ...
  if (!EFI_ERROR (Status)) {
    Status = RtcTimeFieldsValid (&Time, Century); // AptioV server override - Eip#433290
  }
  if (EFI_ERROR (Status)) {
    //
  ...
  )
{
  ...
  //
  // AptioV server override Start - Eip#433290
  Century = RtcRead (RTC_ADDRESS_CENTURY);
  Status = RtcTimeFieldsValid (Time, Century);
  // AptioV server override End - Eip#433290
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  Century = RtcRead (RTC_ADDRESS_CENTURY); // AptioV server override - Eip#433290
  ...
  if (Enable) {
    ...
    Status = RtcTimeFieldsValid (&RtcTime, Century); // AptioV server override - Eip#433290
    if (EFI_ERROR (Status)) {
      return EFI_DEVICE_ERROR;
    }
  ...
  )
{
  // AptioV server override Start - Eip#433290
  Century       = CheckAndConvertBcd8ToDecimal8 (Century);
  if (((Time->Year < 1998) && !((Time->Year/100 == Century) && (Time->Year%100 == 0))) ||
      Time->Year > 2099 ||
      Time->Month < 1 ||
      Time->Month > 12 ||
      (!DayValid (Time)) ||
