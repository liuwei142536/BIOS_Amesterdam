...
UINT32 RcSimFineEridFeedback[8] = {
  0, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0, 0, 0
};
              ...
              )
{
  ...
  if (mode == 0) {
    rcPrintf ((host, "Early RID Fine sweep for socket:%d dimm:%d\nPi Offset", socket, dimm));
  } else {
    rcPrintf ((host, "Early RID Final sweep for socket:%d dimm:%d\nPi Offset", socket, dimm));
  }
  ...
  for (eridOffset = 0; eridOffset < ERID_PI_RANGE; eridOffset += ERID_STEP_SIZE) {
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      ...
      if (mode) {
        SetNvmGnt2Erid (host, socket, ch, dimm, gnt2erid);
      } // if mode
      GetSetClkDelay (host, socket, ch, dimm + 2, GSM_FORCE_WRITE | GSM_UPDATE_CACHE, (INT16 *)&eridDelay);
...
*/
...
#ifdef  RC_SIM_FEEDBACK
      eridOffset2 = (eridOffset-(ch*8)+32)%256; // variation across channels
      trainRes[ch][dWord] |= ((RcSimFineEridFeedback[eridOffset2/32] >> (31 - (eridOffset2 % 32))) & 1) << (eridOffset % 32);
#else
      ...
      rcPrintf ((host, "     %d", (trainRes[ch][dWord] >> (eridOffset % 32)) & 1));
  ...
  for (ch = 0; ch < MAX_CH; ch++) {
    ...
    if (pulseWidth[ch] <= RE_EDGE_SIZE_THRESHOLD) {
      ...
      OutputWarning (host, WARN_EARLY_RID, WARN_EARLY_RID_UNCORR, socket, ch, NO_DIMM, NO_RANK);
    } //if pulseWidth
  } // ch loop
             ...
             )
{
  ...
  struct dimmNvram     (*dimmNvList)[MAX_DIMM];
  UINT16                eridDelay;
  INT16                 Gnt2eridOffset[GNT2ERID_RANGE] = {0, -1, -2, -3};
  UINT8                 gnt2eridIndex;
...
1.	Coarse ERID Training
  ...
  b.	This basic test will be run while testing different settings for the GNT2ERID setting in the host MC (one per slot). When the pattern
      comparison passes, the GNT2ERID is correct and coarse ERID training is complete.

2.	Final Fine RID Training
...
*/
  ...
  for (dimm = 0; dimm < MAX_DIMM; dimm++) {
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      ...
      SetNvmGnt2Erid(host, socket, ch, dimm, GNT2ERID_START);
  ...
  SetAepTrainingMode (host,socket, EARLY_RID_COARSE);
  //
  // Reset DDR IO
  //
  ...
  for (dimm = 0; dimm < MAX_DIMM; dimm++) {
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      ...
      SetupCoerseEridTraining (host, socket, ch, dimm);
    ...
    rcPrintf ((host, "Early RID Coarse search for socket:%d dimm:%d\nCycleOff   PI", socket, dimm));
    ...
    for (gnt2eridIndex = 0; gnt2eridIndex < GNT2ERID_RANGE; gnt2eridIndex++) {
      ...
      for (eridOffset = 0; eridOffset < 128; eridOffset += ERID_COARSE_STEP_SIZE) {
        ...
        rcPrintf ((host, "  %3d     %3d", Gnt2eridOffset[gnt2eridIndex], eridOffset));
        ...
        for (ch = 0; ch < MAX_CH; ch++) {
          ...
          SetNvmGnt2Erid (host, socket, ch, dimm, (*dimmNvList)[dimm].gnt2erid + Gnt2eridOffset[gnt2eridIndex]);
          GetSetClkDelay (host, socket, ch, dimm+2, GSM_FORCE_WRITE | GSM_UPDATE_CACHE, (INT16 *)&eridOffset);     // CLK 2 for DIMM0 or CLK 3 for DIMM1
        } //ch loop
        ...
        for (ch = 0; ch < MAX_CH; ch++) {
          ...
          rcPrintf ((host, "     %d", gnt2eridPassed[ch]));
        } //ch loop
        rcPrintf ((host, "\n"));
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      // skip non AEP or unpopulated channels
      if ((chBitMask & (1 << ch)) == 0) continue;
      
      // error message for failing channel
      if (gnt2eridPassed[ch] == 0) {
        DisableChannel(host, socket, ch);
        OutputWarning(host, WARN_EARLY_RID, WARN_EARLY_RID_CYCLE_FAIL, socket, ch, dimm, rank);
      }
    } // ch loop     
  } //dimm loop
  ...
  for (dimm = 0; dimm < MAX_DIMM; dimm++) {
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      ...
      SetNvmGnt2Erid (host, socket, ch, dimm,  (*dimmNvList)[dimm].gnt2erid);
      GetSetClkDelay (host, socket, ch, dimm+2, GSM_FORCE_WRITE | GSM_UPDATE_CACHE, (INT16 *)&eridDelay);
  ...
  if (checkMsgLevel(host, SDBG_MAX)) {
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      ...
      for (dimm = 0; dimm < MAX_DIMM; dimm++) {
        ...
        GetSetClkDelay (host, socket, ch, dimm + 2, GSM_READ_ONLY, (INT16 *)&eridDelay);
