...
//    host->bdat.socketList[socket].recvenStep  = (UINT8) 33;
 ...
 //   host->bdat.socketList[socket].wrLevStep  = (UINT8) 33;
    ...
    for (ch = 0; ch < MAX_CH; ch++) {
      ...
      for (dimm = 0; dimm < MAX_DIMM; dimm++) {
        ...
        for (rank = 0; rank < host->var.mem.socket[socket].maxRankDimm; rank++) {
          ...
          //
          GetSetClkDelay (host, socket, ch, (*rankList)[rank].ckIndex, GSM_READ_ONLY, &clkDelay);
          (*rankBDAT)[rank].rankTraining.clkDelay = (UINT16)clkDelay;

          ctlMinVal = 256;
          ctlMaxVal = 0;
          GetSetCtlGroupDelay (host, socket, ch, (*rankList)[rank].ctlIndex + CtlGrp0, GSM_READ_ONLY, &piDelay, &ctlMinVal, &ctlMaxVal);
          (*rankBDAT)[rank].rankTraining.ctlDelay = ctlMinVal;

          for (cg = CmdGrp2; cg >= CmdGrp0; cg--) {
            cmdMinVal = 256;
            GetSetCmdGroupDelay (host, socket, ch, cg, GSM_READ_ONLY, &piDelay, &cmdMinVal, &cmdMaxVal);
            (*rankBDAT)[rank].rankTraining.cmdDelay[cg - CmdGrp0] = cmdMinVal;
          } // cg loop

          (*rankBDAT)[rank].rankTraining.ioLatency = GetIOLatency(host, socket, ch, logRank);
          (*rankBDAT)[rank].rankTraining.roundtrip = GetRoundTrip(host, socket, ch, logRank);
          ...
          for (strobe = 0; strobe < MAX_STROBE; strobe++) {
            ...
            GetSetDataGroup (host, socket, ch, dimm, rank, strobe, 0, DdrLevel, RxDqsDelay, GSM_READ_ONLY,
            ...
            GetSetDataGroup (host, socket, ch, dimm, rank, strobe, 0, DdrLevel, TxVref, GSM_READ_ONLY,
    ...
    //Memory Schema 4b
    host->bdat.refCodeRevision = RC_REVISION;  //Save reference code revision
    ...
    //RMT Schema5
    host->bdatRmt.refCodeRevision  = RC_REVISION;  //Save reference code revision
