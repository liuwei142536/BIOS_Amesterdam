              ...
              )
{
  ...
  for (profile = 0; profile < 2; profile++) {
    ...
    //
    if (host->nvram.mem.profileMemTime[profile].ddrFreq >= 3200) {
    ...
    } else if (host->nvram.mem.profileMemTime[profile].ddrFreq >= 2667) {
      host->nvram.mem.profileMemTime[profile].ddrFreqLimit = DDR_2667 + 1;
    ...
    } else if (host->nvram.mem.profileMemTime[profile].ddrFreq >= 1867) {
      host->nvram.mem.profileMemTime[profile].ddrFreqLimit = DDR_1867 + 1;
      ...
      host->nvram.mem.profileMemTime[profile].ddrFreqLimit = DDR_1067 + 1;
      ...
      for (ch = 0; ch < MAX_CH; ch++) {
        ...
        for (dimm = 0; dimm < (*channelNvList)[ch].maxDimm; dimm++) {
          ...
          if (tempTCK <= DDR_3200_TCK_MIN) {
            ...
            tempFreq = 2800;
          } else if (tempTCK <= DDR_2667_TCK_MIN) {
            tempFreq = 2667;
            ...
            tempFreq = 2000;
          } else if (tempTCK <= DDR_1867_TCK_MIN) {
            tempFreq = 2133;
