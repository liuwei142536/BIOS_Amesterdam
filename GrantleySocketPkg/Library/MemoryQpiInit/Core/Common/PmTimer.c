            ...
            )
{
...
#else
  base    = ReadPciCfg (host, 0, REG_ADDR (LPC_BUS, LPC_DEVICE, LPC_FUNC, REG_ACPI_BASE_ADDRESS, sizeof (UINT16)));
  enable  = ReadPciCfg (host, 0, REG_ADDR (LPC_BUS, LPC_DEVICE, LPC_FUNC, PM_ENABLE, sizeof (UINT8)));
#endif //PCH_SPT
  ...
  //
    ...
    host->var.common.pmBase = ICH_PMBASE_ADDR;
#ifdef PCH_SPT
    WritePciCfg (
                host,
                0,
    ...
    WritePciCfg (
                host,
                0,
                REG_ADDR (LPC_BUS, LPC_DEVICE, LPC_FUNC, REG_ACPI_BASE_ADDRESS, sizeof (UINT16)),
                host->var.common.pmBase | BIT0
                );
    WritePciCfg (host, 0, REG_ADDR (LPC_BUS, LPC_DEVICE, LPC_FUNC, PM_ENABLE, sizeof (UINT8)), (UINT8) enable | BIT7);
#endif //PCH_SPT
  }
}
           ...
           )
{
  ...
  ReadTsc64 (&endTsc);
#if defined(YAM_ENV) || defined(SLE_FLAG)
  startCount = GetCount (host);
  while (GetDelay (host, startCount) < usDelay);
#else // YAM_ENV or SLE_FLAG
  while(TimeDiff(host, startTsc, endTsc, TDIFF_UNIT_US) < usDelay) {
    ReadTsc64 (&endTsc);
  };
#endif // YAM_ENV or SLE_FLAG
