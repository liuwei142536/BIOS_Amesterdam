...
)
{
    ...
    EFI_KEY_DATA        *KeyData = (EFI_KEY_DATA*)pKey;
    ...
    if(EFI_ERROR(Status)) {
        //Return the current KeyState, even when no KeyData is available in buffer.
        //As per UEFI Spec,"It should also be noted that certain input devices may not be able to produce shift or toggle
        //state information, and in those cases the high order bit in the respective Toggle and Shift state fields should not be active".
        //As Terminal driver do not report KeyShiftState and KeyToggleState, updating the KeyStates to Zero.
...
//**********************************************************************
//  Part 3.
//  The following code is implementation of standard SimpleTextIn functions
//**********************************************************************
...
)
{
    ...
    KEY_WAITING_RECORD *TerminalKeyIn = OUTTER(mTerminalKeyboardData.pHead,
    ...
    while ( TerminalKeyIn != NULL) {
		...
		//  If KeyData.Key, KeyData.KeyState.KeyToggleState and KeyData.KeyState.KeyShiftState are 0, then any
		// incomplete keystroke will trigger a notification of the KeyNotificationFunction
        if (TerminalKeyIn->Context.Key.ScanCode == 0
                && TerminalKeyIn->Context.Key.UnicodeChar == 0
                && TerminalKeyIn->Context.KeyState.KeyShiftState == 0
                && TerminalKeyIn->Context.KeyState.KeyToggleState == 0) {
        ...
        //
        if ((KeyNotify||(KeyScanCodeMatch && KeyUniCodeMatch && KeyShiftCodeMatch && KeyToggleKeyMatch)) && (!TerminalKeyIn->CallbackStatus)) {

            //Call the notification function
            TerminalKeyIn->CallbackStatus=TRUE;
            CallBackCalled=TRUE;
            TerminalKeyIn->Callback((EFI_KEY_DATA *)Key);
