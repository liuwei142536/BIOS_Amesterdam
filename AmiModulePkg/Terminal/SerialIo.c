...
INVALID_PCI_UART gInvalidPciUartList[] = {
    INVALID_PCI_UART_DEVICE_LIST
    {0xFFFF, 0xFFFF}
};
...
)
{
    ...
    if(!EFI_ERROR(GetSerialIo_DP(This, Controller, &acpiDP,
        ...
        while(gInvalidPciUartList[i].VendorId != 0xFFFF && 
                gInvalidPciUartList[i].DeviceId != 0xFFFF) {
            // Skip Unsupported PCI Uart device
            if(gInvalidPciUartList[i].VendorId == VendorId && 
                    gInvalidPciUartList[i].DeviceId == DeviceId ) {
                return EFI_UNSUPPORTED;
            }
            i++;
        }
        ...
        if ((RevisionId[3] == PCI_CL_COMM) &&
            ...
            for (i = 0; i < PCI_MAX_BAR_NO; i++) {
                ...
                if (Resources->ResType == ACPI_ADDRESS_SPACE_TYPE_IO) {
                    ...
                    break;
                } else if(Resources->ResType == ACPI_ADDRESS_SPACE_TYPE_MEM) {
            ...
            if(CommandReg != 0) {
                ...
                if (EFI_ERROR(Status)) {
                    DEBUG((DEBUG_ERROR, "%a PciIo enabling attributes failed Status :%r \n", __FUNCTION__,Status));
                } 
...
//------------------------------------
    ...
    if(!EFI_ERROR(Status)) {
        ...
        if (ResourceList.SmallHeader){
            for(SmlHdr=ResourceList.SmallHeader; (SmlHdr->Byte)!=ACPI_END_TAG_DESCRIPTOR; SmlHdr+=((sizeof(ACPI_SMALL_RESOURCE_HEADER))+SmlHdr->Bits.Length)) {
                switch(SmlHdr->Bits.Name) {
                    case ACPI_SMALL_FIXED_IO_PORT_DESCRIPTOR_NAME:
                        Parameters->BaseAddress = ((EFI_ACPI_FIXED_LOCATION_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddress;
                        break;
                    case ACPI_SMALL_IO_PORT_DESCRIPTOR_NAME: 
                        Parameters->BaseAddress = ((EFI_ACPI_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddressMin;
                        break;
                }
             }
         }
    ...
    if(!EFI_ERROR(Status)) {
        ...
        if (ResList != NULL){
            for(i = 0; i < ResList->ItemCount; i++) {
                ...
                SmlHdr = (ACPI_SMALL_RESOURCE_HEADER*)ResList->Items[i];
                switch(SmlHdr->Bits.Name){
                    case ACPI_SMALL_FIXED_IO_PORT_DESCRIPTOR_NAME: 
                        Parameters->BaseAddress = ((EFI_ACPI_FIXED_LOCATION_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddress;
                        break;
...
SerialTest:
    ...
    if (RemainingDevicePath) {
        ...
        UartDevicePath->Parity = (UINT8)Parameters->SerialMode.Parity;
        UartDevicePath->DataBits = (UINT8)Parameters->SerialMode.DataBits;
        UartDevicePath->StopBits = (UINT8)Parameters->SerialMode.StopBits;
    }
    //
...
)
{
    if ((Parameters->UartType < UART16550A) || (NewFifoDepth == 1)) {
        SerialWritePort(Parameters, SERIAL_REGISTER_FCR,(UINT8) ~TRFIFOE);
    } else {
