...
#if defined (VirtualSerial_SUPPORT) &&(VirtualSerial_SUPPORT == 1)
...
)
{
    ...
    *IsPciSerialInterface = FALSE; 
    ...
    if ((AcpiPrevDPNodePtr!=NULL) &&
        (AcpiPrevDPNodePtr->Header.Type == ACPI_DEVICE_PATH) &&
        (AcpiPrevDPNodePtr->Header.SubType == ACPI_DP) &&
        (AcpiPrevDPNodePtr->HID == EISA_PNP_ID(0x501)))  {
        *ComPort=SetSerialPortsEnabledVar_Sio((UINT8)AcpiPrevDPNodePtr->UID);
...
)
{
    ...
    UINTN                            i=0;
#endif
    ACPI_SMALL_RESOURCE_HEADER       *SmlHdr = NULL;
    ACPI_LARGE_RESOURCE_HEADER       *LarHdr = NULL;
    UINT8                            IrqNo = 0;
    UINT16                           IrqMask = 0;
    EFI_DEVICE_PATH_PROTOCOL         *DevicePathProtocol = NULL;
    EFI_HANDLE                       Handle=0;
    ...
    UINT8                            AmiSioVersion = 0;
#if defined (VirtualSerial_SUPPORT) && (VirtualSerial_SUPPORT == 1)
    VIRTUAL_DEV_DATA                 *VirtualSerialData = NULL;
#endif
    BOOLEAN                          IsVirtualSerialInstance  = FALSE; 
    ...
    if (ResourceList.SmallHeader != NULL){
        SmlHdr = ResourceList.SmallHeader;
        do{
            if ( SmlHdr->Bits.Type == ACPI_SMALL_ITEM_FLAG) {
                switch(SmlHdr->Bits.Name) {
                case ACPI_SMALL_FIXED_IO_PORT_DESCRIPTOR_NAME:
                    SpcrInfo->BaseAddress = ((EFI_ACPI_FIXED_LOCATION_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddress;
                    break;
                case ACPI_SMALL_IO_PORT_DESCRIPTOR_NAME:
                    SpcrInfo->BaseAddress=((EFI_ACPI_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddressMin;
                    break;
                case ACPI_SMALL_IRQ_DESCRIPTOR_NAME:
                    IrqMask = ((EFI_ACPI_IRQ_DESCRIPTOR*)SmlHdr)->Mask;                   
                    SpcrInfo->Irq = (UINT8)IrqMask;
                   
#if defined (VirtualSerial_SUPPORT) &&(VirtualSerial_SUPPORT == 1)
                    VirtualSerialData = BASE_CR(EfiSioProtocol,VIRTUAL_DEV_DATA,EfiSio);
                    if (VirtualSerialData->Signature ==  VIRTUAL_SERIAL_SIG) {
                        IsVirtualSerialInstance = TRUE; 
                    }
#endif

#if  defined(AMI_SIO_MINOR_VERSION) &&(AMI_SIO_MINOR_VERSION != 0)
                    AmiSioVersion = AMI_SIO_MINOR_VERSION;
#endif
                    if ( (AmiSioVersion >= 12 )|| IsVirtualSerialInstance) {
                        
                        // As per ACPI Spec v6.2, under Section 6.4.2.1 IRQ value can be 0 to 15
                        // From the value of Mask ,Bit [0] represents IRQ0, Bit[1] is IRQ1, and so on. 
                ...
                SmlHdr += ((sizeof(ACPI_SMALL_RESOURCE_HEADER)) + SmlHdr->Bits.Length);
            } else {
                LarHdr = (ACPI_LARGE_RESOURCE_HEADER*)SmlHdr;
                switch(LarHdr->Header.Bits.Name) {
                case ACPI_LARGE_EXTENDED_IRQ_DESCRIPTOR_NAME:
                    
                    // Refer ACPI Spec v6.2, under Section 6.4.3.6 Extended Interrupt 
                    IrqNo = (UINT8)(((EFI_ACPI_EXTENDED_INTERRUPT_DESCRIPTOR*)LarHdr)->InterruptNumber[0]);
                    SpcrInfo->Irq = IrqNo;
                    break;
                }
                SmlHdr += ((sizeof(ACPI_LARGE_RESOURCE_HEADER))+ LarHdr->Length);
            }
        }while((SmlHdr->Byte)!=ACPI_END_TAG_DESCRIPTOR);
    }
#else
    ...
    if(ResourcesList != NULL){
        for(i=0; i<ResourcesList->ItemCount; i++){
            SmlHdr = (ACPI_SMALL_RESOURCE_HEADER*)ResourcesList->Items[i];
            if (SmlHdr->Bits.Type == ACPI_SMALL_ITEM_FLAG) {
                switch(SmlHdr->Bits.Name) {
                case ACPI_SMALL_FIXED_IO_PORT_DESCRIPTOR_NAME:
                    SpcrInfo->BaseAddress=((EFI_ACPI_FIXED_LOCATION_IO_PORT_DESCRIPTOR*)SmlHdr)->BaseAddress;
...
#if defined (VirtualSerial_SUPPORT) &&(VirtualSerial_SUPPORT == 1)
                    VirtualSerialData = BASE_CR(AmiSioProtocol,VIRTUAL_DEV_DATA,AmiSio);
                    if (VirtualSerialData->Signature ==  VIRTUAL_SERIAL_SIG) {
                        IsVirtualSerialInstance = TRUE; 
                    }
#endif
                ...
                LarHdr = (ACPI_LARGE_RESOURCE_HEADER*)ResourcesList->Items[i];
                switch(LarHdr->Header.Bits.Name) {
                case ACPI_LARGE_EXTENDED_IRQ_DESCRIPTOR_NAME:
                    IrqNo = (UINT8)(((EFI_ACPI_EXTENDED_INTERRUPT_DESCRIPTOR*)LarHdr)->InterruptNumber[0]);
                    SpcrInfo->Irq = IrqNo;
    ...
    ASSERT_EFI_ERROR(Status);
        ...
        for (i = 0; i < PCI_MAX_BAR_NO; i++) {
            ...
            if (Resources->ResType == ACPI_ADDRESS_SPACE_TYPE_IO) {
                SpcrInfo->BaseAddress = Resources->AddrRangeMin;
                break;
            }
        }
...
)
{
    ...
    spcr_tbl->Header.CreatorId = PcdGet32 (PcdAcpiDefaultCreatorId);   //"AMI "
...
)
{
    ...
    if(AmiSerialDetected) {
        ...
        if (Port >= gTotalSioSerialPorts) {
            ...
            if (EFI_ERROR(Status)) {
                DEBUG((DEBUG_ERROR, "%a Updating PciSerial SpcrTable Values is failed, Status :%r \n", __FUNCTION__,Status));
            }
        } else {
            Status = SetComSuperIoSpcrTableValues(This, Controller);
            if (EFI_ERROR(Status)) {
                DEBUG((DEBUG_ERROR, "%a Updating ComSuperIo SpcrTable Values is failed, Status :%r \n", __FUNCTION__,Status));
            }
        }
    }
