...
)
{
    ...
    // Read memory over write request Nvram variable value  
    pRS->GetVariable(L"MemoryOverwriteRequestControl",
                     &MorGuid,
                     NULL,
                     &MorValueSize,
                     &gMorValue);
...
)
{
    ...
    if ((gMorValue & 01)!= 0) {
        Status = GetAndStoreSecurityProtocolInformation(
...
)
{
    ...
    if (!EFI_ERROR(Status)) {
        ...
        pBS->FreePool(SupportedSilosArrayBuffer);
    }
    pBS->FreePool(ProbeHostDataBuffer);
