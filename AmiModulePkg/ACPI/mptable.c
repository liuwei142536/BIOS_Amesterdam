...
static UINT8    *BusTypeString[] = {(UINT8*)"CBUS  ", (UINT8*)"CBUSII", (UINT8*)"EISA  ", (UINT8*)"FUTURE", (UINT8*)"INTERN", (UINT8*)"ISA   ",
                                    (UINT8*)"MBI   ", (UINT8*)"MBII  ", (UINT8*)"MCA   ", (UINT8*)"MPI   ", (UINT8*)"MPSA  ", (UINT8*)"NUBUS ",
                                    (UINT8*)"PCI   ", (UINT8*)"PCMCIA", (UINT8*)"TC    ", (UINT8*)"VL    ", (UINT8*)"VME   ", (UINT8*)"XPRESS" };
...
static VOID *LegacyRegionAddress = NULL;
...
VOID EFIAPI MpsTableReadyToBootNotify(
	IN EFI_EVENT Event, 
    IN VOID *Context)
{
    EFI_STATUS Status;
...
#if INSTALL_FLOATING_POINTER == 0
    //Install actual MP Table
    Status=pBS->InstallConfigurationTable(&gMpsTableGuid,MpsTablePointer); 
#else
    //Install Floating Pointer instead
    if((EFI_ERROR(MpsTableFloatPointerModifyStatus)) || (LegacyRegionAddress == NULL))
    {	// Floating Pointer was not copied to Legacy Region for some reason, so lets use structure address
    	LegacyRegionAddress = (VOID*)&MpsTableFloatingPointer;
    }
    //Install Floating Pointer
    Status=pBS->InstallConfigurationTable(&gMpsTableGuid,LegacyRegionAddress); 
#endif
    ASSERT_EFI_ERROR(Status);
...
//add CPU Entry
    ...
    if (!EFI_ERROR(Status))  {
        ...
        BaseTableEntryCount += (UINT16)EntryCount;
...
EFI_STATUS MpsTableAddHeader(VOID)
{
    ...
    HeaderPointer->BaseTableLength = (UINT16)BaseTableLength;
...
//first we detect IO APICS mapped into default address range
	for(SearchAddress = IO_APIC_BASE_ADDRESS_BOTTOM;
		...
		if(*((UINT8 *)(UINTN)SearchAddress) != 0xFF)             //we have something
...
//Check also SDL created Structures they might address very none standard IOAPICS.
//-------------------------------------------------------------------------------------------
#if USE_BOARD_INFO_APIC_DATA == 1
{
	//-----------------        
	for(i=0; i< gAmiBoardInfo2Protocol->ApicBrdData->IoApicCount; i++){
		...
		if(BrdApicInfo->ApicAddress != 0xDEADBEEF) // Skipping dummy DEADBEEF address on some boards.
    ...
    IN UINT8 BusAttributes)
{
    ...
    pPciRootBridgeIoProtocol->Configuration(pPciRootBridgeIoProtocol, (VOID**)&Resources);
    ...
    IN UINT32 BaseAddress OPTIONAL)
{
    ...
    if(BaseAddress == (UINT32)(UINTN)NULL)     //IO APIC visible as PCI device
    {
	    Status = pPciIoProtocol->Pci.Read(pPciIoProtocol, EfiPciIoWidthUint32,
