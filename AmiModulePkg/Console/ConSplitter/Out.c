...
/** @file 
    @brief File contains the Simple Text Output functionality for the Console Splitter Driver
...
/// Variables for storing the current graphics mode
UINT32 GraphicsMode = 0;
UINT32 PixelBuffer_HorRes = 0;
UINT32 PixelBuffer_VertRes = 0;
...
)
{
    ...
    // check to see if the text mode resolution size fits within the INT32 data type.  exit if it doesn't
    Offset = MultU64x32((UINT64)MasterMode.CursorRow, Columns);
    if ( Offset > 0x80000000 )
    {
        return EFI_INVALID_PARAMETER;
    }

    // Set Offset to the INT32 value left in Total 
    // and set the pointers to the correct place in ScreenBuffer and Attribute Buffer
    pChar =  ScreenBuffer + Offset + Col;
    pAttr =  AttributeBuffer + Offset + Col;
...
VOID RestoreUgaScreen(IN EFI_GRAPHICS_OUTPUT_PROTOCOL *Gop)
{
    ...
    if(PixelBuffer != NULL)
	{
        // Find the mode of the current GOP device that matches the resolution of the saved screen
        // if one isn't found, don't restore the screen, just clear it.
        if (Gop->Mode->MaxMode > 0)
        {
                
            while (Index < Gop->Mode->MaxMode)
            {
                UINTN Size = sizeof(EFI_GRAPHICS_OUTPUT_MODE_INFORMATION);
                Status = Gop->QueryMode(Gop, Index, &Size, &Info);
                if (!EFI_ERROR(Status))
                {
                    if ((Info->HorizontalResolution == PixelBuffer_HorRes) && (Info->VerticalResolution == PixelBuffer_VertRes))
                    {
                        ModeFound = TRUE;
                        DEBUG((DEBUG_INFO, "ConSplitter: RestoreUgaScreen: Mode of the graphics device found that matches the saves Pixel Buffer.\n\r"));
                        DEBUG((DEBUG_VERBOSE, "PixelBuffer_HorRes = %d \n\r", PixelBuffer_HorRes));
                        DEBUG((DEBUG_VERBOSE, "PixelBuffer_VertRes = %d \n\r", PixelBuffer_VertRes));
                        DEBUG((DEBUG_VERBOSE, "Index = %X \n\r", Index));
                        break;
                    }
                    pBS->FreePool(Info);
                    Info = NULL;
        ...
        PixelBuffer = NULL;
        PixelBuffer_HorRes = 0;
        PixelBuffer_VertRes = 0;
...
VOID SaveUgaScreen(IN EFI_GRAPHICS_OUTPUT_PROTOCOL *Gop)
{
    ...
    if (Gop->Mode->MaxMode != 0)
    {
        ...
        PixelBuffer_HorRes = Gop->Mode->Info->HorizontalResolution;
        PixelBuffer_VertRes = Gop->Mode->Info->VerticalResolution;
...
)
{
    ...
    // Check to see if this is the current mode. 
    // If it is, clear screen, set cursor to 0 and exit 
    if ( ModeNum == (UINTN)MasterMode.Mode)
    {
        CSClearScreen(This);
        return EFI_SUCCESS;
    } 
