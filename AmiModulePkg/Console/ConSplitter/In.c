...
/**
    This function registers key notifications when the standard in is lock due to CSM executions.
    
    
 */
EFI_STATUS RegisterKeyNotifications(
        IN  EFI_KEY_DATA                      *KeyData,
        IN  EFI_KEY_NOTIFY_FUNCTION           KeyNotificationFunction,
        OUT KEY_NOTIFY_LINK                   *NotifyLink)
{
    UINTN   ChildIndex = 0;
...
VOID CheckForCtrlAltDelNotification( 
        IN EFI_KEY_DATA *KeyData,
        IN EFI_KEY_NOTIFY_FUNCTION KeyNotificationFunction)
{
    UINT8 Idx;
    ...
    for (Idx = 0; Idx < 8; Idx++)
    {
        ...
        {
            if (KeyNotificationFunction != HandleCtrlAltDel)
            {
                CtrlAltDel_Override[Idx] = TRUE;
                DEBUG((DEBUG_ERROR, "Default ConIn Function overridden\n\r"));
...
)
{
	...
	while (SimpleIn != NULL)
	{
        if (SimpleIn->KeycodeInEx) {
            ...
            // Clear the Key Toggle member becasue SimpleTxtIn doesn't support this feature and 
            //  the value of this variable is invalid
            TempKey.KeyState.KeyToggleState = 0;  
        ...
        if (!EFI_ERROR(Status) && (TempKey.KeyState.KeyToggleState & TOGGLE_STATE_VALID)) {
            if ((TempKey.KeyState.KeyToggleState & ~KEY_STATE_EXPOSED ) != mCSToggleState) {
                // Keep the upper two bits from mCSToggleState and then set the scroll lock, caps lock and num lock
                //  as defined by the call to the physical SimpleTextInEx/AmiKeyCode device.
                mCSToggleState = ((mCSToggleState & (KEY_STATE_EXPOSED | TOGGLE_STATE_VALID)) 
                        | (TempKey.KeyState.KeyToggleState & (SCROLL_LOCK_ACTIVE | NUM_LOCK_ACTIVE | CAPS_LOCK_ACTIVE)));
                CSInSetState ( (EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL*) This, &mCSToggleState );
            }
        }
...
/**
    ...
    @retval EFI_DEVICE_ERROR input device not found
    @retval EFI_ACCESS_DENIED input device is busy - but keys notification is added to the global list if NotifyHandle is non NULL
**/
EFI_STATUS EFIAPI
CSInRegisterKeyNotify(
    IN  EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL *This,
...
)
{
    ...
    if (CurrentStdInStatus)
    {   
        // Need to add the notifications to the global list
        
        DEBUG((DEBUG_WARN, "ConIn Devices are blocked.  Registering the key notifications in the global list"));
        Status = pBS->AllocatePool(EfiBootServicesData, sizeof(KEY_NOTIFY_LINK), (VOID**)&NotifyLink);
        if(!EFI_ERROR(Status))
        {
            Status = RegisterKeyNotifications(KeyData, KeyNotificationFunction, NotifyLink);
    ...
    *NotifyHandle = (EFI_HANDLE) NotifyLink;
    ...
    // If another function registers a notification on CtrlAltDel, then disable the default one from Consplitter.
    CheckForCtrlAltDelNotification(KeyData, KeyNotificationFunction);
    ...
    if (ConInList.pHead != NULL)
    {
	    for(ChildIndex = 0; ChildIndex < ConInList.Size; ChildIndex++) {
	        ...
	        if(SimpleIn != NULL && SimpleIn->SimpleInEx != NULL && Handle != (EFI_HANDLE) UNUSED_NOTIFY_HANDLE) {
	        	SimpleIn->SimpleInEx->UnregisterKeyNotify(SimpleIn->SimpleInEx, Handle);
            }
        }
    }
