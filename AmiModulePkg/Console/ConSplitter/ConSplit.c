...
//**********************************************************************
/** @file 
    @brief This file contains the main control flow for the Console Splitter Driver.
...
EFI_KEY_TOGGLE_STATE mCSToggleState = TOGGLE_STATE_VALID | KEY_STATE_EXPOSED;
...
static BOOLEAN InitialMode = TRUE;
...
/// Array describing Control-Alt-Del key combinations
...
// array containing the overwritten state of the CTRL-ALT-DEL handler
BOOLEAN CtrlAltDel_Override[] = 
{
        FALSE,
        FALSE,
        FALSE,
...
**/
VOID EFIAPI ReportNoConOutError(IN EFI_EVENT Event, IN VOID *Context)
{
    ...
    for(Link = ConOutList.pHead; Link!=NULL; Link=Link->pNext)
    {
        ...
        if (EFI_ERROR(Status) && (GraphicsSplitter_SUPPORT == 1))
        {
            // If graphics splitter is enabled, then it will be a virtual device with no Device Path
            //  Check for gAmiGraphicsSplitterModeChangeProtocolGuid instead
            Status = pBS->HandleProtocol(SimpleOut->Handle, &gAmiGraphicsSplitterModeChangeProtocolGuid, (VOID**)&Mode);
        }
        if(!EFI_ERROR(Status))
            break;
    }
...
/**
    This function determines if this particular CtrlAltDel key combination has been overridden by 
    another registered Key Notification
 
    @param KeyData - pointer to the key information about the key that was pressed
    @return BOOLEAN
    @retval TRUE - The function has been overwritten
    @retval FALSE - The function has not been overwritten
 */
BOOLEAN HasCtrlAltDelBeenReplaced(IN EFI_KEY_DATA *KeyData)
{
    UINT8 Idx;
    
    for (Idx = 0; Idx < 8; Idx++)
    {
        if ((KeyData->Key.ScanCode == ControlAltDelKeys[Idx].Key.ScanCode) && (KeyData->Key.UnicodeChar == ControlAltDelKeys[Idx].Key.UnicodeChar) &&
                    (KeyData->KeyState.KeyShiftState  ==  ControlAltDelKeys[Idx].KeyState.KeyShiftState))
        {
            return CtrlAltDel_Override[Idx];
        }
    }
...
/**
    ...
    @param Context pointer to event context
**/
VOID EFIAPI UpdateSystemTableConIn(IN EFI_EVENT Event, IN VOID *Context)
{
    UINT32 CRC32 = 0;
    ...
    // handle already exists it is defined in the EntryPoint
...
)
{
    ...
    RegisterProtocolCallback(&gAmiConOutStartedProtocolGuid, ReportNoConOutError, NULL, &Event,&pRegistration);
    RegisterProtocolCallback(&gAmiConInStartedProtocolGuid, UpdateSystemTableConIn, NULL, &Event,&pRegistration);
...
{    
    EFI_ABSOLUTE_POINTER_PROTOCOL           *AbsolutePointer = NULL;
    EFI_SIMPLE_POINTER_PROTOCOL *SimplePointer = NULL;
    EFI_STATUS  SimpleStatus, AbsoluteStatus;

    SimpleStatus = pBS->OpenProtocol(ControllerHandle, &gEfiSimplePointerProtocolGuid,
                               (VOID**)&SimplePointer, This->DriverBindingHandle,
                               ControllerHandle, EFI_OPEN_PROTOCOL_BY_DRIVER);
    if (!EFI_ERROR(SimpleStatus)) {
        SimpleStatus = InstallSimplePointerDevice( SimplePointer, ControllerHandle );
        if (!EFI_ERROR(SimpleStatus)) {
            SimpleStatus = pBS->OpenProtocol(ControllerHandle, &gEfiSimplePointerProtocolGuid,
                                       (VOID**)&SimplePointer, This->DriverBindingHandle,
                                       ConSplitHandle, EFI_OPEN_PROTOCOL_BY_CHILD_CONTROLLER);
            ASSERT_EFI_ERROR(SimpleStatus);
        } else {
            pBS->CloseProtocol(ControllerHandle, &gEfiSimplePointerProtocolGuid,
                               This->DriverBindingHandle, ControllerHandle);
            if (SimpleStatus == EFI_OUT_OF_RESOURCES)
                return SimpleStatus;
        }
    }
    
    AbsoluteStatus = pBS->OpenProtocol(ControllerHandle, &gEfiAbsolutePointerProtocolGuid,
                               (VOID**)&AbsolutePointer, This->DriverBindingHandle,
                               ControllerHandle, EFI_OPEN_PROTOCOL_BY_DRIVER);
    if (!EFI_ERROR(AbsoluteStatus)) {
        AbsoluteStatus = InstallAbsolutePointerDevice( AbsolutePointer, ControllerHandle );
        if (!EFI_ERROR(AbsoluteStatus)) {
            AbsoluteStatus = pBS->OpenProtocol(ControllerHandle, &gEfiAbsolutePointerProtocolGuid,
                                       (VOID**)&AbsolutePointer, This->DriverBindingHandle,
                                       ConSplitHandle, EFI_OPEN_PROTOCOL_BY_CHILD_CONTROLLER);
            ASSERT_EFI_ERROR(AbsoluteStatus);
        } else {
            pBS->CloseProtocol(ControllerHandle, &gEfiAbsolutePointerProtocolGuid,
                               This->DriverBindingHandle, ControllerHandle);
            if (AbsoluteStatus == EFI_OUT_OF_RESOURCES)
                return AbsoluteStatus;
        }
    }

    // If no devices were installed, then Houston we have a problem
    if ( EFI_ERROR(SimpleStatus) && EFI_ERROR(AbsoluteStatus) )
        return EFI_UNSUPPORTED;
...
VOID UpdateConVar(IN EFI_HANDLE Controller, IN CHAR16 *sVarName)
{
    ...
    if(!EFI_ERROR(Status))
    {
        Status = GetEfiVariable(sVarName, &gEfiGlobalVariableGuid, &Attributes, &DataSize, (VOID**)&pConDev);
        if (EFI_ERROR(Status))
        {
            ...
            pRS->SetVariable(sVarName, &gEfiGlobalVariableGuid,
        ...
        if (!DPIsOneOf(pConDev, pDevicePath, FALSE))
        {
            ...
            pRS->SetVariable(sVarName, &gEfiGlobalVariableGuid, Attributes, DataSize, pNewConDev);
...
)
{
    ...
    // The SupportedModes array is initialized in the entrypoint and so this is a valid reference
    // even when all ConOut devices are removed, this structure should still have a valid mode.
    // These are used to test if the current mode changes when we create the Supported modes table 
    // for the new device
    OldRows = SupportedModes[MasterMode.Mode].Rows;
    OldColumns = SupportedModes[MasterMode.Mode].Columns;

    //The SupportedModes structure was already been initialized in UpdateSystemTableConOut.
    ...
    for(i = 0; i < MaxMode; i++)
    {
        Status = This->QueryMode(This, i, &Cols, &Rows);
        SupportedModes[i].Rows = (INT32)Rows;
        SupportedModes[i].Columns = (INT32)Cols;
    ...
    if (InitialMode || (OldRows != SupportedModes[MasterMode.Mode].Rows) || (OldColumns != SupportedModes[MasterMode.Mode].Columns))
    {    
        InitialMode = FALSE;
    ...
    for(i = 0; i < This->Mode->MaxMode; i++)
    {
        ...
        if(!EFI_ERROR(Status))
        {
            ...
            if( SupportedModes[CurrentSystemMode].Rows == (INT32)Rows && 
                SupportedModes[CurrentSystemMode].Columns == (INT32)Cols)
            {
...
VOID AdjustSupportedModes(VOID)
{
    ...
    for(i = 0; i < MasterMode.MaxMode; i++) {
        Status = SimpleOut->SimpleOut->QueryMode(SimpleOut->SimpleOut, i, &Cols, &Rows);
        SupportedModes[i].Rows = (INT32)Rows;
        SupportedModes[i].Columns = (INT32)Cols;
        SupportedModes[i].AllDevices = EFI_ERROR(Status) ? FALSE : TRUE;
    }
    ...
    IN OUT  AMI_EFI_KEY_DATA                *KeyData)
{
    ...
    if( (KeyData->KeyState.KeyToggleState & EFI_NUM_LOCK_ACTIVE) != 0 )
    {
        NumLockState = TRUE;
    }
...
/**
    ...
    registered with any new Simple Text In Ex protocols. This function accomplishes
...
EFI_STATUS KeyNotifyAddChild(IN CON_SPLIT_IN *Child)
{
    ...
    {
        DEBUG((DEBUG_ERROR, "You have exceeded the number of ConIn devices tracked for Key notifications. Increase MAX_CON_IN_CHILDREN in Consplit.h \n\r"));
        return EFI_OUT_OF_RESOURCES;
    }
