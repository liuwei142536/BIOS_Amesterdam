...
//**********************************************************************
/** @file
  @brief This file contains the Graphics console driver that produces the Simple Text Out interface for the GOP protocol
  
  This file translates between a SimpleTextOut protocol and makes calls to the GOP protocol that it attaches to.
  The Simple Text Out is in turn controlled by the ConsoleSplitter.  
...
extern UINT8 UsStdNarrowGlyphData[];
	...
	for(i=0;i<Number; i++)
	{
		FontStatus=pBS->HandleProtocol(pHandle[i], &gEfiFirmwareVolume2ProtocolGuid, (VOID**)&pFV);
		if (EFI_ERROR(FontStatus)) continue;
		DataSize=0;
		//NOTE: Only one section per FFS file is supported
		FontStatus=pFV->ReadSection (
			pFV,&FontFfsFileGuid,
			EFI_SECTION_FREEFORM_SUBTYPE_GUID,0, (VOID**)&pSectionGuid, &DataSize,
			&Authentication
	 	);
...
)
{
...
#if defined(GraphicsSplitter_SUPPORT) && (GraphicsSplitter_SUPPORT == 0)
	Status = pBS->OpenProtocol(
                        ControllerHandle, 
	...
	Status = pBS->LocateProtocol ( &gEfiHiiFontProtocolGuid, NULL, (VOID**)&HiiFont);
...
)
{
	...
	Status = pBS->AllocatePool(EfiBootServicesData, sizeof(GC_DATA), (VOID**)&GcData);
	if (EFI_ERROR(Status))
        return Status;
	...
	Status = pBS->AllocatePool(EfiBootServicesData, 
                               ...
                               (VOID**)&(GcData->SimpleTextOut.Mode));
    ...
    if (EFI_ERROR(Status))
    {
        GcData->VirtualGopDriver = TRUE;
    }
    else 
    {
        GcData->VirtualGopDriver = FALSE;
    }
    
	// Find the Hii Protocol and attach it to the datastructure
	Status = pBS->LocateProtocol ( &gEfiHiiFontProtocolGuid, NULL, (VOID**)&(GcData->HiiFont));
   ...
   (GcData->SimpleTextOut.Mode)->CursorVisible = FALSE;    //since initial position of window is undefined we cannot draw cursor yet
    ...
    DEBUG((DEBUG_VERBOSE, " Graphics Console: DriverBindingStart: SetMode Call    Mode = %d\n\r", StartMode));
...
)
{
    ...
    EFI_TPL                 OldTpl;
       ...
       DEBUG((DEBUG_INFO, "GCOutputString : enter \n\r"));
	...
	while (*String != 0)
	{
		switch (*String)
		{
			...
			case LINE_FEED:
			    FlushString(GcData);
                if((GcData->SimpleTextOut.Mode)->CursorRow == (INT32)(GcData->MaxRows - 1))
                {
                    ScrollUp(GcData);
...
)
{
    ...
    DEBUG((DEBUG_INFO, "GCSetMode : enter \n\r"));

    DEBUG((DEBUG_INFO, " GraphicsConsole:  Enter SetMode ModeNum = %X\n\r", ModeNum));
    ...
    if(GcData->GraphicsOutput->Mode->Mode != GcData->SupportedModes[i].GraphicsMode)
    {
        ...
        // Check to make sure that the current graphics mode is valid.  If so, clear before setting, otherwise skip it
        if (GcData->GraphicsOutput->Mode->Mode != -1)
        {
            if (GcData->GraphicsOutput->Mode->Mode < GcData->GraphicsOutput->Mode->MaxMode )
            {
                DEBUG((DEBUG_VERBOSE, "GraphicsConsole: GCSetMOde: Clear screen first\n\r"));
                This->ClearScreen(This);
            }
        }

        DEBUG((DEBUG_INFO, "GraphicsConsole: GCSetMOde: Current graphics mode of the device doesn't match the requested mode\n\r"));
    ...
    {    // Check to make sure that the current graphics mode is valid.  If so, clear before setting, otherwise skip it
        if (GcData->GraphicsOutput->Mode->Mode < GcData->GraphicsOutput->Mode->MaxMode )
        {
            DEBUG((DEBUG_VERBOSE, "GcData->GraphicsOutput->Mode->Mode < GcData->GraphicsOutput->Mode->MaxMode = %d < %d\n\r", 
                    GcData->GraphicsOutput->Mode->Mode, GcData->GraphicsOutput->Mode->MaxMode ));
            if ( GcData->SimpleTextOut.Mode->Mode != -1)
            {
                DEBUG((DEBUG_INFO, "GraphicsConsole: GCSetMode:  Call to ClearScreen\n\r"));
    ...
    if(TextBuffer == NULL || TextBufferSize < GcData->MaxColumns) {
        ...
        Status = pBS->AllocatePool(EfiBootServicesData, (GcData->MaxColumns + 1) * sizeof(CHAR16), (VOID**)&TextBuffer);
        if(EFI_ERROR(Status))
            return Status;
...
)
{
    ...
    DEBUG((DEBUG_INFO, " GraphicsConsole:  Enter ClearScreen\n\r"));
	GcData = (GC_DATA *) This;

	DEBUG((DEBUG_INFO, "GraphicsConsole: Clear Screen : (GcData->SimpleTextOut.Mode)->Mode = %d\n\r", (GcData->SimpleTextOut.Mode)->Mode));
	while( i < MaxTextMode && (GcData->SupportedModes[i].ModeNum != (GcData->SimpleTextOut.Mode)->Mode))
	{
	    DEBUG((DEBUG_INFO, "GraphicsConsole: Clear Screen : GcData->SupportedModes[i].ModeNum = %d   \n\r", GcData->SupportedModes[i].ModeNum));
	    i++;
	}
    DEBUG((DEBUG_VERBOSE, "i = %d   GcData->GraphicsOutput->Mode->Mode = %d\n\r", i, GcData->GraphicsOutput->Mode->Mode));
	
	
	if (( i < MaxTextMode && (GcData->GraphicsOutput->Mode->Mode != GcData->SupportedModes[i].GraphicsMode)) || (GcData->VirtualGopDriver == TRUE))
	{
        DEBUG((DEBUG_VERBOSE, "GraphicsConsole: Clear Screen : Set Mode call Index = %d   Mode = %d\n\r", i, GcData->SupportedModes[i].GraphicsMode));
		...
		{ 
            if(HorRes == MAX_RES || VerRes == MAX_RES )
            {
                if( MultU64x32(pInfo->HorizontalResolution, pInfo->VerticalResolution) > MultU64x32(MaxHorizontalResolution, MaxVerticalResolution))
                {
                    ...
                    MaxModeNumber = i;
                    pBS->FreePool(pInfo); //free memory from QueryMode
                }
                continue;
            }
...
#if (EFI_DEBUG)
VOID PrintModeTable(IN GC_DATA *GcData)
{
    UINTN i = 0;
    
    DEBUG((DEBUG_VERBOSE, "\n\r Mode#  Text Col  Text Row  Vid Col  Vid Row  Supported   GraphicsMode\n\r"));
    for(i = 0; i < GcData->SimpleTextOut.Mode->MaxMode; i++) 
    {
        DEBUG((DEBUG_VERBOSE, " %5d %9d %9d %7d %7d %10d %15d\n\r", 
                GcData->SupportedModes[i].ModeNum, 
                GcData->SupportedModes[i].Col, GcData->SupportedModes[i].Row, 
                GcData->SupportedModes[i].VideoCol, GcData->SupportedModes[i].VideoRow,
                GcData->SupportedModes[i].Supported,
                GcData->SupportedModes[i].GraphicsMode ));
    }
}
#endif
