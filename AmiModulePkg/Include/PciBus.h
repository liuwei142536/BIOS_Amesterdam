...
//Need to store some more information for device capabilities
...
//Only this attributes make sense for the BAR 
...
/////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------
//PCI Bus Driver Types Definitions
//---------------------------------------------------------------------------
...
/**
 ...
 Fields:
   ...
   ------------------------------------------------------------------
    ...
    stDisableAll    ENUM                Disable all possible decoding
...
/**
 ...
 Fields:
   ...
   ------------------------------------------------------------------
    ...
    rcNarrow        ENUM        Device doesn't supports "wide" resources. So if Device has any Io32 or Mmio64, it must be addressed as Io16 and Mmio32 resources correspondingly. 
...
/**
    ...
    @note  Don't change the order! Some PCI Bus internal routines rely on it!
...
/**
 ...
 Fields:        
   ...
   ------------------------------------------------------------------
    ...
    tBarBus         ENUM    Virtual BAR type to use the same mechanism to claim BUS resources for the device used in bridge specific resource routines
...
/**
 ...
 Fields:        
   ...
   ------------------------------------------------------------------
    ...
    VendorId        UINT16            Vendor ID of the PCI Device (PCI CFG OFFSET 0x00)
...
/**
 ...
 Fields:        
    ...
    *OverrideHandles EFI_HANDLE          Pointer    to the EFI_HANDLEs Buffer gets overridden by the    instance of EFI_BUS_SPECIFIC_DRIVER_OVERRIDE_PROTOCOL above.
...
/**
 ...
 Fields:        
    Name            Type                    Description
   ------------------------------------------------------------------
    VcResOffset     UINT16        Offset in PCI Express Extended Configuration Space of the Virtual Channel Resources header.
...
/**
 ...
 Fields:        
    Name            Type                    Description
   ------------------------------------------------------------------
    VcResOffset    UINT16                  Offset in extended PCI Express Extended Configuration Space of the Virtual Channel Capability header
    Reserved       UINT8                   2 Reserved bytes to make structure 4 byte aligned
...
typedef struct _PCIE_DATA {
    ...
    UINT8               CurrLnkSpeed;   /// encoding changed from previous version 8GT/s speed added so name as well  
    UINT8               CurrLnkWidth;
    PCIE2_DATA          *Pcie2; 
    PCIE3_DATA          *Pcie3;
...
/**
    ...
    It holds all device specific data and references all other data structures defined in "PciBus.h".
 ...
 Fields:        
    ...
    *HostData      PCI_HOST_INFO                    Pointer to This Device's Host Bridge private data.
    ...
    Started        BOOLEAN                          Flag to indicate that device has been Registered with HandleDatabase and has its EFI_PCI_IO_PROTOCOL installed. Device is READY TO USE
    ...
    Discovered     BOOLEAN                          Flag to indicate that resource requested information of This Device has been gathered and recorded.
    Discovered     BOOLEAN                          Flag to indicate that This Device was enumerated and it's Address; ClassCode; Type and DevID/VendId data recorded.
    ...
    Class          PCI_DEV_CLASS                    This Device RevisionId Class, Sub Class and ProgrInterface Code. 
    ...
    *HotPlug       PCI_HPC_INFO                     If This Device is a Hot Plug Controller or the bridge behind which Hot Plug Slots located this field will have a Pointer to a member of PCI_HOST_INFO.HpcList[?].
    Incompatible   BOOLEAN                          Flag to indicate that This Device's VendorID DeviceID was found in Bad Pci Device Table.
    RbError        BOOLEAN                          Flag to indicate that Root Bridge Access failed during initialization.
    RomBarError    BOOLEAN                          Flag to indicate that Option ROM BAR implemented but Invalid.
    DebugPort      BOOLEAN                          Flag to indicate that This Device is reserved as a Debug Port.
**/
...
//-- Device Specific Info
    ...
    PCI_DEV_CLASS           Class;        ///Device Class, Sub Class and Programming Interface Code 
...
///and signify Function 0 since with PCIe some programming applicable for F0 only.
    ...
    /// following fields holds all other functions of the device.
    UINTN                   FuncInitCnt;
...
/**
 ...
 Fields:        
   ...
   ------------------------------------------------------------------
    ...
    ChildCount   UINTN                Number of PCI Devices discovered on Bridge's secondary interface.
    **ChildList  PCI_DEV_INFO         Pointer to the array of pointers on Child Devices Private Data.
    Padded       BOOLEAN              Indicates that resources were Padded for that Bridge.
**/
...
/**
    Root Structure of the PCI Subsystem. 

 Fields:        
    Name            Type                    Description
   ------------------------------------------------------------------
    *VgaDev          PCI_DEV_INFO    Pointer on currently selected Legacy VGA device.
    *PriIde          PCI_DEV_INFO    Pointer on currently selected Legacy Primary IDE device.
    *SecIde          PCI_DEV_INFO    Pointer on currently selected Legacy Secondary IDE device.
...
//------------------------------------------------------------------------
    ...
    BOOLEAN                                             EnumerationCompleteFlag; // Indicates Pci Enumeration Complete
