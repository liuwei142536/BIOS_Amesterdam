...
{
    {
        ...
        (EFI_PEIM_NOTIFY_ENTRY_POINT)ReportFV2PeiAfterMem
    }
};
...
{
    {
        ...
        (EFI_PEIM_NOTIFY_ENTRY_POINT)ReportFv2DxeWrapper
    }
};
    ...
    IN UINT32 FvType)
{
    ...
    if(Result != FALSE)
    {
        ...
        for(i = 0; LegacyProcessFvBeforePublishingList[i] != NULL && Result; i++)
        {
            ...
            if((UINT32)LegacyRomArea.Type != Area->Type)
...
/**
    This function validates the Alignment of the FV.  If not properly it tries to allocate a new aligned buffer and 
    copies the FV into it
    
   @param PeiServices Pointer to the Pointer to the Pei Services Table
   @param Area Pointer to the AMI_ROM_AREA that is requested to be published
   
   @return BOOLEAN
   @retval TRUE - buffer is is properly aligned
   @retval FALSE - correct Alignment of FV failed
 */
BOOLEAN
VerifyFvAlignment( 
    IN CONST EFI_PEI_SERVICES **PeiServices, 
    IN OUT AMI_ROM_AREA *Area)
{
    ...
    // get the address of the FV.  The AMI_ROM_AREA member Address, is the in memory address of the FV.  This is the 
    //  location where we can find the FV header and get values like alignment.
    FvHeader = (EFI_FIRMWARE_VOLUME_HEADER *)(UINTN)Area->Address;
    ...
    {
        DEBUG((DEBUG_INFO, "VerifyFvAlignment: FV is not aligned, allocating aligned buffer \n\r"));

        //  Alignment is incorrect.  Allocate aligned memory area
        FvLength    = ReadUnaligned64 (&FvHeader->FvLength);
        NewFvBuffer = AllocateAlignedPages (EFI_SIZE_TO_PAGES ((UINT32) FvLength), FvAlignment);
        if (NewFvBuffer == NULL) 
        {
            DEBUG((DEBUG_ERROR, "VerifyFvAlignment:  ERROR: New buffer allocation failed!!\n\r"));
            //  Couldn't allocated aligned memory.  return error
            return FALSE;
        }
        CopyMem (NewFvBuffer, FvHeader, (UINTN) FvLength);

        // now update the pointer in the Area structure
        Area->Address = (UINT64)(UINTN)NewFvBuffer;
        DEBUG((DEBUG_INFO, "VerifyFvAlignment: Allocation successful.\n\r"));
    }
...
EFI_STATUS PublishFv(IN CONST EFI_PEI_SERVICES **PeiServices, IN AMI_ROM_AREA *Area)
{
    ...
    if (VerifyFvAlignment( PeiServices, Area) == FALSE)
        return EFI_VOLUME_CORRUPTED;
