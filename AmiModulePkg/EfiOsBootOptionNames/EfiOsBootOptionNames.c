...
)
{
    ...
    NumberOfCharacters = UnicodeSPrint(Name, NameSize, L"%S", ControllerName);
...
)
{
    ...
    for ( i = 0; i < gBootOptionCount; i++ )
    {
        ...
        if (!bootData || !bootData->DevicePath) continue;

        for (Dp = bootData->DevicePath; !isEndNode(Dp); Dp = NEXT_NODE(Dp))
        {
            if ( Dp->Type == MEDIA_DEVICE_PATH
        ...
        UnicodeSPrint(BootStr, sizeof(BootStr), L"Boot%04X", bootData->Option);

        OptionSize = 0;
        LoadOption = NULL;
        Status = GetEfiVariable(BootStr, &EfiGlobalVariableGuid, NULL, &OptionSize, (VOID**)&LoadOption);
        if (EFI_ERROR(Status)) continue;

        BootOptionName = (CHAR16*)(LoadOption+1);
