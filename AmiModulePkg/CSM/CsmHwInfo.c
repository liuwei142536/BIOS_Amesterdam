...
)
{
    ...
    for (i = 0; i < HandlesNo; i++) {
        ...
        Status = pBS->HandleProtocol (
            (*DiskInfoHandles)[i],
            &gEfiDiskInfoProtocolGuid,
            (VOID**)&pDiskInfo);    // Get DiskInfo protocol
        ASSERT_EFI_ERROR (Status);
        ...
        Status = pBS->HandleProtocol (
            ...
            (VOID**)&pPciIo);           // Get PciIo protocol
        ASSERT_EFI_ERROR (Status);
...
SetINT0F_E0000()
{
    ...
    //
    Status = GetLegacyRegion(&gCoreBiosInfo->iBios, sizeof(E0000_Int0f), E0000_BIT, 0x10, (VOID**)&P_E0000_Int0f);
...
)
{
    ...
    *MemMap = NULL;
    ...
    Status = pBS->AllocatePool(EfiBootServicesData, MemMapSize, (VOID**)MemMap);
    ASSERT_EFI_ERROR(Status);
