...
#if defined(AMIUSB_SUPPORT)
{
    ...
    if (gUsbProtocol == NULL) {
        Status = pBS->LocateProtocol(&gEfiUsbProtocolGuid, NULL, (VOID**)&gUsbProtocol);
        if (EFI_ERROR(Status)) {
...
)
{
    ...
    if (Stack)
    {
        ThunkData->StackData.Stack = (UINT32)(UINTN)Stack;
...
*/
    ...
    // Clear key modifiers status in BDA
    if (OldTpl == TPL_HIGH_LEVEL) {
        *(UINT8*)(UINTN)0x417 &= 0x70;
    } else {
        ACCESS_PAGE0_CODE(
            *(UINT8*)(UINTN)0x417 &= 0x70;
        );
