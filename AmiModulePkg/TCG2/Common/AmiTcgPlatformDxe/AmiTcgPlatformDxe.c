...
EFI_STATUS MeasureCpuMicroCode( )
{
    ...
    if(IsTcmSupportType())
    {
        Status = doTcmCpuMicrocodeTcgEvent((VOID *)(UINTN) MicrocodeBuffer, MicrocodeSize );
    }
    else
    {
        Status = doCpuMicrocodeTcgEvent( (VOID *)(UINTN)MicrocodeBuffer, MicrocodeSize );
    }
#endif
    return Status;
}
...
)
{
    ...
    for(i=0; i<5; i++)
    {
        ...
        if(!IsTcmSupportType())
        {
            Status = MeasureVariable(AMI_PCRi_HOST_PLATFORM_MANUFACTURER_CONTROL,
                                 Eventtypes[i],
                                 SecurityVar[i],
                                 &Varguid,
                                 VarData,
        ...
        {
            Status = TcmMeasureVariable(
                            AMI_PCRi_HOST_PLATFORM_MANUFACTURER_CONTROL,
                            Eventtypes[i],
                            SecurityVar[i],
                            &Varguid,
                            VarData,
    ...
    UINT16   pfa   )
{
    ...
    if(!IsTcmSupportType())
    {
        ...
        ev.Header.PCRIndex      = AMI_PCRi_OPROM_CODE;
        ...
        ev.Header.EventDataSize = _TPM_STRUCT_PARTIAL_SIZE(struct _AMI_EFI_TCG_EV_TAG,
    ...
    else
    {
        ...
        TcmEvent.Header.PCRIndex      = AMI_PCRi_OPROM_CODE;
        ...
        TcmEvent.Header.EventDataSize = _TPM_STRUCT_PARTIAL_SIZE(struct _AMI_EFI_TCG_EV_TAG,
...
EFI_STATUS MeasureWakeEvent()
{
    ...
    if(IsTcmSupportType())
    {
        Status =  TcmMeasureActionI( WakeString, (UINT32)AMI_PCRi_STATE_TRANSITION );
        if ( EFI_ERROR( Status ))
        {
            return Status;
        }
    ...
    {
        Status = TcgMeasureActionI( WakeString, (UINT32)AMI_PCRi_STATE_TRANSITION );
        if ( EFI_ERROR( Status ))
        {
            return Status;
        }
...
//        PERF_END(0,L"SelfTest",NULL,0);
        ...
        if(Support)
        {
            Status = TcmMeasureActionI (
                         ...
                         (UINT32)AMI_PCRi_IPL_CONFIG_AND_DATA );
        }
        else
        {
