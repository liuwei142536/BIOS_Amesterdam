...
)
{
    ...
    AmiGetFileListFromExtVolume(PeiServices, (UINT8*)RootBuffer, gRootSize, &NumberOfFiles, ExtRecoveryFiles);
...
/**
    ...
    @param Size - Size of the root directory
...
)
{
    ...
    *NumberOfFiles = 0;     // no files found yet
    ...
    while ( IsolateName (&FilePath, &NextName) == FALSE ) {
        ...
        //
        if ( FindFileInExtDirectory((UINT8 *)DirPtr, Size, FilePath, &EntryPtr) ) {
            //
            // An EXT(n) directory  entry only contains the name and inode, so we have to
            // read the inode to get the size.
            //
