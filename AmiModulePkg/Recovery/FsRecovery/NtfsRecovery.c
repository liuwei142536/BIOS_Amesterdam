...
)
{
    ...
    if (!RootAllocated) {
        ...
        Status = (*PeiServices)->AllocatePages((CONST EFI_PEI_SERVICES  **)PeiServices, EfiBootServicesData, DirPages, &Allocate );
        if ( EFI_ERROR( Status )) {
...
)
{
    ...
    *NumberOfFiles = 0;     //no files found yet
    ...
    while ( IsolateName (&FilePath, &NextName) == FALSE ) {
        ...
        //
        if ( FindFileInNtfsDirectory( PeiServices, (UINT8 *)DirPtr, Size, FilePath, &IndxPtr )) {
