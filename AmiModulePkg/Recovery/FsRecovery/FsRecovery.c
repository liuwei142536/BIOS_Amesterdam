...
// Get capsule from variable support++
BOOLEAN             IsUnicode = FALSE;
BOOLEAN             CapsuleFromVariable = FALSE;
BOOLEAN             SignatureMatch;
//
// PORTING REQUIRED
// This structure, and the variable that contins it, may require porting.
//
typedef struct {
    HARDDRIVE_DEVICE_PATH pdp;
    CHAR16 fname[256];
    UINT32 fsize;
} AMI_CAPSULE_FILENAME;
#define CMM_FLASH_UPDATE_VAR_GUID \
        {0xa4ea2f68, 0x5778, 0x40aa, 0xb1, 0xd7, 0x60, 0x0e, 0x6e, 0x16, 0xb3, 0xcc}
static CHAR16 CmmFlashUpdateVarName[] = L"FlashUpdateImageName";
static EFI_GUID CmmFlashUpdateVarGuid = CMM_FLASH_UPDATE_VAR_GUID;
// Porting end
AMI_CAPSULE_FILENAME CapInfo;
// Get capsule from variable support--
...
)
{
    ...
    if (ThisVolume->FatType != FAT32) return FALSE;
    if (IsUnicode) {
...
)
{
    ...
    if ( BufferSize < EFI_PAGES_TO_SIZE( ActualPages )) {
        Status = (*PeiServices)->AllocatePages((CONST EFI_PEI_SERVICES **)PeiServices, EfiBootServicesData, ActualPages, &Allocate );
...
)
{
    ...
    if(FatWindow == NULL) {
        Status = (*PeiServices)->AllocatePool((CONST EFI_PEI_SERVICES  **)PeiServices, sizeof(FAT_WINDOW), (VOID**)&FatWindow );
        if(EFI_ERROR(Status)) {
            return Status;
        }
        ...
        Status = (*PeiServices)->AllocatePages((CONST EFI_PEI_SERVICES **)PeiServices, EfiBootServicesData, FatPages, &Allocate );
        if( EFI_ERROR(Status) ) {
            FatWindow = NULL;
            return Status;
        }
...
)
{
    ...
    if ( RootBufferSize < EFI_PAGES_TO_SIZE( RootPages )) {
        Status = (*PeiServices)->AllocatePages((CONST EFI_PEI_SERVICES **)PeiServices, EfiBootServicesData, RootPages, &Allocate );
        if ( EFI_ERROR( Status )) {
            return Status;
        }
...
)
{
    ...
    do {
        ...
        if ( !EFI_ERROR(Status) && gIntegrityCheck ) {
            if ( CapsuleFromVariable && (SignatureMatch == FALSE) ) continue;
...
)
{
    ...
    Status = (*PeiServices)->AllocatePages((CONST EFI_PEI_SERVICES **)PeiServices, EfiBootServicesData, BlockPages, &Allocate );
    if ( EFI_ERROR( Status )) {
        return EFI_NOT_FOUND;
    }
...
;
#elif defined(BIOS_GUARD_RECOVERY_SUPPORT) && BIOS_GUARD_RECOVERY_SUPPORT == 1
;
#else
{
    return EFI_SUCCESS;
...
)
{
    //
    // PORTING REQUIRED
    // This code will attempt to get the capsule information from a
    // variable. The name of the variable, and the information in it
    // may require porting.
    //
    EFI_STATUS  Status;
    EFI_PEI_READ_ONLY_VARIABLE2_PPI *ReadOnlyVariable;
    UINTN Size = sizeof(CapInfo);
    Status = PeiServicesLocatePpi (
             &gEfiPeiReadOnlyVariable2PpiGuid,
             0, NULL, (VOID **) &ReadOnlyVariable);
    if (EFI_ERROR (Status))
        return Status;
    
    Status = ReadOnlyVariable->GetVariable (
             ReadOnlyVariable,
             CmmFlashUpdateVarName,
             &CmmFlashUpdateVarGuid,
             NULL,
             &Size,
             &CapInfo);
    ...
    if (!EFI_ERROR (Status)) {
        if( pCapsuleName != NULL ) {
            *pCapsuleName = &CapInfo.fname[0];
        }
        if ( pCapsuleSize != NULL ) {
            *pCapsuleSize = CapInfo.fsize;
        }
        if( ExtendedVerification != NULL )
            *ExtendedVerification = FALSE;

        IsUnicode = TRUE; // Indicate path is unicode
        CapsuleFromVariable = TRUE;
        SearchPath = TRUE;
        return EFI_SUCCESS;
    //
    // PORTING END
    //
    } else { // Get capsule name and path from tokens
        if( pCapsuleName != NULL ) {
...
)
{
    ...
    do
    {
        ...
        UINTN                         j;
        UINTN                         Size;
        Status = (*PeiServices)->LocatePpi((CONST EFI_PEI_SERVICES  **)PeiServices, &gEfiPeiVirtualBlockIoPpiGuid, i++, &pDummy, (VOID**)&pBlockIo );

        if ( EFI_ERROR( Status )) {
            break;
        }

        Status = pBlockIo->GetNumberOfBlockDevices( PeiServices, pBlockIo, &NumberBlockDevices);
...
{
    ThisPeiServices = (EFI_PEI_SERVICES **)PeiServices;
...
/**
    If SearchPath is TRUE, isolates each name in a path
...
/**
    ...
    @param Entries - Number of root entries
        ...
        for( i = 0; i < DirEntries; i++ ) {
            ...
            if(!EFI_ERROR( FileSearch ((CHAR8*)FatName, DirPtr[i].FileName, TRUE, FAT_FILE_NAME_SIZE ))) {
                ...
                if ( DirSize < EFI_PAGES_TO_SIZE( DirPages )) {
                    Status = (*ThisPeiServices)->AllocatePages((CONST EFI_PEI_SERVICES  **)ThisPeiServices,
...
/**
    ...
    @param Count              - Counter to keep track of search
    @param Mbr                - True if looking for MBR partitions
	@param RecoveryDeviceType - Recovery Device Type
    @param *MemoryBlk         - pointer to Memory Block structure
...
OemPartitionVolumeIntegrityCheck(
  ...
  IN     MEMORY_BLOCK    *MemoryBlk  )
{
  EFI_STATUS  Status = EFI_SUCCESS;
  UINTN       i;

  for (i = 0; OemRecoveryVolumeIntegrityCallback[i] != NULL; i++)
    Status = OemRecoveryVolumeIntegrityCallback[i](Count,Mbr,RecoveryDeviceType,MemoryBlk);
...
)
{
    ...
    if ( IsMbr ) {
        while ( PartCount < 4 ) {
            ...
            if ( CapsuleFromVariable ) {
                UINT32 *CapSig = (UINT32*)&CapInfo.pdp.Signature[0];
                SignatureMatch = TRUE;
                if ( CapInfo.pdp.MBRType != 1 ) {
                    SignatureMatch = FALSE;
                }
                if ( MemBlk->Mbr.UniqueMbrSig != *CapSig ) {
                    SignatureMatch = FALSE;
                }
            }
            ...
            Status  = OemPartitionVolumeIntegrityCheck(PartCount-1,IsMbr, gRecoveryDeviceType,MemBlk);
                if (!EFI_ERROR(Status))
    ...
    while ( PartCount < GpeCount ) {
        ...
        if ( CapsuleFromVariable ) {
            UINT8 j;
            SignatureMatch = TRUE;
            if ( CapInfo.pdp.MBRType != 2 ) SignatureMatch = FALSE;
            for (j=0; j<16; j++) {
                if (Gbr->GuidPart[i].UniqueGuid[j] != CapInfo.pdp.Signature[j]) {
                    SignatureMatch = FALSE;
                }
            }
        }
        ...
        )
{
    ...
    Status = (*PeiServices)->GetHobList((CONST EFI_PEI_SERVICES **)PeiServices, (VOID **)&RecoveryHob);
    if(!EFI_ERROR(Status)) {
