...
)
{
    ...
    //
    ComSerialDevice.SerialDeviceBaseAddress = PcdGet64( PcdSerialRegisterBase );
...
)
{
    ...
    do {
        ...
        *ReadChar = '\n';
        SerialPortWrite((UINT8 *)ReadChar, 0x01 );
        ...
        do {
            ...
            while ( waitCount-- ) {
                if ( GetSerialData ((UINT8 *)ReadChar ) == TRUE ) {
                    Recovery = TRUE;
                    break;
                }
            }
        ...
        //
        mStallPpi->Stall((CONST EFI_PEI_SERVICES **)mPeiServices, mStallPpi, 20000000 );
        ...
        if ( !ReadFileFromSerial( pBuff, &FileSize, &Result )) {
            ...
            if( ExtendedVerification || FileSize == (UINT32)RecoveryCapsuleSize ) {
                ReportCapsuleSize(PeiServices, (UINT32)FileSize);
                return EFI_SUCCESS;
            } else {
        ...
        if ( Data == XMODEM_SOH ) {
            ...
            if ( Status && (Data == BlockNum)) {
                ...
                if ( Status ) {
                    ...
                    //
                    if ( ReadSerialBlock((UINT8 *)Buffer) ) {     
                        //
...
{
    UINTN    Result;
    UINTN    MaxDelay = SERIAL_PACKET_DELAY;// ms

    // Default Timeout of 1sec as some console application
    // takes time to send the packets.
    while(!SerialPortPoll() && MaxDelay) {
        mStallPpi->Stall((CONST EFI_PEI_SERVICES **)mPeiServices, mStallPpi, 1000 );
        MaxDelay--;
    }
