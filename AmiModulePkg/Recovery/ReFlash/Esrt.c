...
)
{
    ...
    if(Image == NULL)
        ...
        Status = GetFidFromBuffer(FvImage, &Fid);
...
)
{
    switch(VerificationStatus) {
        ...
        case 5:
            return LAST_ATTEMPT_STATUS_ERROR_PWR_EVT_AC;
        case 6:
            return LAST_ATTEMPT_STATUS_ERROR_PWR_EVT_BATT;
        default:
            break;
    }
...
)
{
    ...
    Status = gBS->LocateProtocol (&gEsrtManagementProtocolGuid, NULL, (VOID **)&EsrtMp);
    if (EFI_ERROR (Status))
        return Status;
...
**/
BOOLEAN IsWin8Update(EFI_STATUS Verification)
{
    VOID *HobStart;
    ...
    if(!ImageHobFound)   //no image hob - we're not on Windows FW update path
        return FALSE;

    if (!EFI_ERROR (Verification)) {
...
)
{
    ...
    Status = FlashControlFlow(&esrt_progress);
    if(EFI_ERROR(Status))
        gRT->ResetSystem(EfiResetCold, Status, 0, NULL);
