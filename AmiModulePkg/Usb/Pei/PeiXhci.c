...
PCI_BUS_DEV_FUNCTION  *gXhciControllerPciTable;

PCI_DEV_REGISTER_VALUE *gPeiXhciInitPciTable;
    ...
    IN EFI_PEI_SERVICES    **PeiServices )
{
    ...
    UINTN                   XhciBaseAddress = gUsbPeiData->XhciIoBase;
    ...
    if ((gUsbPeiData->UsbPeiFeature & USB_XHCI_PEI_SUPPORT) != USB_XHCI_PEI_SUPPORT)
        return EFI_UNSUPPORTED;
    gXhciControllerPciTable = gUsbPeiData->XhciPciDev;
    gPeiXhciInitPciTable = gUsbPeiData->XhciRegTbl;
    ...
    for (PciDevIndex = 0; PciDevIndex < gUsbPeiData->XhciRegInitCount; PciDevIndex++) {
    ...
    for (ControllerIndex = 0; ControllerIndex < gUsbPeiData->XhciHcCount;
         ControllerIndex++, XhciBaseAddress += gUsbPeiData->XhciMmioSize) {
        UINT16 Vid;
        UINT16 Did;
...
)
{
    ...
    if ((gUsbPeiData->UsbPeiFeature & USB_HOST_ERROR_SUPPORT) == USB_HOST_ERROR_SUPPORT)
...
{    
    ...
    switch (Attributes) {
        case EFI_USB_HC_RESET_GLOBAL:
            ...
            if (Usb3Hc->ControllerIndex == gUsbPeiData->XhciHcCount - 1) {
