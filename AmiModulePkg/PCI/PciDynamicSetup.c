...
//-----------------------    
    ...
    varid.IdField.PciDevIndex= PciIdx & 0xFFF;
    return (EFI_VARSTORE_ID)(varid.VAR_ID);
}


/**
    Creates PCI Latency Time One of Options.
...
/**
    ...
    @param OpcodeHandle Pointer on Opcode Handle created
    @param OpType Option padding type
...
//        STRING_TOKEN(STR_256G),    STRING_TOKEN(STR_512G),
    ...
    //START: Fix for Coverity Issue : UNINIT - cnt,d and strid Un-Initialized value
    EFI_STRING_ID    *strid = NULL;
    UINTN            cnt=0,i,d = 0;
    //END: Fix for Coverity Issue : UNINIT - cnt,d and strid Un-Initialized value
    UINT64            base,val;
    UINT8            Flags;
//--------------------------------
    ...
    for (i=0; i<cnt; i++){
            ...
            //Update Default flag for one of option.
...
/**
    ...
    Device Class Sub Class; Features Supported
...
//----------------------------    
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            ARI_FORWARDING_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            ATOMIC_OPERATION_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            ATOMIC_OPERATION_EGRESS_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            IDO_REQ_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            IDO_COMPLETION_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            LTR_REPORTING_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            E2E_TLP_PREFIX_BLOCK_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if (!gIntegratedPcie){
        ...
        if(!DontTouch){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                COMPLIANCE_SOS_SETUP_DEFAULT_VALUE, // Disable - the default settings.
                NULL
            );
        }
        ...
        if(!DontTouch){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                HW_AUTO_WIDTH_SETUP_DEFAULT_VALUE, // Enable - the default settings.
                NULL
            );
        }
        
        //14. HW Auto Speed Disable... Check Box...
        DontTouch=FALSE;
        ...
        if(!DontTouch){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                HW_AUTO_SPEED_SETUP_DEFAULT_VALUE, // Enable - the default settings.
                NULL
            );
        }
    }//gIntegratedPcie
...
//----------------------------    
    ...
    if(!DontTouch){
        ...
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            RELAXED_ORDERING_SETUP_DEFAULT_VALUE, // Enable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            EXTENDED_TAG_FIELD_SETUP_DEFAULT_VALUE, // Disable - the default settings.
            NULL
        );
    }
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            NO_SNOOP_SETUP_DEFAULT_VALUE, // Enable - the default settings.
            NULL
        );
    }
    ...
    if (!gIntegratedPcie){
        ...
        if(!DontTouch){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                EXTENDED_SYNC_SETUP_DEFAULT_VALUE, // Disable - the default settings.
                NULL
            );
        }
        ...
        if(!DontTouch){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                CLOCK_PM_SETUP_DEFAULT_VALUE, // Disable - the default settings.
                NULL
            );
        }
        
        //11. Now goes Link Training Retry Count selection...
        //11a. It has 4 one of options.
        DontTouch=FALSE;
        ...
        if(!DontTouch){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                PCIE_LINK_ENABLE_SETUP_DEFAULT_VALUE, // Enable - the default settings.
                NULL
            );
        }
        DontTouch=FALSE;
...
/**
    ...
    @param KeyValue Setup Browser Key Value of the control.
...
//--------------------------------------------
    ...
    //Now check if device or slot supports PCI EXPRESS and HP features and what kind it is.  
    //and check if it is integrated PCIe device...for integrated OnBoard devices we are not suppose 
    ...
    if(gotodata->PciDevInfo!=NULL){
        //if Slot is Populated or OnBoard Device we should have this Filled in...
    ...
    //Now check if device or slot supports HP features and what kind it is.  
    //Check if it is Slot with Enabled bridge, bridge must have HotPlug flag set..
    ...
    //now apply tokens value for display HP and PCIe options.
    if (PcdGetBool(AmiPcdPcieProgrammingSupport)){
    ...
    if(!DontTouch){
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            VGA_PALLETE_SNOOP_SETUP_DEFAULT_VALUE,
            NULL
        );
    }
    ...
    if(!DontTouch){
        ...
        HiiCreateCheckBoxOpCode (
            ...
            EFI_IFR_FLAG_RESET_REQUIRED,
            !(ABOVE_4G_PCI_DECODE_DEFAULT_VALUE),
            NULL
        );
    ...
    if(!DontTouch){
        if(pcie1){
            HiiCreateCheckBoxOpCode (
                ...
                EFI_IFR_FLAG_RESET_REQUIRED,
                PCIE_GEN1_ENABLE_SETUP_DEFAULT_VALUE,
                NULL
            );
            ...
            if(!DontTouch){
                if(pcie2){
                    ...
                    HiiCreateCheckBoxOpCode (
                        ...
                        EFI_IFR_FLAG_RESET_REQUIRED,
                        PCIE_GEN2_ENABLE_SETUP_DEFAULT_VALUE,
                        NULL
                    );
                    AmiHiiTerminateScope(gPciDevForm.DevStartOpCodeHandle);
                }
...
//DEBUG//DEBUG//DEBUG//DEBUG//DEBUG//DEBUG
    ...
    //----------------------------------
        ...
        for (j=0; j<PciDevHandleCount; j++){
            ...
            //Check if we are running with AMI driver and can type cast data like that
...
/**
    This will sort setup devices items in Following order:
...
//START: Fix for Coverity Issue : CHECKED_RETURN
    ...
    if(!EFI_ERROR(Status)){
        ...
        //START: Fix for Coverity Issue : NINIT - Un-Initialized Status used in DEBUG, Removed the Status printing 
        //SortData(&gPciIfrInfo);
        //Tell how many devices with setup we have found...
        DEBUG((DEBUG_VERBOSE,"PciSetup: Found %d PCI Devices With Setup Pages... \n",gPciIfrInfo.PciDevCount));
        //END: Fix for Coverity Issue : UNINIT - Un-Initialized Status used in DEBUG, Removed the Status printing 
