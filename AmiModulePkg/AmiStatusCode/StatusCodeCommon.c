...
EFI_STATUS ReportAssertString(IN EFI_STATUS_CODE_DATA *Data, OUT UINT8 *String)
{
    ...
    Sprintf_s((CHAR8*)String, StringMaxSize, "ASSERT in %s on %i: %s\n", FileName, LineNum, Desc);
    return EFI_SUCCESS;
}
...
//#if STRING_STATUS_SUPPORT
    ...
    if (String[0] != '\0') StringStatusReport(PeiServices,(CHAR8*)String);
...
)
{
    ...
    AMI_STATUS_CODE_BUFFER     *Buffer;
    
    if (Format==NULL) return;
    AsciiVSPrint ((CHAR8*)String, StringMaxSize, Format, VaListMarker);
    
    DebugServicePrintEnable = DebugServicePrintGate((CHAR8*)String, ErrorLevel);
    
    if (DebugServicePrintEnable == FALSE) return;
    
    Buffer = BASE_CR (String, AMI_STATUS_CODE_BUFFER, String);
    
    Buffer->DebugInfo.ErrorLevel = (UINT32)ErrorLevel;
    ...
    DebugServicePrint((CHAR8*)String, ErrorLevel);
    StringStatusReport(PeiServices, (CHAR8*)String);
}
...
)
{
    ...
    if (DebugServiceAssertEnable == FALSE) return;
  	...
  	Buffer->DebugInfo.ErrorLevel = EFI_D_ERROR;
    ...
    while (ResumableAssert)
  	{
  		SerialPortRead ((UINT8*)KeyBuffer, 1);
  		// Check if key was pressed
  		if (KeyBuffer[0] != 0) return;
