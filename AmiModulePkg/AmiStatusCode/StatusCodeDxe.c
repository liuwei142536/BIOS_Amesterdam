...
AMI_STATUS_CODE_BUFFER StausCodeBuffer = {
		{	(UINT16) sizeof (EFI_STATUS_CODE_DATA), 
			2048 + sizeof (EFI_DEBUG_INFO) + 12 * sizeof (UINT64),
...
UINT8 *String = (UINT8*)&(StausCodeBuffer.String[0]);
...
#elif defined(MDE_CPU_ARM) || defined(MDE_CPU_AARCH64)
CHAR8 *CpuExceptions[] =
{
    "Reset",
...
BOOLEAN PrintDetailedErrorMessage(IN EFI_STATUS_CODE_VALUE Value, OUT UINT8 *Str){
    ...
    if  (ClassSubClass==EFI_SOFTWARE_IA32_EXCEPTION || ClassSubClass==EFI_SOFTWARE_X64_EXCEPTION){
    ...
    UINT32 ClassSubClass = Value & (EFI_STATUS_CODE_CLASS_MASK | EFI_STATUS_CODE_SUBCLASS_MASK);
    if  (ClassSubClass==EFI_SOFTWARE_ARM_EXCEPTION){
        UINT32 ExceptionNumber = Value & EFI_STATUS_CODE_OPERATION_MASK;
...
**/
VOID EFIAPI RscHandlerNotification (
    IN EFI_EVENT        Event,
    IN VOID             *Context
    )
{
...
**/
EFI_STATUS EFIAPI RouterRegister (
...
**/
EFI_STATUS EFIAPI RouterUnregister (IN EFI_RSC_HANDLER_CALLBACK Callback)
{
    RSC_HANDLER_CALLBACK_ENTRY    *CallbackEntry;
    UINT32                        i;
    VOID                          *Ptr;
    ...
    for (i = 0; i != RouterCallbackStr->RegisteredEntries; i++) 
    {
        ...
        if (CallbackEntry->RscHandlerCallback == Callback) 
        {
         ...
         // If the function is found in list, delete it and return.
            ...
            if ((CallbackEntry->Tpl != TPL_HIGH_LEVEL) && (CallbackEntry->StatusCodeDataBuffer != 0) && !StatusRuntime) // There are no BS after Exit BS
            {
                pBS->FreePool ((VOID *) (UINTN) CallbackEntry->StatusCodeDataBuffer);
                pBS->CloseEvent (CallbackEntry->Event);
            }
            CallbackEntry->RscHandlerCallback = 0;
            CallbackEntry->Tpl = 0;
            CallbackEntry->Event = 0;
            CallbackEntry->StatusCodeDataBuffer = 0;
            CallbackEntry->BufferSize = 0;
            CallbackEntry->EndPointer = 0;
        ...
        if (CallbackEntry->EndPointer > (CallbackEntry->StatusCodeDataBuffer + (CallbackEntry->BufferSize / 8) * 7)) 
        {
            if ((CurrentTpl < TPL_HIGH_LEVEL) && !InRscHandlerNotification)
            {
                ...
                if (NewBuffer != NULL && CallbackEntry->StatusCodeDataBuffer != 0)
                {
                    ...
                    MemCpy (NewBuffer, (VOID*)(UINTN)CallbackEntry->StatusCodeDataBuffer, CallbackEntry->BufferSize);
                    pBS->FreePool ((VOID *)(UINTN)CallbackEntry->StatusCodeDataBuffer);
                }
...
{
	return ReportDxeAndRtRouter(Type, Value, Instance, CallerId, Data); 
}

EFI_STATUS EFIAPI DxeRtReportStatusCode (
...
**/
VOID EFIAPI ScRuntimeVirtualAddressChange (IN EFI_EVENT Event, IN VOID *Context)
{
    ...
    pRS->ConvertPointer(0,(VOID**)&(AmiDebugServiceProtocol.Debug));
    pRS->ConvertPointer(0,(VOID**)&(AmiDebugServiceProtocol.DebugAssert));
...
VOID EFIAPI RuntimeStatusCodeExitBS (IN EFI_EVENT Event, IN VOID *Context)
{
	UINTN i;
	StatusRuntime = TRUE;
...
)
{
    ...
    static EFI_GUID gStdCallerIdGuid = EFI_STANDARD_CALLER_ID_GUID;
    ...
    if (CallerId==NULL)  Record->CallerId = gStdCallerIdGuid;
...
**/
VOID EFIAPI DataHubInstalled (IN EFI_EVENT Event, IN VOID *Context)
{
    EFI_STATUS Status;
...
**/
VOID EFIAPI StopConOutCheckpoints (IN EFI_EVENT Event, IN VOID *Context)
{
    ConsoleControl=NULL;
...
**/
VOID EFIAPI ConOutInstalled (IN EFI_EVENT Event, IN VOID *Context)
{
    EFI_STATUS Status;
