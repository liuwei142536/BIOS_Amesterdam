        ...
        switch (sl->OpType){
            ...
            case cfRoutine:
            {
                ...
                if(gSioInitProtocolPtr) {
                    SIO_TRACE((TRACE_SIO,"SmmSIO: Calling InitProtocol->Function[%d](Enter/ExitCfgMode)...", *FunctionNo));
                    Status = gSioInitProtocolPtr->Functions[*FunctionNo](gSioInitProtocolPtr, (UINTN *)*FunctionNo, &Parameters);
    ...
    for (i=0; i<sl->InstrCount; i++){
        switch (sl->OpType){
            ...
            case cfRoutine:
            {
                ...
                Parameters.Param4 = NULL;                
                if(gSioInitProtocolPtr) {
                    DEBUG((-1,"DavidLee:BootScript FunctionNo =  %X\n",*FunctionNo));
                    gSioInitProtocolPtr->Functions[*FunctionNo](gSioInitProtocolPtr, (UINTN *)*FunctionNo, &Parameters);
                }
