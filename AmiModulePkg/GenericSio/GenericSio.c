...
//-----------------------------------------
	...
	Status=pBS->LocateProtocol( &gAmiSioPortCompatibilityProtocolGuid,	//*Protocol,
								...
								(VOID **)&gSioCompProtocol);				//*Interface
	...
	Status=pBS->LocateProtocol( &gAmiBoardSioInitProtocolGuid,	//*Protocol,
								NULL,							//*Registration OPTIONAL,
								(VOID **)&gSioInitProtocol);				//*Interface
...
//	EFI_DEADLOOP();
	...
	if(Event && gPDsdt) pBS->CloseEvent(Event);
}
#endif	
...
//----------------
    ...
    //in case of non Standard Logical Device just return.
    if(!Dev->StdLogicDev) return;
...
//--------------------------
    ...
    SIO_TRACE((TRACE_SIO,"\nGSIO: ApplyIO StdSio=%d, MIN=%x, LEN=%x, rd=%x; ",Dev->Owner->SpioSdlInfo->StdSioDevice,iord->_MIN,iord->_LEN,rd));
    //Don't need to run all this if we got an empty descriptor
    if(!iord->_LEN) {
        SIO_TRACE((TRACE_SIO,"Empty Descriptor\n"));
        //Don't need to run all this if we got an empty descriptor
...
//----------------------------------
	...
	Status=pBS->OpenProtocol( CtrlHandle, &gEfiPciIoProtocolGuid, (VOID **)&PciIo,
...
//-------------------------
    ...
    //Logical device is standard or not, we assume older SIO module's LDs are all standard.
    SioLibLaunchInitRoutine(Dev,isNonStdLogicalDev);
					   ...
					   IN EFI_DEVICE_PATH_PROTOCOL			*RemainingDevicePath )
{
	EFI_PCI_IO_PROTOCOL		*PciIo;		
	EFI_STATUS			Status;
	EFI_STATUS          StatusRdp = EFI_SUCCESS;
	INTN				i;
	UINTN				j,k;
	SIO_DEV2			*dev;
    ...
    UINTN               DataSize;
    SIO_SETUP_DATA      *SioData = NULL;
    EFI_GUID            SioSetupGuid = SIO_SETUP_GUID;
...
//--------------------------------
    ...
    DataSize = sizeof(SIO_SETUP_DATA);
    Status = GetEfiVariable(L"SioSetupData", &SioSetupGuid, NULL, &DataSize, (VOID **)&SioData);
    if(EFI_ERROR(Status))
        SioData->LockLegacyRes = 0;
    ...
    for(k=0;k<2;k++){
		...
		for (i=0; (UINTN)i<spio->LdCount; i++){
		    ...
		    SIO_TRACE((TRACE_SIO,"\n"));
            if(SioData->LockLegacyRes) {
                if(gPDsdt!=NULL){
                    Status = GetAslObj((UINT8*)gPDsdt+sizeof(ACPI_HDR),
                            gPDsdt->Length-sizeof(ACPI_HDR)-1, &dev->DeviceInfo->AslName[0],
                            otDevice, &AslObj);
                    if (!EFI_ERROR(Status)) {
                        HideAslMethodFromOs (&AslObj, (UINT8 *)"_PRS");
                        Recalculate = TRUE;
                    }
                }
            }
    ...
    if (gPDsdt && Recalculate)
    {
        gPDsdt->Checksum = 0;
        gPDsdt->Checksum = ChsumTbl((UINT8*)gPDsdt, gPDsdt->Length);
    }
    if(SioData != NULL) pBS->FreePool(SioData);
    if(RemainingDevicePath!=NULL) {
...
//----------------------------------
    ...
    for(Index = 0;Index < gSpio->LdCount;Index ++) {
        gSmmCommunicationBuffer->MessageLength = gSpio->LdCount;
        gSmmCommunicationBuffer->Data[Index] = gSpio->DeviceList[Index]->StdLogicDev;
    }
    CommSz = sizeof(EFI_SMM_COMMUNICATE_HEADER) + (gSpio->LdCount * sizeof(UINT8));
...
//--------------------------------------------------------------------
	...
	if(!gPDsdt) {
		...
		SIO_TRACE((TRACE_SIO,"GSIO: Get DSDT FAILURE. Installing Callback (%r)\n", Status));
	} else SIO_TRACE((TRACE_SIO,"GSIO: Get DSDT SUCCESS \n"));
    ...
    pBS->AllocatePool(EfiRuntimeServicesData, sizeof(EFI_SMM_COMMUNICATE_HEADER) + (gSpio->LdCount * sizeof(UINT8)), (VOID **)&gSmmCommunicationBuffer);
