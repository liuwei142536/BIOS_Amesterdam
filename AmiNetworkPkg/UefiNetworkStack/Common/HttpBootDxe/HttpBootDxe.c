...
AMI_NETWORK_BLOCKED  BlockedHttpBootNetworkList[] = {
  ...
  )
{
  ...
  #if (NET_PKG_AMI_PORTING_ENABLE == 1)
  // Check if the Network device is present in the Http Boot Blocked List.
  // If present, don't install the Http boot support for the Network device.
  if(IsAmiNetworkDeviceBlocked(ControllerHandle, (AMI_NETWORK_BLOCKED*)BlockedHttpBootNetworkList)) {
...
GetHttpBootSetupOptions (&Ipv4HttpBoot,
                          &Ipv6HttpBoot,
                          &MediaDetectCount
                          );
