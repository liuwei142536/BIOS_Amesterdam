...
#include "UefiNetworkStackElink.h"

AMI_NETWORK_BLOCKED  BlockedPxeBootNetworkList[] = {
        BLOCKED_NETWORK_DEVICE_INFO_LIST
        { 0xFFFF, 0xFFFF } 
};
UINT8   Ipv4PxeBoot;
UINT8   Ipv6PxeBoot;
  ...
  )
{
  ...
  GetPxeBootSetupOptions (&Ipv4PxeBoot,
                          &Ipv6PxeBoot,
                          &PxeBootWaitTime,
                          &MediaDetectCount
                          );
  ...
  )
{
  ...
  if (IpVersion == IP_VERSION_4) {
	...
	//
    if (Ipv4PxeBoot == 0)  return EFI_UNSUPPORTED;
  } else {
    //
    // Check whether PxeIpv6 is disabled in setup if so return EFI_UNSUPPORTED.
    //
    if (Ipv6PxeBoot == 0)  return EFI_UNSUPPORTED;
  }
...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  if(!EFI_ERROR(Status)) {
      // Check if the Network device is present in the Pxe Boot Blocked List.
      // If present, don't install the Pxe boot support for the Network device.
      if(IsAmiNetworkDeviceBlocked(ControllerHandle, (AMI_NETWORK_BLOCKED*)BlockedPxeBootNetworkList)) {
          return EFI_UNSUPPORTED;
      }
   }
#endif
