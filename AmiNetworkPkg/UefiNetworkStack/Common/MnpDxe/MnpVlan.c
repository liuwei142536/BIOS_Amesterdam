  ...
  )
{
  ...
  {
      //    Get Redfish Vlan data corresponding to current NIC	
  ...
  // If Vlan data aready available in redfish skip the creation of Mnp Service data and the conneting process.
  for (Index = 0; Index < gNumberOfRedfishVlan; Index++) 
  {
      if( gParsedRedfishVlanData[Index].VLANID != VlanId )
      {
          continue;
      }
      else
  ...
  MnpGetVlanVariable (MnpDeviceData, &NumberOfVlan, &OldVariable);
	...
	//  Newly added valan data is available in redfish vlan and to check same data is available in vlan NVRAM 
	//	If available override it otherwise add to NVRAM.
...
#if(AMIRedFishVlan_SUPPORT)
  //    Get the MAC of the current NIC.
  MacAddrToStr (&MnpDeviceData->Snp->Mode->PermanentAddress,6,0,MacString);
#endif  
  if (VlanId == NULL) {
    //
    ...
    *Entries = VlanData;
    NET_LIST_FOR_EACH (Entry, &MnpDeviceData->ServiceList) {
      MnpServiceData = MNP_SERVICE_DATA_FROM_LINK (Entry);
#if(AMIRedFishVlan_SUPPORT)    
      //  Get the Vlan added from setup separately from available service list
	  //  For display purpose
      for (Index = 0; Index < gNumberOfRedfishVlan; Index++) 
      {
          MacAddrToStr (&gParsedRedfishVlanData[Index].MacAddress,6,0,RedfishMacString);
          if(!(StrCmp(MacString, RedfishMacString)) && (gParsedRedfishVlanData[Index].VLANID == MnpServiceData->VlanId))
          {    
              ServiceData = TRUE;
              break;
          }
  ...
  )
{
  ...
  VLAN_TCI          *VlanData;
#if(AMIRedFishVlan_SUPPORT)
  VLAN_TCI          *VlanConfigData;
  UINTN             NumberOfVlan, Index, VlanDataCount = 0, ServiceCount = 0;
#endif
...
#if(AMIRedFishVlan_SUPPORT)
  //  Get the total no of services available for particular devicedata
  //  If service count is 1 only Default service is available
  NET_LIST_FOR_EACH (Entry, &MnpDeviceData->ServiceList) {
      ServiceCount++;
  }  
  if(ServiceCount != 1)
  ...
  Status = MnpGetVlanVariable (MnpDeviceData, &NumberOfVlan, &VlanConfigData);
  if (EFI_ERROR (Status)) {
      NumberOfVlan = 0;
  }
#endif
  ...
  if (MnpDeviceData->NumberOfVlan != 0) {
	...
	//	Update only if its not a Redfish Vlan Data.

      for (Index = 0; Index < NumberOfVlan; Index++)
      {
          if(VlanConfigData[Index].Bits.Vid == MnpServiceData->VlanId)
          {
#endif
...
#if(AMIRedFishVlan_SUPPORT)
  Status = MnpSetVlanVariable (MnpDeviceData, VlanDataCount, VlanVariable);
    if (VlanConfigData != NULL) {
    FreePool (VlanConfigData);
  }
#else
