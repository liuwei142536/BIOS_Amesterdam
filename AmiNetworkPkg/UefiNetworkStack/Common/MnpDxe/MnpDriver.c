...
#include "MnpImpl.h"
#include "MnpVlan.h"
#include <Library/AmiNetworkLib.h> // AMI PORTING
...
REDFISH_VLAN_CONFIG_DATA  *gParsedRedfishVlanData;
...
{
  EFI_STATUS        Status;
  MNP_SERVICE_DATA  *MnpServiceData = NULL;
  ...
  Status = RedfishVlanconfigProtocol->GetData(RedfishVlanconfigProtocol, &gParsedRedfishVlanData, &gNumberOfRedfishVlan);
  if (EFI_ERROR (Status)) {
      goto REDFISH_EXIT;
  }
  MacAddrToStr (&MnpDeviceData->Snp->Mode->PermanentAddress,6,0,MacString);
  MnpDeviceData->NumberOfVlan = gNumberOfRedfishVlan;
  
  //	Add new Redfish Vlan data to their corresponding NIC
  
  for (Index = 0; Index < gNumberOfRedfishVlan; Index++) 
  {
      MacAddrToStr (&gParsedRedfishVlanData[Index].MacAddress,6,0,RedfishMacString);
      if(!(StrCmp(MacString, RedfishMacString)) && (gParsedRedfishVlanData[Index].VLANEnable == 1))
      {     
          VlanCount++;
		  
		  // Create Mnp Service Data for the new Vlan Data
		  
          MnpServiceData = MnpCreateServiceData (
                  MnpDeviceData,
                  gParsedRedfishVlanData[Index].VLANID,
                  (UINT8) 0
          );      
...
#if(AMIRedFishVlan_SUPPORT)
	  // Check newly added Vlan data is available in Redfish vlan data or not.
      VlanIdStatus = CheckRedfishVlanData(gParsedRedfishVlanData,( UINT8 ) VlanVariable[Index].Bits.Vid);
      if( VlanIdStatus )        
      {
          continue;
      }
...
#if( AMIRedFishVlan_SUPPORT )
  if( gParsedRedfishVlanData!=NULL )
  {
    FreePool( gParsedRedfishVlanData );
  }
  if( gRedfishVlanData!=NULL )
  {
	FreePool( gRedfishVlanData );
  }
#endif
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  //Initialize the Redfish Vlan Protocol
#if(AMIRedFishVlan_SUPPORT)
  EFI_STATUS            Status;
  Status = RedfishVlanConfigInit(ImageHandle); 
#endif
  //Based on the Setup option Start the Driver   
  if(!CheckNetworkDriverLoadStatus()) {
      return EFI_UNSUPPORTED;
  }
  //
  // AMI PORTING END
...
/**
  The unload point for Mnp driver which uninstalls the driver binding and component
  name protocol on its ImageHandle.
  ...
  )
{
  ...
  //
    ...
    UINTN               Size = 0;
#if(AMIRedFishVlan_SUPPORT)
    EFI_HANDLE          RedfishVlanHandle;
    EFI_REDFISH_VLAN_CONFIG_PROTOCOL *RedfishVlanConfig;
    //
    //		Locate Redfish Vlan Config Protocol
    //   
    Status = gBS->LocateProtocol (&gEfiRedfishVlanConfigProtocolGuid, NULL, (void**)&RedfishVlanConfig);
    if (EFI_ERROR(Status))
    {
        goto ON_EXIT;
    }
	//		Locate Redfish Vlan Handle
    Status = gBS->LocateHandle (ByProtocol, &gEfiRedfishVlanConfigProtocolGuid, NULL, &Size, &RedfishVlanHandle);
    if(EFI_ERROR(Status))
    {
        goto ON_EXIT;
    }
    //
    // Uninstall Redfish Vlan Config protocol.
    //
    Status = gBS->UninstallMultipleProtocolInterfaces (
            RedfishVlanHandle,
