...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)	  
SkipMediaDetect= PcdGetEx8 (&gEfiNetworkPkgTokenSpaceGuid, PcdSkipMediaDetection);
if(SkipMediaDetect == 1)    
{
      Snp = MnpServiceData->MnpDeviceData->Snp;
      Snp->GetStatus (Snp, &InterruptStatus, NULL);
      if( Snp->Mode->MediaPresent == FALSE)
        {
            DEBUG((-1,"\n Media Not Present"));
