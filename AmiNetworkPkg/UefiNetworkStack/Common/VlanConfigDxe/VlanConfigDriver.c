...
AMI_NETWORK_BLOCKED  BlockedVlanBootNetworkList[] = {
...
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
      // Check if the Network device is present in the Vlan Boot Blocked List.
      // If present, don't install the Vlan boot support for the Network device.
      if(IsAmiNetworkDeviceBlocked(ControllerHandle, (AMI_NETWORK_BLOCKED*)BlockedVlanBootNetworkList)) {
