  ...
  )
{
  ...
  GuidIndex = 0;
  //APTIOV_OVERRIDE_START: Address AMISV_317
  // EIP#684109
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  while ((CertList!= NULL) && (ItemDataSize > 0) && (ItemDataSize >= CertList->SignatureListSize) &&
  ...
  // APTIOV_OVERRIDE_END
    ...
    // EIP#684109
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
    // SignatureSize should be greater enough to limit CertCount value
    if(CertList->SignatureSize > 1)
        CertCount  = (CertList->SignatureListSize - sizeof (EFI_SIGNATURE_LIST) - CertList->SignatureHeaderSize) / CertList->SignatureSize;

    if (CertCount <= 0) {
      DEBUG ((DEBUG_ERROR, "%a: no X509 certificates avaiable \n", __FUNCTION__));
      goto ON_EXIT;
    }
#else
    CertCount  = (CertList->SignatureListSize - sizeof (EFI_SIGNATURE_LIST) - CertList->SignatureHeaderSize) / CertList->SignatureSize;
