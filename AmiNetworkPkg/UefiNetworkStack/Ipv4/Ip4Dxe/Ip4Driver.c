...
BOOLEAN gSkipIpV4NetworkDeviceState = FALSE;
AMI_NETWORK_BLOCKED  BlockedIp4BootNetworkList[] = {
  ...
  )
{
 ...
 #if (NET_PKG_AMI_PORTING_ENABLE == 1)
  gSkipIpV4NetworkDeviceState = FALSE;
  if(!EFI_ERROR(Status)) {
      // Check if the Network device is present in the Ip4 Boot Blocked List.
      // If present, don't install the Ip4 boot support for the Network device.
      if(IsAmiNetworkDeviceBlocked(ControllerHandle, (AMI_NETWORK_BLOCKED*)BlockedIp4BootNetworkList)) {
          gSkipIpV4NetworkDeviceState = TRUE;
      }
   }
   #endif
