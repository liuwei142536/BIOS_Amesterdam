...
extern BOOLEAN gSkipIpV6NetworkDeviceState;
#endif
...
//APTIOV_OVERRIDE_START: Address AMISV_397 Security Fix
// EIP#682694
// AMI PORTING
//Check the Variable Record with Boundary to have secure check
#if (NET_PKG_AMI_PORTING_ENABLE)   
    if (EFI_ERROR (Status) || ((UINT16)(~NetblockChecksum ((UINT8 *)Variable, (UINT32)VarSize)) != 0) ||
            (Variable->DataRecordCount >= Ip6ConfigDataTypeMaximum)) {
#else
    if (EFI_ERROR (Status) || ((UINT16)(~NetblockChecksum ((UINT8 *)Variable, (UINT32)VarSize)) != 0)) {
#endif
//APTIOV_OVERRIDE_END
  ...
  )
{
  ...
  // 
#if (NET_PKG_AMI_PORTING_ENABLE == 1)
  if(gSkipIpV6NetworkDeviceState == TRUE)
  {
      return EFI_SUCCESS;
  }
#endif
