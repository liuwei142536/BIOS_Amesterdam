...
BOOLEAN gSkipIpV6NetworkDeviceState = FALSE;
AMI_NETWORK_BLOCKED  BlockedIp6BootNetworkList[] = {
  ...
  )
{
  ...
  gSkipIpV6NetworkDeviceState = FALSE;
  if(!EFI_ERROR(Status)) {
        // Check if the Network device is present in the Ip6 Boot Blocked List.
        // If present, don't install the Ip6 boot support for the Network device.
        if(IsAmiNetworkDeviceBlocked(ControllerHandle, (AMI_NETWORK_BLOCKED*)BlockedIp6BootNetworkList)) {
            gSkipIpV6NetworkDeviceState = TRUE;
        }
     }
