  ...
  if ((mPrivate != NULL) && (mPrivate->AttemptCount != 0)) {
    NET_LIST_FOR_EACH (Entry, &mPrivate->AttemptConfigs) {
      ...
      *(AttemptNameList + StringLen)     = L' ';
      ...
      IScsiConvertIsIdToString (IfrNvData->Keyword[Index].ISCSIIsId, SessionConfigData->IsId, 6);    // AMI PORTING.
  ...
  )
{
  ...
  //   if (Attempt->AuthenticationType == ISCSI_AUTH_TYPE_CHAP) { // AMI PORTING.
  ...
  if (IfrNvData->Enabled != ISCSI_DISABLED) {
    ...
    //
    // AMI PORTING - Get Target info via DHCP is failed if DnsMode is TRUE 
    //
  ...
  // AMI PORTING STARTS - RSD iSCSI Support.
#if (RSD_ISCSI_SUPPORT)
...
// AMI PORTING STARTS - RSD iSCSI Support.
  ...
  } else if (Action == EFI_BROWSER_ACTION_CHANGED && !RsdIScsiIsBmcDataAvailable()) {
#else
// AMI PORTING ENDS - RSD iSCSI Support.
