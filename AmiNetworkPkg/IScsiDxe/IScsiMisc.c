  ...
  )
{
  ...
  EFI_STATUS 		      	  RsdIScsiStatus = EFI_NOT_FOUND; //  AMI PORTING - RSA iSCSI Support.
...
// AMI PORTING STARTS - RSD iSCSI Support.
  ...
  if(!RsdIScsiIsBmcDataAvailable()) { 
      ...
      DEBUG((DEBUG_INFO,"\n RSD_ISCSI: RsdIScsiInitializeBmcInformation Status :: %r \n",RsdIScsiStatus));
  }
  
  // Check RSD BMC Data availability
  // Get AttemptConfigOrder
  if(RsdIScsiIsBmcDataAvailable())
  {
      // Use RSD interface to get the AttemptConfigOrder
      AttemptConfigOrder = IScsiGetVariableAndSize (
              L"RsdAttemptOrder",
...
#if (RSD_ISCSI_SUPPORT)
        // If BMC data is available, don't Update it in Setup.
        if(!RsdIScsiIsBmcDataAvailable())
        {
#endif  // RSD_ISCSI_SUPPORT
  //
