    ...
    if (AccessType == SetType){
        ...
        if ( Mgr->CheckStatus(Mgr, CMOS_IS_USABLE) ){
            if ( BitMap & CMOS_BAD_BATTERY ) {
                ...
                // Also set bad checksum
                Mgr->ManagerStatus |= CMOS_BAD_CHECKSUM;
    ...
    IN EFI_CMOS_ACCESS_INTERFACE       *ApiPointer )
{
    ...
    if ( !Mgr->CheckAnyStatus(Mgr, CMOS_IS_USABLE ) ){
        DEBUG((DEBUG_ERROR, "CMOS Not Usable: Could not save API pointer.\n"));
    ...
    EFI_CMOS_ACCESS_INTERFACE       **ApiPointer )
{
    ...
    if ( !Mgr->CheckAnyStatus(Mgr, CMOS_IS_USABLE ) ){
        DEBUG((DEBUG_ERROR, "CMOS Not Usable: Could not read API pointer.\n"));
        return EFI_DEVICE_ERROR;
    }
    ...
    IN EFI_PEI_SERVICES        **PeiServices )
{
    ...
    EFI_CMOS_IS_CMOS_USABLE       *IsCmosUsable;
    ...
    IsCmosUsable = NewManager->CmosIsUsable;

    while (*IsCmosUsable != NULL){
      if ( (*IsCmosUsable)( NewManager->Access.PeiServices ) ){
        NewManager->SetStatus(NewManager, CMOS_IS_USABLE);
      }
      else {
        NewManager->ClearStatus(NewManager, CMOS_IS_USABLE);
        DEBUG((DEBUG_INFO, "CMOS is not usable\n"));
      }
      ++IsCmosUsable;
    }
