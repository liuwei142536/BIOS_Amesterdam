...
//**********************************************************************
/** @file
  AMI specific library functions.
*/
...
EFI_STATUS LibInitStatusCodePtr()
{
    ...
    if (IsInSmm){
        if (!StatusCodeSmmPtr){
            ...
            &gLocalEfiSmmStatusCodeProtocolGuid, NULL, (VOID**)&StatusCodeSmmPtr
            );
        }
        return Status;
        ...
        Status = pBS->LocateProtocol(
            &gLocalEfiStatusCodeRuntimeProtocolGuid, NULL, (VOID**)&StatusCodePtr
        );
    }
    return Status;
}
...
EFI_STATUS TraceGetAmiDebugServicesProtocolInstance (VOID)
{
    ...
    if (IsInSmm)
    {
        ...
        Status = pSmst->SmmLocateProtocol(&gAmiSmmDebugServiceProtocolGuid, NULL, (VOID**)&AmiDebugServiceProtocolSmmPtr);
    ...
    else 
    {
        ...
        Status = pBS->LocateProtocol(&gAmiDebugServiceProtocolGuid, NULL, (VOID**)&AmiDebugServiceProtocol);
...
BOOLEAN GetImageName(EFI_HANDLE ImageHandle, CHAR8 *sName)
{
	...
	CHAR16 *wsName;
	EFI_STATUS Status;
	if (   !EFI_ERROR(Status = pBS->HandleProtocol(ImageHandle, &gLocalEfiLoadedImageProtocolGuid, (VOID**)&pImage))
	...
	for(i=0; !NameFound && i<Count; i++)
	{
		...
		for(j=0; j<InfoCount; j++)
		{
			...
			if (!EFI_ERROR(pBS->HandleProtocol(
				pInfo[j].AgentHandle, &gLocalEfiComponentName2ProtocolGuid, (VOID**)&pComponentName
			))){
			...
			//Try UEFI 2.0 ComponentName protocol
			if (!EFI_ERROR(pBS->HandleProtocol(
				pInfo[j].AgentHandle, &gLocalEfiComponentNameProtocolGuid, (VOID**)&pComponentName
			))){
    			if (!EFI_ERROR(pComponentName->GetControllerName(
...
{
	const EFI_DEVICE_PATH_PROTOCOL End = {END_DEVICE_PATH,END_ENTIRE_SUBTYPE, {sizeof(EFI_DEVICE_PATH_PROTOCOL),0}};
	BOOLEAN NameFound;
	EFI_HANDLE Parent;
	...
	if (EFI_ERROR(pBS->HandleProtocol(Controller, &guidDevicePath, (VOID**)&pDevicePath))) return FALSE;
  ...
  OUT UINT32                        *AuthenticationStatus)
{
	...
	{
		pBS->HandleProtocol(HandleBuffer[i], &guidFV, (VOID**)&Fv);
...
)
{
    ...
    if (pDp==NULL) return EFI_INVALID_PARAMETER;
...
//<AMI_PHDR_END>
//*************************************************************************
VOID EFIAPI TimerIsSet(EFI_EVENT Event, VOID *Context)
{
...
)
{
	...
	Status=pBS->HandleProtocol(ImageHandle, &gLocalEfiLoadedImageProtocolGuid, (VOID**)&pImage);
	if (EFI_ERROR(Status)) return Status;
	Status=pBS->HandleProtocol(pImage->DeviceHandle, &guidFV, (VOID**)&pFV);
	if (EFI_ERROR(Status)) return Status;
	do{
		pSectionGuid=NULL;
		DataSize=0;
		Status=pFV->ReadSection (
			...
			EFI_SECTION_FREEFORM_SUBTYPE_GUID,i++, (VOID**)&pSectionGuid, &DataSize,            // strcture MEDIA_FW_VOL_FILEPATH_DEVICE_PATH has
...
//--------------------------------
	...
	for(i=0; i<ResCount; i++){
		...
		if(Res->Attributes==-1) {
			...
			else if((UINTN)idsc.GcdIoType!=Res->ResType) Res->ResType=idsc.GcdIoType;
			...
			//If Space is not allocated but marked as different type of Space - use the one we got
			else if((UINTN)mdsc.GcdMemoryType!=Res->ResType) Res->ResType=mdsc.GcdMemoryType;
...
)
{
	...
	Status = pBS->LocateProtocol(&gLocalEfiSmmBase2ProtocolGuid, NULL, (VOID**)&pSmmBase);
...
//<AMI_PHDR_END>
//*************************************************************************
VOID EFIAPI DummyFunction(IN EFI_EVENT Event, IN VOID *Context) {}
...
VOID AmiFillFpdt (IN FPDT_FILL_TYPE FieldToFill){
    ...
    if (((PERF_TAB_HEADER*)(UINT8*)(UINTN)FpdtVar->BasBootPointer)->Signature != 0x54504246)
        return;
    BasBootPerRec = (BASIC_BOOT_PERF_REC*)((UINT8*)(UINTN)FpdtVar->BasBootPointer + sizeof(PERF_TAB_HEADER));
...
static VOID EFIAPI AmiLibExitBs (IN EFI_EVENT Event, IN VOID *Context){
    IsAtRuntime=TRUE;
    if (Context!=NULL) ((EFI_EVENT_NOTIFY)(UINTN)Context)(Event,NULL);
...
static VOID EFIAPI AmiLibGoVirtual (IN EFI_EVENT Event, IN VOID *Context){
    ...
    if (--NumberOfGoVirtualCallbacks==0){
        ...
        LibTempCopyOfpRs->ConvertPointer(0, (VOID**)&pST);
        LibTempCopyOfpRs->ConvertPointer(0, (VOID**)&pRS);
        if (StatusCodePtr != NULL) LibTempCopyOfpRs->ConvertPointer(0, (VOID**)&StatusCodePtr);
        if (AmiDebugServiceProtocol != NULL) LibTempCopyOfpRs->ConvertPointer(0, (VOID**)&AmiDebugServiceProtocol);
    }
}
...
VOID EFIAPI InitAmiRuntimeLib(
	...
	pBS->CreateEvent(
        ...
        &AmiLibExitBs, (VOID*)(UINTN)ExitBsCallback, &Event
    );
    pBS->CreateEvent(
        EVT_SIGNAL_VIRTUAL_ADDRESS_CHANGE,TPL_CALLBACK, 
        &AmiLibGoVirtual, (VOID*)(UINTN)GoVirtualCallback, &Event
    );
}
