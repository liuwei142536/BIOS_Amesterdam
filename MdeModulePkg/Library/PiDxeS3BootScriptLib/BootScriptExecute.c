...
static AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR *gMemoryRangesStructure = NULL;
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  DEBUG_CODE(
	  ...
	  for (i = 0; i < gMemoryRangesStructure->NumberOfMemoryRanges; i ++) {
	          if (    OriginalAddress >= gMemoryRangesStructure->MemRanges[i].Address
	               && OriginalAddress < gMemoryRangesStructure->MemRanges[i].Address + gMemoryRangesStructure->MemRanges[i].Size
	          ){
	              GoodMemory = FALSE; // Buffer starts in the illegal region
	          } else if (    OriginalAddress < gMemoryRangesStructure->MemRanges[i].Address
	               && OriginalAddress+Length > gMemoryRangesStructure->MemRanges[i].Address
	          ){
  ...
  )
{
  ...
  // Improvement. Make sure boot script does not overwrite OS-owned memory.
  DEBUG_CODE(
    gMemoryRangesStructure = (AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR*)(UINTN)mS3BootScriptTablePtr->MemoryRangesStructure;
    ASSERT (gMemoryRangesStructure != NULL);
  );
  //*** AMI PORTING END *****//
