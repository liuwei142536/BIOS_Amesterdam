  ...
  )
{
  ...
  DEBUG_CODE_BEGIN ();
	    ...
	    EFI_GUID    mAmiMemoryRangesStructureGuid = { 0x6f8d5d48, 0xa285, 0x44e8, { 0xaf, 0x6a, 0xd, 0x4, 0x29, 0x51, 0x13, 0xc6 } };
	    EFI_GUID	mAmiMemoryRangesBufferGuid = { 0xf754f36f, 0x1767, 0x414e, { 0x8a, 0xf2, 0x8b, 0xd4, 0x99, 0xab, 0x6e, 0x2d } };
	    AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR *MemoryRangesStructure = NULL;
	    
	    mS3BootScriptTablePtr->MemoryRangesStructure = NULL;
	    Size = 0;
	    ...
	    if (EFI_ERROR(Status)){
	        DEBUG ((EFI_D_ERROR, "ERROR in SaveBootScriptDataToLockBox: AllocatePages MemoryRangesStructure failed with %r Status !!!\n", Status));
	    	ASSERT_EFI_ERROR(Status);
	    }
	    MemoryRangesStructure = (VOID*)(UINTN)Buffer;
	    MemoryRangesStructure->NumberOfMemoryRanges = 0;
	    for(   Descriptor = MemMap
	        ...
	        ) MemoryRangesStructure->NumberOfMemoryRanges ++;
	    }
	    Buffer = SIZE_4GB - 1;
	    Status = gBS->AllocatePages (
	    	                        AllocateMaxAddress,
	    	                        EfiReservedMemoryType,
	    	                        EFI_SIZE_TO_PAGES(MemoryRangesStructure->NumberOfMemoryRanges * sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE)),
	    	                        &Buffer
	    	                        );
	    
	    if (EFI_ERROR(Status)){
	        DEBUG ((EFI_D_ERROR, "ERROR in SaveBootScriptDataToLockBox: AllocatePages MemoryRangesStr2  %r Status !!!\n", Status));
	    	ASSERT_EFI_ERROR(Status);
	    }
	    
	    MemoryRangesStructure->MemRanges = (VOID *)(UINTN)Buffer;
	   
	    if (MemoryRangesStructure->MemRanges == NULL){
	    	MemoryRangesStructure->NumberOfMemoryRanges  = 0;
	        DEBUG ((EFI_D_ERROR, "ERROR: No memory to save MemMap !!!\n"));
	        ASSERT (MemoryRangesStructure->MemRanges != NULL);
	    }
	    for(   Descriptor = MemMap, i=0
	        ...
	        if (    Descriptor->Type != EfiRuntimeServicesData && Descriptor->Type != EfiRuntimeServicesCode
	            ...
	            MemoryRangesStructure->MemRanges[i].Address = Descriptor->PhysicalStart;
	            MemoryRangesStructure->MemRanges[i].Size = EFI_PAGES_TO_SIZE((UINTN)Descriptor->NumberOfPages);
	            i++;
	        }
	    }
	    ASSERT(i==MemoryRangesStructure->NumberOfMemoryRanges );
	    gBS->FreePool(MemMap);
	    Status = SaveLockBox (
	               &mAmiMemoryRangesStructureGuid,
	               (VOID *)(UINTN)MemoryRangesStructure,
	               sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE_STR)
	               );
	    ASSERT_EFI_ERROR (Status);
	    
	    Status = SetLockBoxAttributes (&mAmiMemoryRangesStructureGuid, LOCK_BOX_ATTRIBUTE_RESTORE_IN_PLACE);
	    ASSERT_EFI_ERROR (Status);
	    
	    Status = SaveLockBox (
	    	               &mAmiMemoryRangesBufferGuid,
	    	               (VOID *)(UINTN)&MemoryRangesStructure->MemRanges,
	    	               MemoryRangesStructure->NumberOfMemoryRanges * sizeof(AMI_BUFFER_VALIDATION_MEM_ADDRESS_RANGE)
	    	               );
	    ASSERT_EFI_ERROR (Status);
	   
	    Status = SetLockBoxAttributes (&mAmiMemoryRangesBufferGuid, LOCK_BOX_ATTRIBUTE_RESTORE_IN_PLACE);
	    ASSERT_EFI_ERROR (Status);
	    mS3BootScriptTablePtr->MemoryRangesStructure = (VOID*)(UINTN)MemoryRangesStructure;
  DEBUG_CODE_END ();
  //*** AMI PORTING END *****//
}
  ...
  )
{
  ...
  return InternalMemCopyMem_MMIO (DestinationBuffer, SourceBuffer, Length);
}
  ...
  )
{
  ...
  CopyMem_MMIO ((VOID*)Script, (VOID*)&ScriptMemWrite, sizeof(EFI_BOOT_SCRIPT_MEM_WRITE));
  CopyMem_MMIO ((VOID*)(Script + sizeof (EFI_BOOT_SCRIPT_MEM_WRITE)), Buffer, WidthInByte * Count);
