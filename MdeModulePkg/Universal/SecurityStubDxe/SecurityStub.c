  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // Support AMI image type detection policy.
  // Original implementation assumes that all the images that are 
  // part of system firmware are getting loaded from FV device path.
  // This is not always the case in Aptio.
  // For example, embedded OpROM images are loaded from different device path.
  if (FileBuffer != NULL && AmiGetImageType(File, FileBuffer, FileSize, BootPolicy) != AMI_IMAGE_FROM_FV) {
  //if (FileBuffer != NULL) {
  //*** AMI PORTING END *****//      
