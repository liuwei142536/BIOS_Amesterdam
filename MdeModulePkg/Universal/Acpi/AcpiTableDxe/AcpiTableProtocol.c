...
#include "AcpiTable.h"
//----AMI PORTING BEGIN ----
//---Support for AO3 without Reboot added
...
BOOLEAN gDoNotIncreaseTblNumber = FALSE;
EFI_OA3_MSDM_STRUCTURE gMsdmVariable;
#endif

//---Hooks on Install Table
typedef EFI_STATUS (ACPI_OEM_FUNCTION)(IN OUT EFI_ACPI_SDT_HEADER *AcpiHdr);
...
EFI_STATUS OemAcpiSetPlatformId(IN OUT EFI_ACPI_SDT_HEADER *AcpiHdr)
{
...
//---Support for AO3 without Reboot added 
#if defined(OemActivation_SUPPORT) && (OemActivation_SUPPORT == 1)
  if (gDoNotIncreaseTblNumber)
	  NewMaxTableNumber = mEfiAcpiMaxNumTables;
  else
#endif
//----AMI PORTONG BEGIN ----
  ...
  if (gOA3Variable)
  {
	  ...
	  DEBUG((EFI_D_ERROR,"ReallocateAcpiTableBuffer for EfiACPIMemoryNVS: Status=%r\n", Status));
  }
  else
#endif
//----AMI PORTONG END ----	 
...
//---Support for AO3 without Reboot added
  ...
  if (!gOA3Variable)
  {
      ...
      {
    	  DEBUG((EFI_D_ERROR,"MSDM_VARIABLE found: Status=%r\n", Status));
          gOA3Variable = TRUE;
              
          //-- Relocating RSDP into EfiACPIMemoryNVS
          {
        	  UINT64                CurrentData;
        	  UINTN                 RsdpTableSize;
        	  UINT8                 *Pointer;
        	  ...
        	  if (TempPrivateData.Rsdp1 != NULL)
        		  gBS->FreePages ((EFI_PHYSICAL_ADDRESS)(UINTN)TempPrivateData.Rsdp1, EFI_SIZE_TO_PAGES (RsdpTableSize));
        	  else 
        		  gBS->FreePages ((EFI_PHYSICAL_ADDRESS)(UINTN)TempPrivateData.Rsdp3, EFI_SIZE_TO_PAGES (RsdpTableSize));
              }
              
          gDoNotIncreaseTblNumber = TRUE; 
          Status = ReallocateAcpiTableBuffer (AcpiTableInstance);
          ASSERT_EFI_ERROR (Status);
          gDoNotIncreaseTblNumber = FALSE; 

          gMsdmVariable.XsdtAddress = (EFI_PHYSICAL_ADDRESS) AcpiTableInstance->Xsdt;
          DEBUG((EFI_D_ERROR,"MSDM_VARIABLE XsdtAddress %lX\n", gMsdmVariable.XsdtAddress));
  ...
  //----Call hook on Install Table -----
  Status = OemAcpiSetPlatformId ((EFI_ACPI_SDT_HEADER*) Table);
  
  if (Status != EFI_UNSUPPORTED) //Hook modified table - we need to checksum it
	  Checksum = TRUE;
//----AMI PORTING END ---- 
  //
  // Init locals
