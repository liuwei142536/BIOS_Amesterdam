...
// 
// Flag to keep track of BIOS Guard being enabled
//
BOOLEAN     PfatEnabled = FALSE;
//
  ...
  )
{
  ...
  if (PfatEnabled) {
    if (EfiAtRuntime ()) { 
      ...
      //
      // Bios Guard is enabled, so just pass on the pointer to the BG update package
      //
  ...
  )
{
  ...
  if ((AsmReadMsr64 (EFI_PLATFORM_INFORMATION)) & B_MSR_PLATFORM_INFO_PFAT_AVAIL) {
    if ((AsmReadMsr64 (MSR_PLAT_FRMW_PROT_CTRL)) & B_MSR_PLAT_FRMW_PROT_CTRL_EN) {
      //
      // BIOS Guard is enabled so we need to skip the Capsule Header checks
      // Also check to make sure file passed in is proper BG Update package
      //
      PfatEnabled = TRUE;
      CapsuleHeader = CapsuleHeaderArray[0];
