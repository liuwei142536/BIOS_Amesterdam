  ...
  )
{
  ...
  for (Link = Head->ForwardLink; Link != Head; Link = Link->ForwardLink) {
    ...
    //*** AMI PORTING BEGIN ***//
    DEBUG((DEBUG_EVENT, "Calling SMI Handler %p; Handle = 0x%X\n", SmiHandler->Handler, SmiHandler));
    //*** AMI PORTING END *****//
    ...
    //*** AMI PORTING BEGIN ***//
    DEBUG((DEBUG_EVENT, "SMI Handler exit status: %r\n", Status));
    //*** AMI PORTING END *****//
...
//*** AMI PORTING BEGIN ***//
  DEBUG((DEBUG_EVENT, "Registering SMI Handler %p; Handle = 0x%X\n", SmiHandler->Handler, SmiHandler));
//*** AMI PORTING END *****//
...
//*** AMI PORTING BEGIN ***//
  DEBUG((DEBUG_EVENT, "Unregistering SMI Handler %p; Handle = 0x%X\n", SmiHandler->Handler, SmiHandler));
//*** AMI PORTING END *****//
  FreePool (SmiHandler);
