            ...
            if (!EFI_ERROR (Status)) {
              for (OpenInfoIndex = 0; OpenInfoIndex < OpenInfoCount; OpenInfoIndex++) {
                if (OpenInfo[OpenInfoIndex].AgentHandle == Image->Handle) {
                  //*** AMI PORTING BEGIN ***
                  // Minor improvements: unused Status assignment is removed.
...
// ACPI FPDT support. Set a flag to report OS loader start in CoreStartImage
  if (BootPolicy){
    gFillFpdt = TRUE;
  }
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // ACPI FPDT support. Report OS loader start. 
  // The status code is processed by ACPI module.
  // PcdProgressCodeOsLoaderLoad status code is reported by TSE.
  if (gFillFpdt) {
