...
#include <Guid/AmiBeforeExitBootServicesEvent.h> // Before-exit-boot-services event support

VOID VeiryMemoryMapKey(IN UINTN MapKey); // See AMI porting comments in CoreExitBootServices
//*** AMI PORTING END *****//
...
//Only call NotifySignalList() if this is first call to ExitBootServices()
  if (FirstCall) {
    ...
    CoreNotifySignalList (&gAmiBeforeExitBootServicesEventGuid);
//*** AMI PORTING END *****//
  //
  // Disable Timer
  //
