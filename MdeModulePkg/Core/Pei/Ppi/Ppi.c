...
{
  //*** AMI PORTING BEGIN ***//
  // Bug fix: PpiData.PpiList.CurrentCount is defined as UINTN. In the original code Index overflows if PPI count is 256 or higher.
  //UINT8                 Index;
  UINTN                 Index;
  //*** AMI PORTING END ***//
  ...
  )
{
  ...
  for (;;) {
    ...
    DEBUG((DEBUG_EVENT, "Notify Function: %p\n", NotifyList->Notify));
    //*** AMI PORTING END *****//
    if (Single) {
      //
