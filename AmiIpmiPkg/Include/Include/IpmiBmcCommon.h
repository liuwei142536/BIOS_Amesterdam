...
//
// Contains fields related to retry count for specific command.
//
typedef struct {
  UINT8     NetFunction;   // Network function code
  UINT8     Command;       // Command number
  union {
  UINT32    BtDelay;       // Retry counter for Bt interface.
  UINT16    RetryCounter;  // Retry counter for Kcs interface.
  };
} COMMAND_SPECIFIC_RETRY_COUNT;
#endif
