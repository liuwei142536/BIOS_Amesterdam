  ...
  IN EFI_MUX_SETTING    MuxSetting )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entry... MuxSetting: %x \n", __FUNCTION__, MuxSetting));
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8 *) &MuxConfig,
                (UINT8 *) &Size );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_TRANSPORT_SET_SERIAL_MUX Status: %r CommandStatus: %x \n", Status, MuxConfig.CommandStatus));
    ...
    if (!EFI_ERROR(Status) && (MuxConfig.CommandStatus == REQUEST_REJECTED)) {
        ...
        while (RetryCount++ < 5) {
            ...
            Status = gIpmiTransport->SendIpmiCommand (
                        ...
                        (UINT8 *) &MuxConfig,
                        (UINT8 *) &Size );
            SERVER_IPMI_DEBUG ((DEBUG_INFO, " RetryCount: %x IPMI_TRANSPORT_SET_SERIAL_MUX Status: %r CommandStatus: %x \n", RetryCount, Status, MuxConfig.CommandStatus));
  ...
  IN VOID             *Context )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                &ResponseSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_CHASSIS_GET_SYSTEM_BOOT_OPTIONS Status: %r\n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " BootFlagValid: %x BiosMuxCtrlOverride: %x \n", BootFlagsResponse.Param5.BootFlagValid, BootFlagsResponse.Param5.BiosMuxCtrlOverride));
  ...
  VOID )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8 *) &ResponseData,
                (UINT8 *) &Size );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_APP_GET_CHANNEL_ACCESS Status: %r AccessMode: %x \n", Status, ResponseData.AccessMode));
    if (EFI_ERROR (Status)) {
  ...
  VOID )
{
    ...
    for (ChannelNumber = 0; ChannelNumber < MAX_IPMI_CHANNELS; ChannelNumber++) {
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " Channel Number: %x \n", ChannelNumber));
        //
        ...
        Status = gIpmiTransport->SendIpmiCommand (
                    ...
                    (UINT8 *) &ChannelInfo,
                    (UINT8 *) &Size );
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_APP_GET_CHANNEL_INFO Status: %r ChannelMediumType: %x \n", Status, ChannelInfo.ChannelMediumType));
        if (EFI_ERROR (Status)) {
            continue;
        }
        //
        ...
        if(ChannelInfo.ChannelMediumType == EfiSmRs_232){
            ...
            SERVER_IPMI_DEBUG ((DEBUG_INFO, " Serial Channel Number: %x \n", gSerialChannel));
            break;
        }
    } //for loop
    return EFI_SUCCESS;
  ...
  IN EFI_SYSTEM_TABLE   *SystemTable )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " ServerMgmtConfiguration.SerialMux: %X \n", ServerMgmtConfiguration.SerialMux));
