...
COMMAND_SPECIFIC_RETRY_COUNT gCommandSpecificRetryCountTable [] = {
    COMMAND_SPECIFIC_RETRY_COUNT_TABLE_LIST
};
#endif
  ...
  IN UINT8                  Command )
{
    ...
    for (Index = 0; Index < TableSize-1; Index++) {
      if (NetFunction == gCommandSpecificRetryCountTable[Index].NetFunction &&\
          ...
          DEBUG((DEBUG_VERBOSE,"Counter value changed\n"));
#if(IPMI_SYSTEM_INTERFACE == KCS_INTERFACE)
          return gCommandSpecificRetryCountTable[Index].RetryCounter;
#endif
#if(IPMI_SYSTEM_INTERFACE == BT_INTERFACE)
          return gCommandSpecificRetryCountTable[Index].BtDelay;
#endif
      }
    }
    DEBUG((DEBUG_VERBOSE,"Counter value not changed as there is no match\n"));
#if(IPMI_SYSTEM_INTERFACE == KCS_INTERFACE)
  ...
  IN VOID                             *Context )
{
    ...
    DEBUG((DEBUG_VERBOSE,"Counter value is %x\n",IpmiInstance->Interface.BT.BtDelay));
#endif
  ...
  IN  VOID                            *Context )
{
    ...
    DEBUG ((DEBUG_VERBOSE, "Get Device ID Command: Status %r\n", Status));
    //
