...
EFI_SMBUS_HC_PROTOCOL           *gEfiSmbusHcProtocol = NULL;
UINT8                           gPublishSpmi = TRUE;
VOID                            *gRegistration;
  ...
  OUT  CHAR16      TimeZone[] )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entered... TimeZone: %S \n", __FUNCTION__, TimeZone));
  ...
  IN  EFI_IPMI_BMC_INSTANCE_DATA    *gIpmiInstance )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO,"SetupUtcOffset: %x \n", SetupUtcOffset));
    ...
    Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                ...
                (UINT8*) &(BmcUtcOffset),
                &DataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "EFI_STORAGE_GET_SEL_TIME_UTC_OFFSET status %r\n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    SERVER_IPMI_DEBUG ((DEBUG_INFO,"BmcUtcOffset: %x \n", BmcUtcOffset));
    //
    ...
    if ((SetupUtcOffset == BmcUtcOffset) || \
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO,"Set SEL Time UTC Offset Command is not send to BMC as SetupUtcOffset = BmcUtcOffset/0x7FF.\n"));
        return EFI_SUCCESS;
    }
    //
    ...
    Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                ...
                NULL,
                &DataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "EFI_STORAGE_SET_SEL_TIME_UTC_OFFSET status %r\n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
  ...
  IN  EFI_IPMI_BMC_INSTANCE_DATA    *gIpmiInstance )
{
    ...
    Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                       ...
                       &DataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "EFI_STORAGE_GET_SEL_TIME_UTC_OFFSET status = %r  Value = %x\n", Status, CurrentBmcUtcOffset));
    ...
    if ( CurrentBmcUtcOffset != EFI_UNSPECIFIED_TIMEZONE) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "gRT->GetTime status %r\n", Status));
        if (EFI_ERROR (Status)) {
            return Status;
        }
        //
        ...
        if (CurrentBmcUtcOffset != (Time.TimeZone * -1)) {
            ...
            SERVER_IPMI_DEBUG ((DEBUG_INFO, "gRT->SetTime status %r\n", Status));
            if (EFI_ERROR (Status)) {
                return Status;
            }
        }
    ...
    Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                ...
                (UINT8*)&SelTime,
                &DataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI_STORAGE_GET_SEL_TIME status %r\n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    ...
    //
    Status = gRT->GetTime (&Time, NULL);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "GetTime status of BIOS %r\n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    //
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "CurrentBmcTime %x NumOfSeconds: %x \n", CurrentBmcTime, NumOfSeconds));
    if(CurrentBmcTime < NumOfSeconds - 2 || CurrentBmcTime > NumOfSeconds + 2) {
        ...
        Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                    ...
                    NULL,
                    &DataSize );
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Status of Platform Event Message command= %r\n", Status));
#else
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Time stamp - First Event pair IPMI_STORAGE_ADD_SEL_ENTRY status %r\n", Status));
        ...
        Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                    ...
                    NULL,
                    &DataSize );
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI_STORAGE_SET_SEL_TIME status %r\n", Status));
        //
        ...
        Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                    ...
                    &DataSize );
#endif
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Time stamp - second Event pair IPMI_STORAGE_ADD_SEL_ENTRY status %r\n", Status));
        ...
        if (CurrentBmcUtcOffset != EFI_UNSPECIFIED_TIMEZONE) {
            ...
            Status = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                                ...
                                NULL,
                                &DataSize );
            SERVER_IPMI_DEBUG ((DEBUG_INFO, "EFI_STORAGE_SET_SEL_TIME_UTC_OFFSET status = %r  Value = %04x\n", Status,CurrentBmcUtcOffset));
        }
    }
...
/**
    ...
    @return  EFI_STATUS  Return status of the Get Device Id command.
  ...
  IN OUT  UINT8 *ResponseDataSize )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "Get Device ID status %r\n", Status));
  ...
  IN  VOID              *Context )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "gBS->LocateProtocol gEfiRealTimeClockArchProtocolGuid protocol  status %r\n", Status));
    if (EFI_ERROR (Status)) {
        return;
    }
...
GetGlobalEnables ()
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "Get BMC Global Enable status %r\n", Status));
    if (!EFI_ERROR(Status)) {
        //
...
/**
    ...
    IpmiInstance structure. Install the BmcSelfTestLogProtocol with Status
    codes based on BmcStatus. Installs the DxeIpmiTransportProtocol if
    status is not Hard fail and at-last create event for virtual address change.
...
**/
EFI_STATUS
CheckSelfTestAndInstallDxeIpmiProtocol (VOID)
{
    ...
    UINT8                               ErrorBitVal = 0;
    UINT8                               DataSize;
    static EFI_BMC_SELF_TEST_LOG_PROTOCOL      *BmcSelfTestProtocol = NULL;
    EFI_STATUS                          StatusOfGetDeviceId = EFI_DEVICE_ERROR;
    EFI_EVENT                           EfiVirtualAddressChangeNotifyEvent = NULL;
    EFI_HANDLE                          NewHandle = NULL;
    EFI_STATUS                          Status = EFI_UNSUPPORTED;
    static BOOLEAN                      IsSelfTestProtocolInstalled = FALSE;
    UINT8                               ResponseDataSize;
    IPMI_SELF_TEST_RESULT_RESPONSE      BstStatus;
    EFI_STATUS                          SelfTestStatus = EFI_SUCCESS;
    static UINT8                        StatusCodeIndex = 0;
    ...
    for (Index = 0; Index < IPMI_SELF_TEST_COMMAND_RETRY_COUNT; Index++) {
        ...
        SelfTestStatus = gIpmiInstance->IpmiTransport.SendIpmiCommand (
                                                        &gIpmiInstance->IpmiTransport,
                                                        IPMI_NETFN_APP,
                                                        BMC_LUN,
                                                        ...
                                                        (UINT8*)&BstStatus,
                                                        &ResponseDataSize);
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Get Self test results status %r\n", SelfTestStatus));
        if (!EFI_ERROR (SelfTestStatus)) {
            break;
        }
    }
    DEBUG ((DEBUG_INFO, "Self test result Status: %r\n", SelfTestStatus));
    
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IsSelfTestProtocolInstalled %d\n", IsSelfTestProtocolInstalled));
    //
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "BMC status %x\n", gIpmiInstance->BmcStatus));

    for (Index = StatusCodeIndex; Index < ErrorCount; Index++) {
       ReportStatusCode (
    ...
    if (!IsSelfTestProtocolInstalled) {
        //
        // Install BmcSelfTestLogProtocol protocol.
        ...
        if (BmcSelfTestProtocol == NULL) {
            SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for BmcSelfTestProtocol! \n"));
            ...
            Status = gBS->InstallProtocolInterface (
                                ...
                                BmcSelfTestProtocol);
            SERVER_IPMI_DEBUG ((DEBUG_INFO, "Install BMC self test protocol status %r\n", Status));
            ...
            if (EFI_ERROR (Status)) {
                ...
                IsSelfTestProtocolInstalled = TRUE;
            }
        }
    } else {
        //
        // If BmcSelfTestLogProtocol is already installed, then update the BmcStatus and status codes.
        //
        if (BmcSelfTestProtocol != NULL) {
            //
            // GetDeviceId command succeed, Copy the BMC details into self test protocol to display it in setup
            ...
            BmcSelfTestProtocol->BmcStatus = gIpmiInstance->BmcStatus;
            if ((gIpmiInstance->BmcStatus != EFI_BMC_OK)) {
                CopyMem (&BmcSelfTestProtocol->BmcSelfTestError[ErrorCount-1], &StatusCodeValue[ErrorCount-1], sizeof (EFI_STATUS_CODE_VALUE));
                BmcSelfTestProtocol->BmcSelfTestErrorCount = ErrorCount;
            }
        }
    }
    ...
    if (gIpmiInstance->BmcStatus != EFI_BMC_HARDFAIL) {
        ...
        Status = gBS->InstallProtocolInterface (
                        ...
                        EFI_NATIVE_INTERFACE,
                        &gIpmiInstance->IpmiTransport);
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Install gEfiDxeIpmiTransportProtocolGuid protocol status %r\n", Status));
        if (EFI_ERROR (Status)) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Create event for the Virtual Address Change Event  status %r\n", Status));
    }
...
/**
    ...
    is found and install the DxeIpmiTransportProtocol and return the Status accordingly.
...
LocateAndFindSmbusInstance (VOID)
{
    ...
    for (Index = 0; Index < HandleCount; Index++) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Index: %d HandleProtocol Status %r\n", __FUNCTION__, Index, Status));
        if (!EFI_ERROR (Status)) {
            //
            ...
            GetSystemInterfaceCapability ();
            GetGlobalEnables ();
            Status = CheckSelfTestAndInstallDxeIpmiProtocol ();
            if (!EFI_ERROR (Status)) {
                return Status;
            }
        }
...
/**
    Notify function for SmbusHcProtocol. Locates SmbusHcProtocol with registration
    key and sends Get Self Test Results command. If the command is success, then
    SSIF BMC instance is found and install the DxeIpmiTransportProtocol and
    closes the event.
...
)
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a LocateProtocol for gEfiSmbusHcProtocolGuid Status %r\n", __FUNCTION__, Status));
    if (!EFI_ERROR(Status)) {
        //
        // Check SSIF interface capability
        //
...
/**
    ...
    @retval EFI_UNSUPPORTED       ServerSetup variable is found and BmcSupport is
                                  ...
                                  installing DxeIpmiTransportProtocol.
    @retval EFI_OUT_OF_RESOURCES  Failed to allocate requested memory.
    @retval Others                Return status from other functions used.
  ...
  IN EFI_SYSTEM_TABLE       *SystemTable )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "GetVariable status of ServerSetup %r ServerMgmtConfiguration.BmcSupport: %x \n", gSetupStatus, gServerMgmtConfiguration.BmcSupport));
    ...
    if (gIpmiInstance == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for gIpmiInstance! \n"));
    ...
    if (IPMI_DEFAULT_HOOK_SUPPORT) {
        ...
        if (EFI_ERROR (Status)) {
            ...
            //
            FreePool (gIpmiInstance);
            SERVER_IPMI_DEBUG ((DEBUG_ERROR, "IpmiOemDxeInitHook is failed and Status is %r.\n", Status));
            return Status;
        }
#endif
    }
    ...
    if (IPMI_SYSTEM_INTERFACE == KCS_INTERFACE) { // KCS Interface
        ...
        if (EFI_ERROR (Status)) {
            ...
            SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a EfiNamedEventListen Status: %r\n", __FUNCTION__, Status));
            if (EFI_ERROR (Status)) {
                FreePool (gIpmiInstance);
                return EFI_UNSUPPORTED;
            }
        ...
        gPublishSpmi = FALSE; // SPMI table does not provide any information about IPMB.
    }
