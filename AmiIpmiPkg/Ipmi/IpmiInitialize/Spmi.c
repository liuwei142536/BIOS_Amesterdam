...

{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI:Getting DSDT\n"));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI:Updating SRVV ASL Object\n"));
   ...
   IN  VOID          *Context )
{
    ...
    if (EFI_ERROR (Status)) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "IpmiOemSpmiUpdateHook is failed and Status is %r and initializing unspecified values.\n", Status));
    ...
    VOID )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI: Adding PublishSpmiAcpiTables to gEfiAcpiTableGuid event group, Status: %r\n", Status));
    ASSERT_EFI_ERROR(Status); 
