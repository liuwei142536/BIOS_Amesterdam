  ...
  VOID )
{
    ...
    Status = gBS->LocateProtocol (
                 ...
                 NULL,
                 (VOID **)&gIpmiTransport);
    SERVER_IPMI_DEBUG((DEBUG_INFO, "LocateProtocol gEfiDxeIpmiTransportProtocolGuid. Status: %r\n", Status));
    if ( EFI_ERROR (Status) ) {
    ...
    //
CallOriginalHook:
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Exiting..... User by pass password bit is not set so calling Original Hook \n", __FUNCTION__));
