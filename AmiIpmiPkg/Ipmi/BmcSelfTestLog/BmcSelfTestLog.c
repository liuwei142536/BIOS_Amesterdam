  ...
  IN UINT16             Class )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "gRT->GetVariable ServerSetup status %r \n", Status));
    ...
    Status = gRT->GetVariable (
                    ...
                    &Size,
                    &gBstLog );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "gRT->GetVariable BstLogVar status %r \n", Status));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "gBstLog.logSize:  %x \n", gBstLog.logSize));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "LocateProtocol gEfiBmcSelfTestLogProtocolGuid: Status:  %r\n", Status));

    // 
    // Displaying the string PASSED or FAILED depending upon the Protocol locate status and Error count
    //
    if ((!EFI_ERROR (Status)) && (BmcSelfTestProtocol->BmcStatus == EFI_BMC_OK)) {
        CharPtrString = HiiGetString (HiiHandle, STRING_TOKEN(STR_PROTOCOL_LOCATE_STATUS_PASSED), NULL);
    } else {
        CharPtrString = HiiGetString (HiiHandle, STRING_TOKEN(STR_PROTOCOL_LOCATE_STATUS_FAILED), NULL);
    }
    ...
    if ((!EFI_ERROR (Status)) && BmcSelfTestProtocol->IsGetDeviceIdCommandPassed) {
        ...
        if (CharPtrString != NULL){
            ...
            // BMC Device ID
            //
            UnicodeSPrint(CharPtrString, STRING_BUFFER_LENGTH,L"%d",BmcSelfTestProtocol->GetDeviceIdResponse.DeviceId);
            HiiSetString (
                HiiHandle,
            ...
            // BMC Device Revision
            //
            UnicodeSPrint(CharPtrString, STRING_BUFFER_LENGTH,L"%x",BmcSelfTestProtocol->GetDeviceIdResponse.DeviceRevision);
            HiiSetString (
                HiiHandle,
            ...
            //
            UnicodeSPrint(CharPtrString,STRING_BUFFER_LENGTH,L"%x.%02x", BmcSelfTestProtocol->GetDeviceIdResponse.MajorFirmwareRev, BmcSelfTestProtocol->GetDeviceIdResponse.MinorFirmwareRev);
            HiiSetString(
                HiiHandle,
            ...
            // IPMI Version
            //
            UnicodeSPrint(CharPtrString,STRING_BUFFER_LENGTH,L"%x.%x", ((BmcSelfTestProtocol->GetDeviceIdResponse.SpecificationVersion)& 0x0F), ((BmcSelfTestProtocol->GetDeviceIdResponse.SpecificationVersion) >> 0x04));
            HiiSetString (
                HiiHandle,
    ...
    if (EFI_ERROR (Status)) {
        ...
        if (LogIndex < BST_LOG_SIZE) {
            ...
            UpdateTime (LogIndex);
#endif
            SERVER_IPMI_DEBUG ((DEBUG_INFO, "gBstLog.log[%x]:  %x \n", LogIndex, gBstLog.log[LogIndex]));
            LogIndex = LogIndex + 1;
        }
    } else {
        //
    ...
    Status = gRT->SetVariable (
                ...
                &gBstLog );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "gRT->SetVariable BstLogVar status %r \n", Status));

    //
    // Add/log BmcSelfTest status codes in setup
    // and also update Log area usage in setup
    //
    CharPtrString = HiiGetString (HiiHandle, STRING_TOKEN(STR_LOG_AREA_USAGE), NULL);
    if (CharPtrString != NULL) {
        CharPtrString1 = AllocateZeroPool (STRING_BUFFER_LENGTH);
        if (CharPtrString1 != NULL){
            UnicodeSPrint(CharPtrString1, STRING_BUFFER_LENGTH, CharPtrString, gBstLog.logSize);
            HiiSetString (
                HiiHandle,
    ...
    if (!gBstLog.logSize) {
        CharPtrString = HiiGetString (HiiHandle, SetupStrings[1], NULL);
        if (CharPtrString != NULL) {
            HiiSetString  (
                HiiHandle,
                StringRef,
        ...
        //
#if TSE_MULTILINE_CONTROLS == 1
        CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_DATE_TIME_STATUSCODE), NULL);
        if (CharPtrString != NULL) {
            HiiSetString (
                HiiHandle,
        ...
        for(Index = 0; Index < gBstLog.logSize ; Index++, StringRef++) {
            ...
            //
            CharPtrString = HiiGetString ( HiiHandle, SetupStrings[gBstLog.log[Index]], NULL);
#if TSE_MULTILINE_CONTROLS == 1
            if (CharPtrString != NULL) {
                CharPtrString1 = AllocateZeroPool (STRING_BUFFER_LENGTH);
                if (CharPtrString1 != NULL){
                    UnicodeSPrint(CharPtrString1,STRING_BUFFER_LENGTH,L"%02d/%02d/%02d  %02d:%02d:%02d  %s\n", gBstLog.date[Index].Month,gBstLog.date[Index].Day,gBstLog.date[Index].Year,gBstLog.time[Index].Hour,gBstLog.time[Index].Minute,gBstLog.time[Index].Second,CharPtrString );
                    HiiSetString (
                        HiiHandle,
                        StringRef,
                        CharPtrString1,
