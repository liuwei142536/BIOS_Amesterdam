  ...
  OUT EFI_FRB_STAT                     *FrbStatData )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entered... FrbType: %x \n", __FUNCTION__, FrbType));
    if (FrbStatData == NULL) {
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                &FrbTimerDataSize);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_APP_GET_WATCHDOG_TIMER Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    //
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Exiting... FrbStatData->FrbEnabled: %x FrbStatData->FrbFailed: %x \n", __FUNCTION__, FrbStatData->FrbEnabled, FrbStatData->FrbFailed));
  ...
  IN UINTN                            *Timeout )
{
   ...
   SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entered... FrbType: %x *Timeout: %x \n", __FUNCTION__, FrbType, *Timeout));
  ...
  IN FRB_SYSTEM_TIMEOUT_INTERRUPT       FrbTimeoutInterrupt )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entered... FrbType: %x FrbTimeoutInterrupt: %x \n", __FUNCTION__, FrbType, FrbTimeoutInterrupt));
  ...
  IN FRB_SYSTEM_BOOT_POLICY           FrbBootPolicy )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entered... FrbType: %x FrbBootPolicy: %x \n", __FUNCTION__, FrbType, FrbBootPolicy));
  ...
  IN BOOLEAN                            ClearFrbGlobalStatus )
{
    ...
    if (FrbType == EfiFrb2) {
        ...
        if (HobList.Header->HobType != EFI_HOB_TYPE_HANDOFF) {
            SERVER_IPMI_DEBUG ((DEBUG_ERROR, "(DxeFrb) Hand off HOB missing!\n"));
            return EFI_NOT_FOUND;
        }
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8*)&GetWatchDogTimerData,
                &FrbTimerDataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a: IPMI_APP_GET_WATCHDOG_TIMER Status : %r\n", __FUNCTION__, Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    //
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_APP_SET_WATCHDOG_TIMER Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    //
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a: IPMI_APP_RESET_WATCHDOG_TIMER Status: %r \n", __FUNCTION__, Status));
  ...
  IN EFI_FRB_TYPE                     FrbType )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a: IPMI_APP_SET_WATCHDOG_TIMER Status: %r \n", __FUNCTION__, Status));
    return Status;
}
...
/**
    ...
    In legacy boot. - When EFI_AMI_LEGACYBOOT_PROTOCOL_GUID gets signaled
  ...
  IN VOID             *Context )
{
    ...
    EFI_STATUS      Status;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Legacy boot or UEFI Boot - Disabling FRB and enabling OS WDT.\n"));
...
VOID ) 
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " Status(Oswdt expiry function) = %x\n", Status));
    return Status;
}
  ...
  IN EFI_SYSTEM_TABLE     *SystemTable )
{
    ...
    if (gFrbProtocol == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for gFrbProtocol! \n"));
    ...
    Status = gBS->InstallProtocolInterface (
                ...
                gFrbProtocol );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiSmFrbProtocolGuid Install protocol : Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "GetVariable Status of ServerSetup  %r \n", Status));
    if(EFI_ERROR(Status)) {
  ...
  IN VOID             *Context )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Entering Setup. Disabling FRB/OS WDT if they are active.\n"));
    gTimerCtlFlag = TRUE;
    if ( gServerMgmtConfiguration.Frb2Enable ) {
         Status = DisableFrb (
                   gFrbProtocol,
      ...
      if ( gServerMgmtConfiguration.OsBootWdtEnable ) {
           ...
           SERVER_IPMI_DEBUG ((DEBUG_INFO, "gTimerCtlFlag = %x\n", gTimerCtlFlag));
      }
    }
}
  ...
  IN VOID             *Context )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : OPROM Scan. Disable/reenable FRB wdt.\n"));
...
{
    EFI_STATUS      Status;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Password Prompt Enter. Disable FRB wdt.\n"));
    Status = DisableFrb (
               gFrbProtocol,
               EfiFrb2);
...
{
    EFI_STATUS      Status;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Password Prompt Exit. Enable FRB wdt.\n"));
    Status = EnableFrb (
               gFrbProtocol,
               EfiFrb2,
               FALSE,
...
{
    EFI_STATUS              Status;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Prompt Timeout Enter. Disable FRB wdt.\n"));
    Status = DisableFrb (
               gFrbProtocol,
               EfiFrb2);
...
{
    EFI_STATUS              Status;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Prompt Timeout Exit. Enable FRB wdt.\n"));
    Status = EnableFrb (
               gFrbProtocol,
               EfiFrb2,
               FALSE,
...
DisableOsWdtBeforeShell (
  ...
  IN VOID             *Context) {
    ...
    EFI_STATUS Status;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Shell Enter. Disable OS WDT.\n"));
    Status = DisableFrb (
               gFrbProtocol,
               EfiOsBootWdt );
...
DisableEnableOsWdtTimer (
  VOID ) {
    ...
    if (gServerMgmtConfiguration.OsBootWdtEnable) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Shell Exit. Enable/Disable OS WDT.\n"));
        ...
        if (gTimerCtlFlag == TRUE) {
          SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : gTimerCtlFlag is set.Boot was done from Setup.Stopping Os Wdt.\n"));
          Status = FrbProtocol->DisableFrb (
                     FrbProtocol,
                     EfiOsBootWdt );
          DEBUG ((DEBUG_INFO, " OS WDT is stopped in setup Status = %r\n",Status));
        } else {
            Status = FrbProtocol->EnableFrb (
                       ...
                       FALSE,
                       FALSE);
            DEBUG ((DEBUG_INFO, " OS WDT is enabled on exiting shell Status = %r\n",Status));
        }
    }
}
...
ClearFlag (
  IN EFI_EVENT        Event,
  IN VOID             *Context) {
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI FRB : Boot option booted from setup.Clearing flag.\n"));
    gTimerCtlFlag = FALSE;
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "Flag set to False.\n"));
}
