  ...
  IN UINT8                  Command )
{
    ...
    for (Index = 0; Index < TableSize-1; Index++) {
      if (NetFunction == gCommandSpecificRetryCountTable[Index].NetFunction &&\
          ...
          SERVER_IPMI_DEBUG((DEBUG_INFO,"Retry Counter value changed\n"));
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO,"Retry counter value not changed as there is no match\n"));
#if (IPMI_SYSTEM_INTERFACE == KCS_INTERFACE)
  ...
  IN VOID                             *Context )
{
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO,"Counter value is %x \n",IpmiInstance->Interface.BT.BtDelay));
#endif
