...
#if defined(IPMI_SEPARATE_DXE_SMM_INTERFACES) && (IPMI_SEPARATE_DXE_SMM_INTERFACES == 1)
/*
  //
  ...
  IN CONST EFI_PEI_SERVICES          **PeiServices )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "Check for Power Failure PWR_FLR bit: %x \n", \
    ...
    if (PciRead8 ((UINTN) PCI_LIB_ADDRESS(LPC_BUS, LPC_DEVICE, LPC_FUNC, LPC_GEN_PMCON_3)) & PWR_FLR_BIT) {
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Power Failure PWR_FLR bit for AC power loss. \n"));
        return TRUE;
    }
