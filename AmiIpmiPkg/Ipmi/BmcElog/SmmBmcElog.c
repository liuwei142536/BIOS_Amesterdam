  ...
  UINT8            *ReserveId )
{
    ...
    while (TRUE) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_CLEAR_SEL Status: %r ClearSelResponse: %x Counter: %x\n", Status, ClearSelResponse, Counter));
  ...
  VOID )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8 *) &SelInfo,
                (UINT8 *) &SelInfoSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_GET_SEL_INFO Status: %r (SelInfo.OperationSupport & OVERFLOW_FLAG_MASK): %x\n", Status, (SelInfo.OperationSupport & OVERFLOW_FLAG_MASK)));
    if (EFI_ERROR (Status)) {
        return FALSE;
    }
...

{
    ...
    if (Size > sizeof (EventMessage)) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, " Request data field length limit exceeded. Status: EFI_UNSUPPORTED \n"));
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                NULL,
                &ResponseDataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_SENSOR_PLATFORM_EVENT_MESSAGE Status: %r \n", Status));
...
#else
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8 *) &ResponseData,
                (UINT8 *) &ResponseDataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_ADD_SEL_ENTRY Status: %r RecordId: %x \n", Status, *((UINT16*) (&ResponseData) )));
    if (!EFI_ERROR (Status)) {
  ...
  IN OUT UINT64                         *RecordId )
{
    ...
    if (*Size < sizeof (IPMI_SEL_EVENT_RECORD_DATA)) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, " Response data field length is small. Status: EFI_BUFFER_TOO_SMALL \n"));
  ...
  SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_GET_SEL_ENTRY Status: %r Next RecordId: %x \n", Status, SelResponseData.NextSelRecordId ));
  if (EFI_ERROR (Status)) {
      return Status;
  }
  //
  ...
  IN OUT UINT64                         *RecordId )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8 *) &SelInfo,
                (UINT8 *) &SelInfoSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_GET_SEL_INFO Status: %r\n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    if ((RecordId != NULL) && ((SelInfo.OperationSupport & DELETE_SEL_ENTRY_COMMAND_SUPPORTED_MASK) == FALSE)) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, " Deleting Single record is not supported. Status: EFI_UNSUPPORTED\n"));
        return EFI_UNSUPPORTED;
    }
    ...
    if (RecordId == NULL) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_CLEAR_SEL Status: %r ClearSelResponse: %x\n", Status, ClearSelResponse));
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_DELETE_SEL_ENTRY Status: %r Completion code: %x\n", Status, gIpmiTransport->CommandCompletionCode));
        if (EFI_ERROR(Status)) {
            return Status;
        }
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " Deleted SEL entry Record Id : %x \n", *((UINT16*)(&DeleteSelRecResponse))));
  ...
  OUT BOOLEAN                            *ElogStatus )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                ...
                (UINT8 *) &ResponseDataSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_APP_GET_BMC_GLOBAL_ENABLES Status: %r BmcGlobalEnables.SystemEventLogging: %x \n", Status, BmcGlobalEnables.SystemEventLogging ));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    //
    ...
    if (EnableElog == NULL) {
        *ElogStatus = BmcGlobalEnables.SystemEventLogging;
        ...
        Status = gIpmiTransport->SendIpmiCommand (
                    ...
                    (UINT8 *) &ResponseDataSize );
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_APP_SET_BMC_GLOBAL_ENABLES Status: %r CompletionCode: %x \n", Status, CompletionCode ));
    }
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " BmcGlobalEnables.SystemEventLogging: %x \n", BmcGlobalEnables.SystemEventLogging ));
    return Status;
}
  ...
  IN EFI_SYSTEM_TABLE       *SystemTable )
{
    ...
    Status = gSmst->SmmLocateProtocol (
                ...
                NULL,
                (VOID **)&gIpmiTransport);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiSmmIpmiTransportProtocolGuid Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "GetDeviceIdResponse.SelDeviceSupport %X\n", GetDeviceIdResponse.SelDeviceSupport));
    if ( !GetDeviceIdResponse.SelDeviceSupport ) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "SEL device is not supported. So returning EFI_UNSUPPORTED \n"));
        return EFI_UNSUPPORTED;
    }

    SERVER_IPMI_DEBUG ((DEBUG_INFO, "GetDeviceIdResponse.SpecificationVersion %X\n", GetDeviceIdResponse.SpecificationVersion));
    if ( !( (GetDeviceIdResponse.SpecificationVersion == IPMI_SPEC_VERSION_1_5) || \
        ...
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "IPMI Spec version is not supported. So returning EFI_UNSUPPORTED \n"));
        return EFI_UNSUPPORTED;
    }
    //
    ...
    if (gRedirProtoPrivate == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for gRedirProtoPrivate! \n"));
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "gEfiRedirElogProtocolGuid protocol  status %r\n", Status));
    if (EFI_ERROR (Status)) {
        //
  ...
  IN OUT UINT8* ReserveId )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                              ...
                              ReserveId,
                              (UINT8 *) &ReserveIdSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_RESERVE_SEL Status: %r ReserveId: %x \n", Status, *((UINT16*)ReserveId)));
    return Status;
}
