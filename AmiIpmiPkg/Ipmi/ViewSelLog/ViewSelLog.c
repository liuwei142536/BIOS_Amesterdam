  ...
  IN  UINT8                     TableSize )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Inside %a\n", __FUNCTION__));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Exit %a\n", __FUNCTION__));
  ...
  IN  UINT32    TimeStamp )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Exiting %a\n", __FUNCTION__));
  ...
  IN  IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    ...
    EFI_STRING_ID      StringRefToken = NULL_EFI_STRING_ID;
    UINTN           Index = 0;
    ...
    if (SelRecordData.RecordType != IPMI_SEL_SYSTEM_RECORD) {
        ...
        CharPtrString = HiiGetString (HiiHandle, STRING_TOKEN(STR_OEM_RECORD_TYPE), NULL);
        //
...
SENSOR_TIME_DETAIL_DONE:
    ...
    StringRefToken = HiiSetString (
                        ...
                        UnicodeSPrintBuffer,
                        NULL) ;
    ASSERT (StringRefToken != NULL_EFI_STRING_ID);
    if (StringRefToken == NULL_EFI_STRING_ID) {
        return StringRefToken;
    }
  ...
  IN  IPMI_SEL_EVENT_RECORD_DATA   SelRecordData )
{
    ...
    EFI_STRING_ID      StringRefToken = NULL_EFI_STRING_ID;
    UINTN           Index = 0;
    EFI_STRING      BmcLunString;
    EFI_STRING      ChannelString;
    ...
    // FE 01 02...
    //
    CharPtrString = HiiGetString (HiiHandle, STRING_TOKEN(STR_SEL_RECORDS_DATA), NULL);
    if (CharPtrString != NULL) {
        UnicodeSPrint (
            UnicodeSPrintBuffer,
            STRING_BUFFER_LENGTH,
    ...
    if (SelRecordData.RecordType != IPMI_SEL_SYSTEM_RECORD) {
        if ((SelRecordData.RecordType >= EFI_SEL_OEM_TIME_STAMP_RECORD_START) &&
            ...
            //
            CharPtrString = HiiGetString (HiiHandle, STRING_TOKEN(STR_MANUFACTURER_ID), NULL);
            if (CharPtrString != NULL) {
                UnicodeSPrint (
                     UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
                     STRING_BUFFER_LENGTH,
    ...
    if ((SelRecordData.GeneratorId & IPMI_SOFTWARE_ID) \
        ...
        //
        CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_GENERATORID_CHANNELNO), NULL);
        ChannelString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_CHANNEL_NUMBER), NULL);
        if ((CharPtrString != NULL) && (ChannelString != NULL)) {
            UnicodeSPrint (
                UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
                STRING_BUFFER_LENGTH,
                CharPtrString,
            ...
            gBS->FreePool (CharPtrString);
            gBS->FreePool (ChannelString);
        }
    } else {
        //
        // Add Channel number and LUN along with Generator string
        //
        CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_BMCLUN_GENERATORID_CHANNELNO), NULL);
        BmcLunString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_BMC_LUN), NULL);
        ChannelString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_CHANNEL_NUMBER), NULL);
        if ((CharPtrString != NULL) && (BmcLunString != NULL) && (ChannelString != NULL))
        UnicodeSPrint (
            UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
            STRING_BUFFER_LENGTH,
            CharPtrString,
    ...
    //
    CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_SENSOR_NUMBER), NULL);
    if (CharPtrString != NULL) {
        UnicodeSPrint (
            UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
            STRING_BUFFER_LENGTH,
    ...
    //
    CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_EVENT_DESCRIPTION), NULL);
    if (CharPtrString != NULL) {
        UnicodeSPrint (
            UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
            STRING_BUFFER_LENGTH,
    ...
    //
    CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_RECORD_TYPE), NULL);
    if (CharPtrString != NULL) {
        UnicodeSPrint (
            UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
            STRING_BUFFER_LENGTH,
    ...
    if (EFI_SEL_EVENT_DIR(SelRecordData.EventDirType) == EFI_SEL_EVENT_DIR_ASSERTION_EVENT) {
        CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_ASSERTION_EVENT), NULL);
    } else {
        CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_DEASSERTION_EVENT), NULL);
    }
    if (CharPtrString != NULL) {
        UnicodeSPrint (
            UnicodeSPrintBuffer + StrLen (UnicodeSPrintBuffer),
...
SENSOR_DETAIL_BUILD_DONE:
    //
    // Add the string to HII string database and get the token value for it.
    // Return if value of the StringRefToken is NULL_EFI_STRING_ID 
    //
    StringRefToken = HiiSetString (
                        HiiHandle,
                        0,
  ...
  IN  IPMI_SEL_EVENT_RECORD_DATA    SelRecordData )
{
    ...
    EFI_STRING_ID          SensorDetailsToken;
    EFI_STRING_ID          SelRecordDescriptionToken;
    ...
    SensorDetailsToken = BuildSelSensorDetailsWithTimeStamp (
                            HiiHandle,
                            SelRecordData );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: SensorDetailsToken: %X \n", SensorDetailsToken));
    ASSERT (SensorDetailsToken != NULL_EFI_STRING_ID);
    if (SensorDetailsToken == NULL_EFI_STRING_ID) {
    ...
    SelRecordDescriptionToken = BuildSelRecordDescription (
                                    HiiHandle,
                                    SelRecordData );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: SelRecordDescriptionToken: %X \n", SelRecordDescriptionToken));
    ASSERT (SelRecordDescriptionToken != NULL_EFI_STRING_ID);
    if (SelRecordDescriptionToken == NULL_EFI_STRING_ID) {
        return EFI_OUT_OF_RESOURCES;
    }
  ...
  IN  VIEW_SEL_LOG_FLAG   ViewSelLogFlag )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Status of HiiGetBrowserData(): %d\n",BrowserDataStatus));
    ...
    BrowserDataStatus = HiiSetBrowserData (
                           ...
                           (UINT8*)&ServerMgmtConfiguration,
                           NULL);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Status of HiiSetBrowserData(): %d\n",BrowserDataStatus));
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Exiting %a\n", __FUNCTION__));
}
  ...
  IN  UINT16            Key )
{
    ...
    if ((Key != VIEW_SEL_LOG_FORM_KEY) && \
        ...
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "ViewSelLog: Callback function is called with \
    ...
    if (CallbackParameters->Action != EFI_BROWSER_ACTION_CHANGING) {
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: CallbackParameters->Action != \
                EFI_BROWSER_ACTION_CHANGING... so return EFI_SUCCESS\n"));
        return  EFI_SUCCESS;
    }

    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: Key: %x \n", Key));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: LocateProtocol for gAmiPostManagerProtocolGuid Status: %r.\n", Status));

    if (!EFI_ERROR (Status)) {
        //
        // Get the Message box strings and display Message box if returned status is not an error.
        //
        MsgBoxTitleString = HiiGetString (
                                HiiHandle,
                                STRING_TOKEN(STR_VIEW_SEL_LOG_TITLE),
                                NULL
                            );
        if (MsgBoxTitleString != NULL) {
            ...
            if (MessageString != NULL) {
                ...
                SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: DisplayMsgBox Status: %r.\n", Status));
                SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: MsgBoxSel value: %x.\n", MsgBoxSel));
                gBS->FreePool (MsgBoxTitleString);
    ...
    Status = SendIpmiCommand (
                ...
                (UINT8 *) &SelInfoSize,
                &CompletionCode );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: IPMI_STORAGE_GET_SEL_INFO Status: %r\n", Status));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: SelInfo.NoOfEntries: %x \n", SelInfo.NoOfEntries));

    //
    // Update No Of Log Entries in STR_SYSTEM_ELOG_COUNT_STRING
    //
    CharPtrString = HiiGetString ( HiiHandle, STRING_TOKEN(STR_LOG_ENTRY_IN_SEL), NULL);
    if (CharPtrString != NULL) {
        CharPtrString1 = AllocateZeroPool (STRING_BUFFER_LENGTH);
        if (CharPtrString1 != NULL){
            UnicodeSPrint(CharPtrString1,STRING_BUFFER_LENGTH, CharPtrString,SelInfo.NoOfEntries); 
            HiiSetString (
                HiiHandle,
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: IsFirstCall: %x\n", IsFirstCall));
    ...
    if (IsFirstCall == TRUE) {
        ...
        if (gSensorDetailsSavedToken == NULL) {
            SERVER_IPMI_DEBUG((DEBUG_ERROR, "Failed to allocate memory for gSensorDetailsSavedToken! \n"));
            return EFI_OUT_OF_RESOURCES;
        }
        ...
        if (gSelRecordDescSavedToken == NULL) {
            SERVER_IPMI_DEBUG((DEBUG_ERROR, "Failed to allocate memory for gSelRecordDescSavedToken! \n"));
            return EFI_OUT_OF_RESOURCES;
        }
    ...
    if (gErrorManager == NULL) {
        SERVER_IPMI_DEBUG((DEBUG_ERROR, "Failed to allocate memory for gErrorManager! \n"));
        return EFI_OUT_OF_RESOURCES;
    }
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: MaxSelPerForm: %x\n", MaxSelPerForm));
    ...
    while (SelResponseData.NextSelRecordId != 0xFFFF) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: RecordId of the event to be read: 0x%02X%02X \n", SelEntry.SelRecID[1], SelEntry.SelRecID[0] ));
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: EFI_STORAGE_GET_SEL_ENTRY Status: %r Next RecordId: %x \n", Status, SelResponseData.NextSelRecordId ));
        //
...
DONE:
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: ViewSelLogFlag: %x \n", ViewSelLogFlag));
    //
    ...
    Status = gRT->GetVariable (
                ...
                &Size,
                gErrorManager);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: gRT->GetVariable Status: %r \n", Status));
    //
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: gRT->SetVariable Status: %r \n", Status));
    //
    // Free the memory allocated for Error Manager
    //
    gBS->FreePool(gErrorManager);

    SERVER_IPMI_DEBUG ((DEBUG_INFO, "ViewSelLog: ViewSelLogCallbackFunction() Exit\n"));
