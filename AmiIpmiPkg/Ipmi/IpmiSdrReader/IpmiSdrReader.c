  ...
  VOID )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                                ...
                                (UINT8 *) &SdrInfo,
                                (UINT8 *) &SdrInfoSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " IPMI_STORAGE_GET_SDR_REPOSITORY_INFO Status: %r\n", Status ));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "SdrVersionNo Address : %x\n", SdrVersionNo));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "SdrVersionNo: %S \n", SdrVersionNo));
