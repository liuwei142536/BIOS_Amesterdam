  ...
  IN  VOID                        *Ppi )
{
    ...
    Status = (*PeiServices)->LocatePpi (
                ...
                &PeiSelPpiDesc,
                (VOID **)&PeiSelPpi);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiPeiSelPpiGuid Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiPeiIpmiTransportPpiGuid Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    //
    ...
    Status = (*PeiServices)->ReInstallPpi (
            (CONST EFI_PEI_SERVICES **)PeiServices,
                PeiSelPpiDesc,
                PeiSelPpiDesc);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " (*PeiServices)->ReInstallPpi Status: %r \n", Status));
  ...
  IN CONST EFI_STATUS_CODE_DATA     *Data OPTIONAL )
{
    ...
    if (((CodeType & EFI_STATUS_CODE_TYPE_MASK) == EFI_PROGRESS_CODE)||
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entry... PeiSelStatusCodeReportWorker Value: %x \n", __FUNCTION__, Value));
        ...
        if (!EFI_ERROR (Status)) {
            Status = PeiSelPpi->EfiSelReportStatusCode (
                        ...
                        CallerId,
                        Data );
            SERVER_IPMI_DEBUG ((DEBUG_INFO, " EfiSelReportStatusCode Status: %r \n", Status));
        }
        ...
        if ((EFI_ERROR(Status)) && ((PeiSelPpi->IpmiTransportPpi)->CommandCompletionCode == INVALID_RESERVATION_ID)) {
           ...
           //
            Status = (PeiSelPpi->IpmiTransportPpi)->SendIpmiCommand (
                        ...
                        (UINT8 *) &ReserveId,
                        (UINT8 *) &ReserveIdSize);
            SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI_STORAGE_RESERVE_SEL Status: %r ReserveId: %4x \n", Status, *((UINT16*)ReserveId)));
            if (EFI_ERROR (Status)) {
                return Status;
            }
  ...
  IN       PEI_SEL_STATUS_CODE_PPI  *PeiSelPpi )
{
    ...
    if(!EFI_ERROR(Status) && ((ClearSelResponse&0x0F) == ERASURE_COMPLETED)) {
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "PEI: SEL Cleared\n"));
        return EFI_SUCCESS;
    }
    Status =  WaitTillErased (PeiSelPpi, ReserveId);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "PEI: WaitTillErased Status : %r\n",Status));
    return Status;
}
  ...
  IN PEI_SEL_STATUS_CODE_PPI          *PeiSelPpi )
{
    ...
    Status = (PeiSelPpi->IpmiTransportPpi)->SendIpmiCommand (
                ...
                (UINT8 *) &SelInfo,
                (UINT8 *) &SelInfoSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "IPMI_GET_SEL_INFO_RESPONSE Status: %r (SelInfo.OperationSupport & OVERFLOW_FLAG_MASK): %x\n", Status, (SelInfo.OperationSupport & OVERFLOW_FLAG_MASK)));
  ...
  IN CONST EFI_STATUS_CODE_DATA     *Data OPTIONAL )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entry... EfiSelReportStatusCode Value: %x \n", __FUNCTION__, Value));
    //
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " FillSelRecord Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return EFI_SUCCESS;
    }

    SERVER_IPMI_DEBUG ((DEBUG_INFO, " (PeiSelPpi->IpmiTransportPpi): %X \n", (PeiSelPpi->IpmiTransportPpi)));
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " (PeiSelPpi->IpmiTransportPpi)-> SendIpmiCommand: %X \n", (PeiSelPpi->IpmiTransportPpi)-> SendIpmiCommand));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " (PeiSelPpi->IpmiTransportPpi)-> SendIpmiCommand Status: %r \n", Status));
  ...
  IN CONST EFI_PEI_SERVICES  **PeiServices )
{
    ...
    Status = (*PeiServices)->LocatePpi (
                ...
                NULL,
                (VOID **)&PeiVariable );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiPeiReadOnlyVariable2PpiGuid status %r\n", Status));
    ...
    if (!EFI_ERROR (Status)) {
        ...
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " PeiVariable->GetVariable status %r \n", Status));
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " SelLogging: %x LogSelStatusCodes: %x \n", ServerMgmtConfiguration.SelLogging, ServerMgmtConfiguration.LogSelStatusCodes));
    }
    if (EFI_ERROR(Status)) {
        //load default value.
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "SelFullBehaviour PCD value is: %d \n", PcdGet8(SelFullBehaviour)));
    ...
    if (PeiSelPpi == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for PeiSelPpi! \n"));
    ...
    if (PeiSelPpiDesc == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for PeiSelPpiDesc! \n"));
        //
    ...
    Status = (*PeiServices)->InstallPpi (
                PeiServices,
                PeiSelPpiDesc );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " (*PeiServices)->InstallPpi Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        //
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " (*PeiServices)->NotifyPpi Status: %r \n", Status));
    ASSERT_EFI_ERROR (Status);
    ...
    Status = (*PeiServices)->LocatePpi (
                ...
                NULL,
                (VOID **) &RscHandlerPpi );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiPeiRscHandlerPpiGuid Status: %r \n", Status));
    if (!EFI_ERROR (Status)) {
        Status = RscHandlerPpi->Register (PeiSelStatusCodeReportWorker);
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " RscHandlerPpi->Register Status: %r \n", Status));
    }
