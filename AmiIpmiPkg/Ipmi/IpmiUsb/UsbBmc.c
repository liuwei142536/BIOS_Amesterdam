...
BotClearEndpoint (
    ...
    if( EFI_ERROR(Status) ) {
       SERVER_IPMI_DEBUG((DEBUG_ERROR, "%a Error : UsbStatus = %x\n", __FUNCTION__, UsbStatus));
    }
    return Status;
  ...
  IN  EFI_IPMI_USB_INSTANCE_DATA    *IpmiUsbInstance )
{
    ...
    if( EFI_ERROR(Status) ) {
        SERVER_IPMI_DEBUG((DEBUG_ERROR, "Dxe BotStatusPhase Error : Status = %r UsbStatus = %x\n",Status,UsbStatus));
        ...
        if( EFI_ERROR(Status) ) {
            SERVER_IPMI_DEBUG((DEBUG_ERROR, "BotStatusPhase Error : Performing RecoveryReset\n"));
  ...
  IN  UINT8                         *RequestData )
{
    ...
    while ( RemainingDataBytes != 0 ) {
        ...
        SERVER_IPMI_DEBUG((DEBUG_INFO, "IpmiRequestSize: %x \n", IpmiRequestSize));
        SERVER_IPMI_DEBUG((DEBUG_INFO, "IpmiRequestOffset: %x \n", IpmiRequestOffset));
        SERVER_IPMI_DEBUG((DEBUG_INFO, "RequestDataOffset: %x \n", RequestDataOffset));
        ...
        SERVER_IPMI_DEBUG((DEBUG_INFO, "RemainingDataBytes: %x \n", RemainingDataBytes));
  ...
  IN  UINT32                        DataSize )
{
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "DataSize: %x \n", DataSize));
    ...
    Status = BotCommandPhase (
                ...
                Sector,
                DataSize );
    SERVER_IPMI_DEBUG((DEBUG_INFO, "BotCommandPhase: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    if ( (Opcode == COMMON_READ_10_OPCODE ) && \
        ...
        Status = ReadResponseData (
                    ...
                    DataSize,
                    Data );
        SERVER_IPMI_DEBUG((DEBUG_INFO, "ReadResponseData: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
        ...
        Status = WriteCommandData (
                    ...
                    DataSize,
                    Data );
        SERVER_IPMI_DEBUG((DEBUG_INFO, "WriteCommandData: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
    } else {
        ...
        Status = UsbIo->UsbBulkTransfer (
                    ...
                    gBulkTransferTimeOut,
                    &UsbStatus );
        SERVER_IPMI_DEBUG((DEBUG_INFO, "UsbIo->UsbBulkTransfer: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
    }
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "BotStatusPhase: Status: %r \n", Status));
    if ( EFI_ERROR(Status)) {
        return Status;
    }
  ...
  IN  UINT32                        DataSize )
{
    ...
    Status = IssueBmcUsbTransaction (
                ...
                (UINT8*)&CmdHeader,
                DataTransferSize );
    SERVER_IPMI_DEBUG((DEBUG_INFO, "Send the Command header: IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    Status = IssueBmcUsbTransaction (
                ...
                IpmiRequest,
                DataTransferSize );
    SERVER_IPMI_DEBUG((DEBUG_INFO, "Send the IPMI packets : IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    do {
        ...
        Status = IssueBmcUsbTransaction (
                    ...
                    (UINT8*)&CmdHeader,
                    DataTransferSize );
        SERVER_IPMI_DEBUG((DEBUG_INFO, "Wait for command completion : IssueBmcUsbTransaction: Status: %r \n", Status));
        if ( EFI_ERROR(Status) ) {
            return Status;
        }
        //
    ...
    switch ( CmdHeader.Status ) {
        case ERR_SUCCESS:
            ...
            SERVER_IPMI_DEBUG((DEBUG_INFO,"gResponseDataSize: %lx \n", gResponseDataSize));
            break;
        case ERR_BIG_DATA:
            SERVER_IPMI_DEBUG((DEBUG_ERROR,"Too much data send to IPMI over USB\n"));
            Status = EFI_INVALID_PARAMETER;
            break;
        case ERR_NO_DATA:
            SERVER_IPMI_DEBUG((DEBUG_ERROR,"Too little data send to IPMI over USB\n"));
            Status = EFI_INVALID_PARAMETER;
            break;
        case ERR_UNSUPPORTED:
            SERVER_IPMI_DEBUG((DEBUG_ERROR,"Unsupported command\n"));
            Status = EFI_UNSUPPORTED;
            break;
        default:
            SERVER_IPMI_DEBUG((DEBUG_ERROR,"Unknown status\n"));
  ...
  IN  UINT32                         *IpmiResponseDataSize )
{
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "%a: DataTransferSize: %x \n", __FUNCTION__, DataTransferSize));
    ...
    Status = IssueBmcUsbTransaction (
                ...
                ResponseData,
                DataTransferSize );
    SERVER_IPMI_DEBUG((DEBUG_INFO, "Get the IPMI response: IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
    ...
    Status = IssueBmcUsbTransaction (
                ...
                (UINT8*)&CmdHeader,
                DataTransferSize );
    SERVER_IPMI_DEBUG((DEBUG_INFO, "Send configuration command: IssueBmcUsbTransaction: Status: %r \n", Status));
    if ( EFI_ERROR(Status) ) {
        return Status;
    }
