  ...
  )
{
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "LocateHandle.... Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        return FALSE;
    }
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "HandleProtocol gEfiUsbIoProtocolGuid .... Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        return FALSE;
    }
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "UsbGetDeviceDescriptor .... Status: %r \n", Status));
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "DevDesc.IdVendor: %X \n", DevDesc.IdVendor));
    SERVER_IPMI_DEBUG((DEBUG_INFO, "DevDesc.IdProduct: %X \n", DevDesc.IdProduct));
    ...
    if ((DevDesc.IdVendor == BMC_USB_INTERFACE_VENDOR_ID) &&\
         ...
         SERVER_IPMI_DEBUG((DEBUG_INFO, "BMC USB interface is found.\n"));
         //
  ...
  )
{
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "UsbGetInterfaceDescriptor: Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        return Status;
    }
    SERVER_IPMI_DEBUG((DEBUG_INFO, "InterfaceDescriptor.NumEndpoints: %X \n", InterfaceDescriptor.NumEndpoints));
    ...
    do {
        ...
        SERVER_IPMI_DEBUG((DEBUG_INFO, "UsbGetInterfaceDescriptor: Status: %r EndPoint: %x \n", Status, EndPoint));
        if (EFI_ERROR(Status)) {
            return Status;
        }
        SERVER_IPMI_DEBUG((DEBUG_INFO, "EndpointDescriptor.Attributes: %x \n", EndpointDescriptor.Attributes));
        SERVER_IPMI_DEBUG((DEBUG_INFO, "EndpointDescriptor.EndpointAddress: %x \n", EndpointDescriptor.EndpointAddress));
        //
        ...
        if ((BulkInEndpointFound != TRUE) && \
            ...
            SERVER_IPMI_DEBUG((DEBUG_INFO, "BulkInEndpointAddr: %x \n", gIpmiUsbInstance->BulkInEndpointAddr));
            ...
            SERVER_IPMI_DEBUG((DEBUG_INFO, "BulkOutEndpointAddr: %x \n", gIpmiUsbInstance->BulkOutEndpointAddr));
        }

        SERVER_IPMI_DEBUG((DEBUG_INFO, "BulkInEndpointFound: %x \n", BulkInEndpointFound));
        SERVER_IPMI_DEBUG((DEBUG_INFO, "BulkOutEndpointFound: %x \n", BulkOutEndpointFound));
  ...
  )
{
    ...
    if (gIpmiUsbInstance == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, " Failed to allocate memory for gIpmiUsbInstance \n"));
        return;
    }
    ...
    SERVER_IPMI_DEBUG((DEBUG_INFO, "GetBulkEndPoints: Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
        SERVER_IPMI_DEBUG((DEBUG_ERROR, "Failed to get Bulk In and Out end point addresses! \n"));
        goto Error;
    }
    ...
    for (Index = 0; Index < IPMI_SELF_TEST_COMMAND_RETRY_COUNT; Index++) {
        ...
        DEBUG((DEBUG_INFO, "Self test result Status: %r Index: %x \n", Status, Index));
        if (!EFI_ERROR(Status)) {
            break;
        }
    }

    SERVER_IPMI_DEBUG((DEBUG_INFO, "Self test result Status: %r \n", Status));

    if (!EFI_ERROR(Status)) {
            SERVER_IPMI_DEBUG((DEBUG_INFO, "ResponseData[0]: %x \n", UsbBstStatus.Result));
            SERVER_IPMI_DEBUG((DEBUG_INFO, "ResponseData[1]: %x \n", UsbBstStatus.Param));
    }
    ...
    if (gIpmiUsbInstance->BmcStatus != EFI_BMC_HARDFAIL) {
        ...
        SERVER_IPMI_DEBUG((DEBUG_INFO, "Install IpmiUsbTransportProtocol - Status: %r\n", Status));
    } else {
        //
  ...
  )
{
    ...
    VOID        *DxeIpmiTransportRegistration;
