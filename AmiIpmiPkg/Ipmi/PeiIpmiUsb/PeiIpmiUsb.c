  ...
  IN  CONST EFI_PEI_SERVICES        **PeiServices )
{
    ...
    do {
        Status = (*PeiServices)->LocatePpi(
                    ...
                    NULL,
                    (VOID **)&BlockIoPpi );
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "gEfiPeiVirtualBlockIoPpiGuid Status %r\n", Status));
        //
        // For error case, print the error status and return
        //
        if (EFI_ERROR (Status)) {
            SERVER_IPMI_DEBUG ((DEBUG_ERROR, "BMC USB interface is not found.\n"));
    ...
    if (IpmiUsbInstance == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for IpmiUsbInstance! \n"));
    ...
    if (PeiIpmiUsbPpiDesc == NULL) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Failed to allocate memory for PeiIpmiUsbPpiDesc! \n"));
        //
    ...
    for (Index = 0; Index < IPMI_SELF_TEST_COMMAND_RETRY_COUNT; Index++) {
        ...
        Status = IpmiUsbInstance->IpmiUsbTransport.SendIpmiCommand (
                    ...
                    (UINT8*)&UsbBstStatus,
                    &DataSize );
        DEBUG ((DEBUG_INFO, "Get Self test results Index: %x Status %r\n", Index, Status));
        if (!EFI_ERROR(Status)) {
    ...
    if (IpmiUsbInstance->BmcStatus != EFI_BMC_HARDFAIL) {
        ...
        Status = (*PeiServices)->InstallPpi (
                    PeiServices,
                    PeiIpmiUsbPpiDesc );
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "Install IpmiUsbTransportPpi - Status: %r\n", Status));
    } else {
        //
