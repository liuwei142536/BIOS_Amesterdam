  ...
  VOID )
{
    ...
    if((!EFI_ERROR(Status)) && ((ClearSelResponse&0x0F) == ERASURE_COMPLETED)) {
        SERVER_IPMI_DEBUG ((DEBUG_INFO, "DXE: SEL Cleared\n"));
        return EFI_SUCCESS;
    }
    Status =  WaitTillErased (ReserveId);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "DXE: WaitTillErased Status : %r\n",Status));
    return Status;
}
  ...
  IN EFI_STATUS_CODE_DATA               *Data OPTIONAL )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "%a Entered... Value: %x \n", __FUNCTION__, Value));
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gIpmiTransport-> SendIpmiCommand Status: %r \n", Status));
    return Status;
}
  ...
  IN EFI_SYSTEM_TABLE   *SystemTable )
{
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiServerMgmtSetupVariableGuid Status: %r \n", Status));
    if (EFI_ERROR(Status)) {
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " ServerMgmtConfiguration.SelLogging: %X \n", ServerMgmtConfiguration.SelLogging));
    ...
    if ( (ServerMgmtConfiguration.SelErase == ERASE_NEXT_BOOT) || (ServerMgmtConfiguration.SelErase == ERASE_EVERY_BOOT) ) {
        ...
        if (ServerMgmtConfiguration.SelErase == ERASE_NEXT_BOOT) {
            ...
            SERVER_IPMI_DEBUG ((DEBUG_INFO, " ServerMgmtConfiguration.SelErase: %X \n", ServerMgmtConfiguration.SelErase));
    ...
    Status = gBS->InstallProtocolInterface (
                    ...
                    &gSelProtocol);
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiSelStatusCodeProtocolGuid Status: %r \n", Status));
    if (EFI_ERROR (Status)) {
        return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
        SERVER_IPMI_DEBUG ((DEBUG_ERROR, "Create event is failing, So returning without registering DxeSelStatusCodeReportWorker handler\n"));
        return Status;
    }
    ...
    SERVER_IPMI_DEBUG ((DEBUG_INFO, " gEfiRscHandlerProtocolGuid Status: %r \n", Status));
    ASSERT_EFI_ERROR (Status);
    if (!EFI_ERROR (Status)) {
        Status = RscHandlerProtocol->Register (DxeSelStatusCodeReportWorker, TPL_HIGH_LEVEL);
        SERVER_IPMI_DEBUG ((DEBUG_INFO, " RscHandlerProtocol->Register Status: %r \n", Status));
    }
  ...
  IN OUT UINT8* ReserveId )
{
    ...
    Status = gIpmiTransport->SendIpmiCommand (
                              ...
                              ReserveId,
                              (UINT8 *) &ReserveIdSize );
    SERVER_IPMI_DEBUG ((DEBUG_INFO, "DXE: IPMI_STORAGE_RESERVE_SEL Status: %r ReserveId: %4x \n", Status, *((UINT16*)ReserveId)));
    return Status;
}
//**********************************************************************
//**********************************************************************
