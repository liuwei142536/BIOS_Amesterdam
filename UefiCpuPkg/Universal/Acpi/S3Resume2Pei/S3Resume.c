...
//*** AMI PORTING BEGIN *****//
//Adding functionality needed for a reset 
  ...
  )
{
  ...
  if (PeiS3ResumeState->ReturnStatus != EFI_SUCCESS) {
    ...
    //*** AMI PORTING BEGIN ***//
    //If error is returned by BootScript Executor will try to reset system, instead of just hanging
    DEBUG (( EFI_D_ERROR, "BootScript Executor Returned an Error! Reseting System!\r\n"));
    {
    	//Reseting System, because BootScript Executed with errors
          ...
          PeiServicesResetSystem2 (EfiResetCold, EFI_SECURITY_VIOLATION, 0, NULL );
