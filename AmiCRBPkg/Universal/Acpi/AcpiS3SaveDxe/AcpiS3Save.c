...
Routine Description:
  ...
  prepare informations and save in ACPI variable set before ready to boot event.
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "S3ToBootEventCallaback at ready to boot event\n"));
  Status = S3Ready (&mS3Save, NULL);
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  ...
  //
  // Create the event to prepare informations and save in ACPI variable set 
  // for S3 resume boot path before ReadyToBoot
  //
  /*
  ...
  ASSERT_EFI_ERROR (Status);
  */
  // Aptio V override - move the event up before we lock SMRAM
  Status = gBS->CreateEvent (
    ...
    DEBUG((-1, "Status after CreateEvent = %r\n", Status));
    Status = gBS->RegisterProtocolNotify (
                    &gEfiDxeSmmReadyToLockProtocolGuid,
                    S3BootEvent,
                    &SmmRegistration
                    );
    DEBUG((-1, "Status after RegisterProtocolNotify = %r\n", Status));
