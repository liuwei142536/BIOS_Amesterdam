  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "Give Control to Aptio S3 Resume Module\n"));
  // ...AMI_OVERRIDE...  >>>
  GuidHob = GetFirstGuidHob (&gEfiPeiS3Resume2PpiGuid);
  pS32 = (EFI_PEI_S3_RESUME2_PPI *) GET_GUID_HOB_DATA (GuidHob);
  DEBUG ((EFI_D_ERROR, "pS32 %x\n", pS32));
  pS32->S3RestoreConfig2(pS32);
  // ...AMI_OVERRIDE...  <<<
  ...
  )
{
 ...
 /*
  ...
  ASSERT_EFI_ERROR (Status);
  DEBUG (( EFI_D_ERROR, "TempEfiBootScriptExecutorVariable = %x\n", TempEfiBootScriptExecutorVariable));
...
/*
//
// Since the memory type of page table is reserved, we do not need to reconstruct page table during s3 resume
//
  if (FeaturePcdGet (PcdDxeIplSwitchToLongMode)) {
    //
