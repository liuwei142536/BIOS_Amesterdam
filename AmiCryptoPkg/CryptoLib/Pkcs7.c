...
/** @file
 Pkcs7.c
 PKCS7 SignedData certificate parsing and processing (RFC 2315 profile)
**/
...
// 
/**
 * Pkcs7_oid_compare - Compare content of 2 OIDs
 * @oid1: OID
 * @oid2: OID
 * Returns: 0 if OID matching, or non-0 if no match
 */
int Pkcs7_oid_compare(struct asn1_oid *oid1, struct asn1_oid *oid2)
{
    int i=-1;

    if(oid1->len == 0 || oid1->len != oid2->len) 
        return i;

    for (i = (int)oid1->len; i > 0; i--) {
...
int Pkcs7_parse_IssuerSerial_sequence(struct pkcs7_signer_info_st *cert, u8 **buf, size_t len)
{
    ...
    wpa_debug_print_name(MSG_MSGDUMP, "PKCS7: Issuer", &cert->issuer);
    ...
    wpa_hexdump(MSG_MSGDUMP, "PKCS7: serialNumber", cert->serial_number,
...
/**
 * pkcs7_parse_signer_info_enc_digest_info - Parse SignerInfo encrypted Digest Info structure
 * @buf: pointer to DER encoded value
 */
int pkcs7_parse_signer_info_enc_digest_info(
    const u8 *buf, size_t len,
    struct pkcs7_signer_info_enc_digest_info_st *info, 
    const u8 **next)
{
    struct asn1_hdr hdr;
    const u8 *pos, *end;
...
/*
1262   61:           SEQUENCE {
1264    9:             OBJECT IDENTIFIER rsaPSS (1 2 840 113549 1 1 10)
1275   48:             SEQUENCE {
1277   13:               [0] {
1279   11:                 SEQUENCE {
1281    9:                   OBJECT IDENTIFIER sha-256 (2 16 840 1 101 3 4 2 1)
         :                   }
         :                 }
1292   26:               [1] {
1294   24:                 SEQUENCE {
1296    9:                   OBJECT IDENTIFIER pkcs1-MGF (1 2 840 113549 1 1 8)
1307   11:                   SEQUENCE {
1309    9:                     OBJECT IDENTIFIER
...
1322    1:                 INTEGER 8
    ...
    */
    ...
    if (asn1_get_next(buf, len, &hdr) < 0 ||
        ...
        wpa_printf(MSG_DEBUG, "PKCS7: Expected SEQUENCE "
               "(enc Digest AlgorithmIdentifier) - found Class %d tag 0x%x",
               hdr.class, hdr.tag);
        return -1;
    }
    pos = hdr.payload;
    ...
    *next = end;
// encDigest ID: rsa+Pkcs1 or +PSS
    if (asn1_get_oid(pos, end - pos, &info->enc_digest_alg.oid,&pos))
        return -1;

// last attriubute
    if (pos >= end ||
        (asn1_get_next(pos, end-pos, &hdr) == 0 &&
         hdr.class == ASN1_CLASS_UNIVERSAL && 
...
//[TAG0]
    if (asn1_get_next(pos, end-pos, &hdr) < 0 ||
        hdr.class != ASN1_CLASS_CONTEXT_SPECIFIC ||
         hdr.tag != 0)
    {
        wpa_printf(MSG_DEBUG, "PKCS7: PSS Expected TAG[0] hashAlgo"
               "- found Class %d "
...
// TAG[1]
    if (asn1_get_next(pos, end-pos, &hdr) < 0 ||
        hdr.class != ASN1_CLASS_CONTEXT_SPECIFIC ||
         hdr.tag != 1)
    {
        wpa_printf(MSG_DEBUG, "PKCS7: PSS Expected TAG[1] pkcs1-MGF"
               "- found Class %d "
               "tag 0x%x", hdr.class, hdr.tag);
...
// saltlen
    if (asn1_get_next(pos, end-pos, &hdr) < 0 ||
        hdr.class != ASN1_CLASS_CONTEXT_SPECIFIC ||
         hdr.tag != 2)
    {
        wpa_printf(MSG_DEBUG, "PKCS7: PSS Expected TAG[2] saltlen"
               "- found Class %d "
               "tag 0x%x", hdr.class, hdr.tag);
...
// Get saltlen
    ...
    if (hdr.class != ASN1_CLASS_UNIVERSAL ||
        hdr.tag != ASN1_TAG_INTEGER) {
        wpa_printf(MSG_DEBUG, "PKCS7: PSS No INTEGER tag found for "
...
// salt is 1 byte????
    if (hdr.length > 1) { 
        wpa_printf(MSG_DEBUG, "PKCS7: PSS Unexpected saltlen field "
               "length %u (expected %u)", hdr.length, 1);
    ...
    info->saltlen = value;
    wpa_printf(MSG_DEBUG, "PKCS7: PSS saltlen %d", info->saltlen );

// skip TAG[3] trailerField as it's currently hardwired to 0xBC
...
struct pkcs7_signer_info_st * Pkcs7_signer_info_common_hdr_parse(u8 **buf, size_t len, int *err)
{
    ...
    const u8 *pos, *next_pos, *end, *next, *next_end;
    struct asn1_oid oid;
    unsigned char *p; 
    size_t left;
    unsigned long value;
    ...
    *err = -1;
    ...
    /* RFC 2315 - PKCS#7 SignerInfo structure / ASN.1 DER */
    ...
    cert->version = value;
    wpa_printf(MSG_DEBUG, "PKCS7: Version X.509v%d", cert->version + 1);
...
////////////////
    ...
    // preserve ptr to next encryptedDigest section
        ...
        while(pos < next)
        {
            ...
            wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: Attribute OID", &oid);
        ...
        */
            if (x509_pkcs_oid(&oid) && oid.len == 7 &&
                ...
                // IDENDIFIER ::= OID 
                if (asn1_get_oid(pos, end - pos, &oid, &next_pos)) {
                     wpa_printf(MSG_DEBUG, "PKCS7: OID Data Expected");

                    return cert;
                }
                asn1_get_oid(pos, end - pos, &cert->authAttr.ContentInfo.oid, &pos);
                wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: contentInfo OID", &cert->authAttr.ContentInfo.oid);
                //
...
/*
                ...
                {
*/
                wpa_printf(MSG_DEBUG, "PKCS7: Supported ContentTypes"
...
//                    return  cert;
        ...
        wpa_printf(MSG_DEBUG, "PKCS7: Missing 'authenticatedAttributes' field");
    }
// end of authenticatedAttributes
    ...
    6977    0:                     NULL
             :                     }
    */
    if (pkcs7_parse_signer_info_enc_digest_info(pos, end - pos,
                        &cert->enc_digest_info, &pos))
    {
        return cert;
    }
    /* encryptedDigest */
    ...
    */
    wpa_printf(MSG_DEBUG, "PKCS7: Encrypted Digest");
                    ...
                    struct pkcs7_signed_data_st *SignedData)
{
    ...
    wpa_printf(MSG_DEBUG, "PKCS7: Check SignerInfo.TimeStamp, len=%x", len);
...
*/
    ...
    if(asn1_get_oid(pos, end-pos, &oid, &pos))
        return err;
    wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: Found OID", &oid);
 ...
 * @buf: pointer to DER encoded value
 * Returns: 0 - no error, non-0 - error
 */
...
//        os_free(cert);
    ...
    // Validate ContentInfo type match in authenticateAttributes
    if(SignedData->Content_start && SignedData->SignerInfo &&
       SignedData->SignerInfo->authAttr.auth_attrib_start) {

        err = Pkcs7_oid_compare(&SignedData->ContentInfo.oid, &SignedData->SignerInfo->authAttr.ContentInfo.oid);
        if(err) {
            wpa_printf(MSG_DEBUG, "PKCS7: Warning. SignerInfo.authAttributes.contenInfo"
                                  "and SignedData.contentInfo Types do not match");
//            return err;
        }
    }
...
*///////////////////////////////////////////////////////
        ...
        wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: digestAlgorithm SHA1/SHA256/SHA384/SHA512 is expected\n\tOID", &oid);
...
*/
    ...
    cert->version = value;
    wpa_printf(MSG_DEBUG, "PKCS7: Version X.509 v%d", cert->version + 1);
...
*/
    ...
    wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: TSAPolicyOID", &oid);
...
*/
        ...
        wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: hashAlgorithm SHA1/SHA256/SHA384/SHA512 is expected\n\tOID", &oid);
...
*/
    ...
    wpa_printf(MSG_DEBUG, "PKCS7: GeneralizedTime: %x",
...
/* DigestAlgorithm.AlgorithmIdentifier */ 
    ...
    wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: Found AlgOID", &(SignedData->Digest_alg.oid));
    ...
    asn1_get_oid(seq_pos, seq_end - seq_pos, &SignedData->ContentInfo.oid, &seq_pos);
    wpa_debug_print_string_oid(MSG_MSGDUMP, "PKCS7: ContentInfo.contentType OID", &SignedData->ContentInfo.oid);
//    seq_pos = pos;
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
...
// This could be AuthVariable Data
    if (x509_pkcs_oid(&oid) &&  oid.len == 7 &&
        ...
        wpa_printf(MSG_DEBUG, "PKCS7: =Data");
        ...
        if(!x509_sha256_oid(&SignedData->Digest_alg.oid)) { 
            wpa_debug_print_string_oid(MSG_DEBUG, "Warning: UEFI AuthVariable SignedData: only SHA256 digestAlgorithm is accepted\n\tOID", &SignedData->Digest_alg.oid);
...
//        pos = seq_end;
        ...
        if (Pkcs7_Microsoft_oid(&oid) && oid.len == 10 &&
            ...
            wpa_printf(MSG_DEBUG, "PKCS7: = spcIndirectDataContext");
        ...
        // 1. Authenticode Microsoft OIDs SPC_INDIRECT_DATA_OBJID  { 1 3 6 1 4 1 311 2 1 4 }  
            ...
            if (err) {
                wpa_printf(MSG_DEBUG, "PKCS7: Error spcIndirectDataContext parser",
                       value);
                // pkcs7_signed_data_free(SignedData);
                return NULL;
            }

        } // SignedData.spcIndirectDataContext OID
        else // /* tstInfo*/
            if (x509_pkcs_oid(&oid) &&  oid.len == 9 &&
                oid.oid[5] == 9  && /* pkcs-9 */
                oid.oid[6] == 16 && /* smime */
                ...
                oid.oid[8] == 4) {
// process tstInfo
                wpa_printf(MSG_DEBUG, "PKCS7: = tstInfo");
                if (value != 3) {
            ...
            {
                wpa_printf(MSG_DEBUG, "PKCS7: = unknown contentType OID"
                   "\n\tExpected types:"
                   "\n\t1.2.840.113549.1.7.1 Data"
                   "\n\t1.3.6.1.4.1.311.2.1.4 spcIndirectDataContext"
                   "\n\t1.2.840.113549.1.9.16.1.4 TimeStamp info");
...
//                     pkcs7_signed_data_free(SignedData);
                ...
                if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
                    ...
                    wpa_printf(MSG_DEBUG, "PKCS7: Error ContentIno.SignedData parser");
...
struct x509_certificate * Pkcs7_return_signing_cerificate(struct pkcs7_signed_data_st *SignedData)
{
    ...
    wpa_debug_print_name(MSG_MSGDUMP, "PKCS7: Sign certificate"
    ...
    for (idx = 0; cert; cert = cert->next, idx++) {
         ...
         wpa_printf(MSG_DEBUG, "PKCS7: %2x", idx);
         wpa_debug_print_name(MSG_MSGDUMP, "\tIssuer", &cert->issuer);
...
// Compare Name and Serial Number
        if (x509_name_compare(&cert->issuer,
            ...
            if (os_memcmp(cert->serial_number, SignerInfo->serial_number, SignerInfo->serial_number_len)) {
    ...
    *reason = (*reason == -1)?X509_VALIDATE_CERTIFICATE_REVOKED:X509_VALIDATE_OK;
    ...
    do {
        ...
        for (cert = SignedData->CertStore, idx = 0; !chain_trusted && cert; cert = cert->next, idx++) {
            ...
            wpa_printf(MSG_DEBUG, "PKCS7: %2x: Chain Cert (%x)", 
                    idx, cert->cert_start);
            wpa_debug_print_name(MSG_MSGDUMP, "\tSubject", &cert->subject);
...
//                    wpa_debug_print_name(MSG_DEBUG, "PKCS7: Signer\n\tIssuer:", &signer->issuer);
...
//            }
            ...
            // hash mismatch keep looking for cert in a chain
                wpa_printf(MSG_DEBUG, "PKCS7: Find a match with revoked Certificate Hash==>");
            ...
            for (trust = trusted; trust; trust = trust->next) {
                  ...
                  wpa_printf(MSG_DEBUG, "PKCS7: Trust Cert(%x)", 
                          trust->cert_start);
                  wpa_debug_print_name(MSG_MSGDUMP, "\tSubject", &trust->subject);
...
/**
 * Pkcs7_parse_digestAlgorithm - Return digestAlgorithm type
 * @param[in]  oid         digest OID
 * @param[out] HashType    hash Type : SHA1, SHA256, SHA384, SHA512
 * @param[out] HashLen     hash buffer length
 * 
 * Returns: 0 - Success, -1 - Failed to detect algorithm
 */
int Pkcs7_parse_digestAlgorithm(struct asn1_oid *oid, u8* HashType, size_t *HashLen)
{
    if(oid == NULL || HashLen == NULL || HashType == NULL)
        return -1;
 ...
 * -1 if not
 */
int Pkcs7_decrypt_validate_digest(
        struct pkcs7_signed_data_st *SignedData,
        struct x509_certificate *SignCert,
        u8 *testHash, size_t hashLen)
{

    if (x509_pkcs_oid(&SignedData->SignerInfo->enc_digest_info.enc_digest_alg.oid) && /* pkcs */
            SignedData->SignerInfo->enc_digest_info.enc_digest_alg.oid.len == 7 &&
            SignedData->SignerInfo->enc_digest_info.enc_digest_alg.oid.oid[5] == 1 /* pkcs-1 */) {
    
        if(SignedData->SignerInfo->enc_digest_info.enc_digest_alg.oid.oid[6] == 10   /* RSASSA-PSS */) {
            wpa_printf(MSG_DEBUG, "PKCS7: RSASSA-PSS signature algorithm");
            return Pkcs7_RsaSsa_Pss_decrypt_validate_digest(SignedData, SignCert, testHash, hashLen);
        }
        if(SignedData->SignerInfo->enc_digest_info.enc_digest_alg.oid.oid[6] == 1   /* RSAencrypt with Pkcs1*/) {
            wpa_printf(MSG_DEBUG, "PKCS7: Rsa PKCS-1 signature algorithm");
            return Pkcs7_Pkcs1_decrypt_validate_digest(SignedData, SignCert, testHash, hashLen);
        }
    }
    wpa_printf(MSG_DEBUG, "PKCS7: Unrecognized signature algorithm");
    return -1;
}


/**
 * Verify signature of certificate encryptedDigest 
 * Returns: 0 if cert has a valid signature that was signed by the issuer,
 * -1 if not
 */
int Pkcs7_Pkcs1_decrypt_validate_digest(
        struct pkcs7_signed_data_st *SignedData,
        struct x509_certificate *SignCert,
        u8 *testHash, size_t hashLen)
{
    ...
    if(SignedData==NULL || SignCert==NULL || testHash==NULL) 
        return err;

    pk = crypto_public_key_import(SignCert->public_key, SignCert->public_key_len);
    if (pk == NULL)
        return err;

    data_len = SignedData->SignerInfo->enc_digest_len;
    ...
    if (crypto_public_key_decrypt_pkcs1(pk, SignedData->SignerInfo->enc_digest, data_len, data, &data_len) < 0) {
    ...
    wpa_hexdump(MSG_MSGDUMP, "PKCS7: Encrypted Pkcs#1.1 Signature data", data, data_len);
...
//        os_free(data);
    ...
    wpa_hexdump(MSG_MSGDUMP, "PKCS7: Decrypted Digest", hdr.payload, hdr.length);
    ...
    if (hdr.length != hashLen ||
        os_memcmp(hdr.payload, (void*)testHash, hdr.length) != 0) {
        wpa_hexdump(MSG_MSGDUMP, "PKCS7: Calculated hash ", testHash, hdr.length);
        wpa_printf(MSG_DEBUG, "PKCS7: Certificate Digest does not match "
 ...
 * -1 if not
 */
int Pkcs7_RsaSsa_Pss_decrypt_validate_digest(
            struct pkcs7_signed_data_st *SignedData,
            struct x509_certificate *SignCert,
            u8 *testHash, size_t hashLen)
{
    ...
    if (crypto_public_key_decrypt_rsa(pk, SignedData->SignerInfo->enc_digest, data_len, data, &data_len) < 0) {
        wpa_printf(MSG_DEBUG, "PKCS7: Failed to decrypt signature");
        crypto_public_key_free(pk);
        os_free(data);
        return err;
    ...
    wpa_hexdump(MSG_MSGDUMP, "PKCS7: Encrypted PSS Signature data ", data, data_len);
    wpa_hexdump(MSG_MSGDUMP, "PKCS7: Calculated hash ", testHash, hashLen);

    if(SignedData->SignerInfo->enc_digest_info.saltlen > hashLen) {
        wpa_printf(MSG_DEBUG, "PKCS7: PSS Saltlen exceed HashLen "
               "length %u (> %u)",SignedData->SignerInfo->enc_digest_info.saltlen, hashLen);
    ...
    modulus_bitlen = data_len << 3;

    if ( 0 != (pkcs_1_pss_decode(testHash, hashLen, data, data_len, SignedData->SignerInfo->enc_digest_info.saltlen, modulus_bitlen)) ) {
        wpa_printf(MSG_DEBUG, "PKCS7: Failed to verify PSS signature");
        os_free(data);

        return -1;
    }

    wpa_printf(MSG_DEBUG, "PKCS7: PSS Certificate Digest matches with "
           "calculated hash");
...
//    }
    ...
    if(0 != Pkcs7_parse_digestAlgorithm(&SignedData->SignerInfo->digest_alg.oid, &hashType, &hashLen))
        return err;
...
// UEFI: AuthVariable Signed Data Certificate must NOT have SignerInfo.authAttributes nor ContentInfo.Content
        ...
        if(SignedData->Digest && fileHash && fileHash_len)
        {
            if(fileHash_len == SignedData->Digest_len) {
                if (os_memcmp(fileHash, SignedData->Digest, fileHash_len) != 0) {
                    ...
                    wpa_printf(MSG_DEBUG, "PKCS7: contentInfo.Digest does not match with calculated message hash");
                    return err;
                }
            } else
                if(fileHash_len == SignedData->Content_len) {
                    if (os_memcmp(fileHash, SignedData->Content_start, fileHash_len) != 0) {
                        ...
                        wpa_hexdump(MSG_MSGDUMP,"PKCS7: Input block    ", fileHash, fileHash_len);
                        wpa_printf(MSG_DEBUG, "PKCS7: SpcIndirectData does not match with input data");
                        return err;
                    }
                }
                else {
    ...
    if(SignedData->SignerInfo->authAttr.auth_attrib_start)
    {
        ...
        if(SignedData->Content_start)
        {
            if( /*Pkcs7_oid_compare(&SignedData->ContentInfo.oid, &SignedData->SignerInfo->authAttr.ContentInfo.oid) != 0 ||*/
                os_memcmp(pHash, SignedData->SignerInfo->authAttr.messageDigest, hashLen) != 0) {
                ...
                wpa_printf(MSG_DEBUG, "PKCS7: SignerInfo.authAttributes.messageDigest does not match with contentInfo digest");
                return err;
            }
        } else
            if(fileHash && (fileHash_len == hashLen)) {
                if (os_memcmp(fileHash, SignedData->SignerInfo->authAttr.messageDigest, fileHash_len) != 0) {
                    ...
                    wpa_hexdump(MSG_MSGDUMP,"PKCS7: AuthAttrib.messageDigest ", SignedData->SignerInfo->authAttr.messageDigest, SignedData->SignerInfo->authAttr.messageDigest_len);
                    wpa_hexdump(MSG_MSGDUMP,"PKCS7: Input block\t\t", fileHash, fileHash_len);
                    wpa_printf(MSG_DEBUG, "PKCS7: SignerInfo.authAttributes.messageDigest does not match with input data");
                    return err;
                }
            }
    ...
    return Pkcs7_decrypt_validate_digest(SignedData, SignCert, pHash, hashLen);
}
/**
        ...
        os_time_t *TimeOfSigning)
{
    ...
    if(SignedData->tstSignedData && SignedData->/*tstSignedData->*/tstInfo) {
        ...
        if (hashLen != SignedData->tstInfo->authAttr.messageDigest_len ||
            ...
            wpa_printf(MSG_DEBUG, "PKCS7: TimeStamp messageDigest does not match "
...
/**
  ...
  @param[out] HashType            0-SHA1, 1-SHA256, 2-SHA384, 3-SHA512, 4-MD5

  @retval 0 - Success, non-zero - Failed to detect algorithm

**/
#define TWO_BYTE_ENCODE 0x82

int Pkcs7_return_digestType (const u8* AuthData, size_t AuthDataSize, u8* HashType)
{
    const u8 *pos, *end;
    struct asn1_oid oid;
    size_t    offs;
    ...
    if(AuthData == NULL || HashType==NULL)
        return err;

    if ((*(u8*)(AuthData + 1) & TWO_BYTE_ENCODE) != TWO_BYTE_ENCODE) {
      //
    ...
    // Default AuthData is expected Pkcs#7v1.5 SignedData construct
    //        SEQUENCE {
    //            version Version,
    //            digestAlgorithms DigestAlgorithmIdentifiers,
    // The Hash algorithm OID is located at offset 11
    offs = 11;
    //
    // Check if AuthData is PE/COFF Authenticode.
    ...
    if (!asn1_get_oid(pos, end-pos, &oid, &pos) &&
        ...
        oid.oid[5] == 7 /* PKCS7*/ &&
        oid.oid[6] == 2 /* SignedData*/) {
        wpa_printf(MSG_DEBUG, "PKCS7: Detected Authenticode SignedData signature algorithm");
    ...
    if(0 != Pkcs7_parse_digestAlgorithm(&oid, HashType, &hashLen))
        return err;
 ...
 *  PKCS7cert: List of trusted certificates
  * Returns: 0 - Success, non-zero - Failed to detect algorithm
 */
int Pkcs7_return_digestAlgorithm(struct pkcs7_signed_data_st *PKCS7cert, u8* HashType)
{
    size_t hashLen = 0;

    if(PKCS7cert == NULL || HashType==NULL)
        return -1;

    if(0 != Pkcs7_parse_digestAlgorithm((struct asn1_oid *)&PKCS7cert->Digest_alg.oid, HashType, &hashLen))
        return -1;
                                     ...
                                     u8 *buf, size_t len)
{
    ...
    for (i = 0; i < name->num_attr; i++) {
        if (name->attr[i].type == X509_NAME_ATTR_CN) {
            res = os_snprintf((char *)buf, len, "%a", name->attr[i].value);
            return (res < 0 || res >= (int)len)?-1:0;
        }
    }
    // CN not found, return full Subject string
    ...
    u8** CertList, size_t* ListLength)
{
  ...
  // run validate chain with NULL in Trust field
    ...
    for (idx = 1; cert; cert = cert->next) {
        ...
        wpa_printf(MSG_DEBUG, "PKCS7: %2x: Chain Cert(%x)", idx, cert->chain);
        wpa_debug_print_name(MSG_MSGDUMP, "\tSubject", &cert->subject);
