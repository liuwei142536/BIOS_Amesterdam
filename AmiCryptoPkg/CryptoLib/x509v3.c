...
#endif
//*** END AMI W/A ***//
#ifndef CONFIG_IGNORE_X509_EXT_ALT_NAME_KEY_USAGE
...
//			x509_free_name(name);
		...
		val = (char*)os_malloc(hdr.length + 1);
		if (val == NULL) {
//			x509_free_name(name);
			return -1;
		}
...
void x509_name_string(struct x509_name *name, char *buf, size_t len)
{
	...
	for (i = 0; i < name->num_attr; i++) {
		ret = os_snprintf(pos, end - pos, "%a=%a, ",
	...
	if (name->email) {
		ret = os_snprintf(pos, end - pos, "/emailAddress=%a",
				  name->email);
//		if (os_snprintf_error(end - pos, ret))
		if (ret < 0 || ret >= end - pos)
			goto done;
			       ...
			       struct x509_certificate *cert, const u8 **next)
{
	...
	*next = pos + plen;
	...
	wpa_printf(MSG_MSGDUMP, "X509: Validity: notBefore: %lx notAfter: %lx",
				       ...
				       const u8 *pos, size_t len)
{
	...
	if (os_strlen(name->alt_email) != len) {
		wpa_printf(MSG_INFO, "X509: Reject certificate with "
			   "embedded NUL byte in rfc822Name (%a[NUL])",
			   name->alt_email);
		os_free(name->alt_email);
		name->alt_email = NULL;
		return -1;
				   ...
				   const u8 *pos, size_t len)
{
	...
	if (os_strlen(name->dns) != len) {
		wpa_printf(MSG_INFO, "X509: Reject certificate with "
			   "embedded NUL byte in dNSName (%a[NUL])",
			   name->dns);
		os_free(name->dns);
		name->dns = NULL;
		return -1;
...
// AMI //
    wpa_debug_print_string_oid(MSG_MSGDUMP, "X509: altName - registeredID", &name->rid);
//	asn1_oid_to_str(&name->rid, buf, sizeof(buf));
//	wpa_printf(MSG_MSGDUMP, "X509: altName - registeredID: %a", buf);
...
//		return -1;
	...
	while (pos < end) {
		...
		wpa_printf(MSG_DEBUG, "ExtKeyUsage KeyPurposeId: %a", buf);
	}
...
#endif //x509_parse_extension_data
...
//*** AMI W/A ***//
    wpa_debug_print_string_oid(MSG_MSGDUMP, "X509: Extension: extnID=", &oid);
    wpa_printf(MSG_DEBUG, "X509: Extension: critical=%d", critical_ext);
//	asn1_oid_to_str(&oid, buf, sizeof(buf));
//	wpa_printf(MSG_DEBUG, "X509: Extension: extnID=%a critical=%d",
//		   buf, critical_ext);
...
//*** AMI W/A ***//
        wpa_printf(MSG_INFO, "X509: Unknown critical extension");
//		wpa_printf(MSG_INFO, "X509: Unknown critical extension %a",
//			   buf);
...
//	wpa_printf(MSG_MSGDUMP, "X509: issuer %a", sbuf);
    wpa_debug_print_name(MSG_MSGDUMP, "X509: issuer", &cert->issuer);
...
//	wpa_printf(MSG_MSGDUMP, "X509: subject %a", sbuf);
    wpa_debug_print_name(MSG_MSGDUMP, "X509: subject", &cert->subject);
				     ...
				     struct x509_certificate *cert)
{
    if (x509_pkcs_oid(&cert->signature.oid) &&
        ...
        if(cert->signature.oid.oid[6] == 10   /* RSASSA-PSS */) {
            wpa_printf(MSG_DEBUG, "X509: RSASSA-PSS signature algorithm");
//            return x509_check_RsaSsa_Pss_signature(issuer, &cert->signature,
//                            cert->sign_value, cert->sign_value_len,
//                            cert->tbs_cert_start, cert->tbs_cert_len);
        }
        /* RSA with Pkcs1*/
        wpa_printf(MSG_DEBUG, "X509: Rsa PKCS-1 signature algorithm");
...
/* AMI FIX */		
	...
	default:
		wpa_printf(MSG_INFO, "X509: Unsupported certificate signature "
			   "algorithm (%x)", signature->oid.oid[6]);
		os_free(data);
		return -1;
	}
#endif
	// AMI
	if(0 != crypto_calculate_digest(hashType, &signed_data, &signed_data_len, hash, &hash_len))
		return -1;
...
//		wpa_printf(MSG_DEBUG, "X509: %x: %a", idx, buf);
		wpa_debug_print_name(MSG_DEBUG, "X509: chain subject", &cert->subject);
...
/*				x509_name_string(&cert->issuer, buf,
						 sizeof(buf)); 
				wpa_printf(MSG_DEBUG, "X509: cert issuer: %a",
					   buf);
...
*/				*reason = X509_VALIDATE_CERTIFICATE_UNKNOWN;
			...
			if ((cert->next->extensions_present &
				...
				wpa_printf(MSG_DEBUG, "X509: pathLenConstraint"
					   " not met (idx=%lx issuer "
					   "pathLenConstraint=%lx)", idx,
